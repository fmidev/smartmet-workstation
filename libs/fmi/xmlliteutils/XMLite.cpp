...
{
	// find the end of pi
    LPTSTR end = _tcsenistr(pszXml, szXMLPIClose, _tcslen(szXMLPIClose), pi ? pi->escape_value : 0);
	if( end == NULL )
		return NULL;
	...
	if( doc )
	{
        ...
        xml += _tcslen(szXMLPIOpen);
        TCHAR* pTagEnd = _tcspbrk(xml, _TEXT(" ?>"));
...
LPTSTR _tagXMLNode::LoadComment( LPCTSTR pszXml, LPPARSEINFO pi /*= &piDefault*/ )
{
    ...
    LPTSTR end = _tcsenistr(pszXml, szXMLCommentClose, _tcslen(szXMLCommentClose), pi ? pi->escape_value : 0);
	if( end == NULL )
		return NULL;
...
LPTSTR _tagXMLNode::LoadCDATA( LPCTSTR pszXml, LPPARSEINFO pi /*= &piDefault*/ )
{
    ...
    LPTSTR end = _tcsenistr(pszXml, szXMLCDATAClose, _tcslen(szXMLCDATAClose), pi ? pi->escape_value : 0);
	if( end == NULL )
		return NULL;
...
LPTSTR LoadOtherNodes( LPXNode node, bool* pbRet, LPCTSTR pszXml, LPPARSEINFO pi /*= &piDefault*/ )
{
	...
	*pbRet = false;
	...
	while( xml && do_other_type )
	{
        ...
        if(_tcsnicmp(xml, szXMLPIOpen, _tcslen(szXMLPIOpen)) == 0)
		{
		...
		// is comment Node?
        if(_tcsnicmp(xml, szXMLCommentOpen, _tcslen(szXMLCommentOpen)) == 0)
		{
		...
		prev = xml;
		// is CDATA Node?
        if(_tcsnicmp(xml, szXMLCDATAOpen, _tcslen(szXMLCDATAOpen)) == 0)
		{
