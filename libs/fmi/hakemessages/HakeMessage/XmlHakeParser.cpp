...
namespace HakeMessage
{
    ...
    HakeMsg XmlHakeParser::parse(const std::string &xmlString)
    {
        ...
        try
        {
            // puretaan kategoorinen id numero ja suuruusluokka irti type-stringist�, joka on siis muotoa:
            ...
            { // luultavasti category stringiss� on merkki per�ss� kuten 403B, joka on yleinen juttu, yritet��n irroittaa t�ll�isess� tapauksessa kategoria stringist�
                if(categoryParts[0].size() > 2)
                {
                    // rakennetaan uusi stringi, miss� per�st� on otettu pois viimeinen merkki, ja kokeillaan konversiota uudestaan
        ...
        { // ei l�ytynyt viestist� katu osoitetta, laitetaan ??? tilalle, koska t�m� n�ytt�� puuttuvan usein viesteist�
        ...
        try
        {
            ...
            // PIT�� TEHD� world xy muunnos YKJ-arean avulla latlon koordinaatistoon
