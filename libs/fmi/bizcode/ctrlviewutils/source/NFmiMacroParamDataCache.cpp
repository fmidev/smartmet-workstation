...
void NFmiMacroParamLayerCacheDataType::getCacheValues(NFmiDataMatrix<float> &dataMatrixOut, bool &useCalculationPointsOut, bool &useAlReadySpacedOutDataOut, std::shared_ptr<NFmiFastQueryInfo> &usedInfoInOut)
{
    ...
    {
        // Jos cacheen laskettu data oli jo harvennettua, pit�� my�s nyt k�ytetty info s��t�� takaisin kyseiseen oikeaan hilakokoon.
...
void NFmiMacroParamDataCacheRow::setCache(unsigned long layerIndex, const NFmiMetTime &time, const std::string &macroParamTotalPath, const NFmiMacroParamLayerCacheDataType &cacheData)
{
    ...
    {
        // Pit�� luoda uusi layer-cache, koska haettua layeri� ei l�ytynyt
...
// Tehd��n cachen n�ytt�riville p�ivitys. Jos esim. jotain on poistettu tai lis�tty rivilt�/riville.
...
bool NFmiMacroParamDataCacheRow::update(NFmiDrawParamList &drawParamList)
{
    ...
    for(auto &drawParam : drawParamList)
    {
        if(drawParam->IsMacroParamCase(true))
        {
            ...
            {
                // Jos ei l�ytynyt listasta valmista layerCachea, niin luodaan sitten uusi sellainen
...
bool NFmiMacroParamDataCacheRow::tryToMoveExistingLayerCache(unsigned long layerIndex, const NFmiDrawParam &drawParam, LayersCacheType &newLayersCacheInOut)
{
    ...
    {
        // Katsotaan l�ytyik� sama macroParam samasta kohtaa layer-listasta
        if(layerIter->second.macroParamTotalPath() == drawParamTotalPath)
        {
            // Jos l�ytyi, siirret��n cache originaalista uuteen cacheen ja poistetaan se originaalista
            swapCacheDataFromOriginalToNew(layerIndex, layersCache_, layerIter, newLayersCacheInOut);
            return true;
        }
    }

    // Etsit��n 1. originaali listasta l�ytynyt oikealla macroParam tiedostopolulla oleva otus ja 
...
{
    // Ei menn� n�ytt�tasoa pidemm�lle kun tehd��n t�ysi tyhjennys
    rowsCache_.clear();
}
...
void NFmiMacroParamDataCacheForView::setCache(unsigned long rowIndex, unsigned long layerIndex, const NFmiMetTime &time, const std::string &macroParamTotalPath, const NFmiMacroParamLayerCacheDataType &cacheData)
{
    ...
    {
        // Pit�� luoda uusi row-cache, koska haettua rivi� ei l�ytynyt
...
// T�t� kutsutaan kun mm. ladataan n�ytt�makroa ja kaikki macroParam cachet pit�� ensin poistaa systeemist�
...
{
    // N�ytt�-cacheja ei tyhjennet� ikin�, siksi halutun n�yt�n pit�� aina l�yty�!
