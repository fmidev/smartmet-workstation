...
// ************************************************************
// ** T�st� alkaa NFmiMTATempSystem::HodografViewData osio ****
// ************************************************************

void NFmiMTATempSystem::HodografViewData::AdjustScaleMaxValue(short theDelta)
{
	// s��det��n arvoalueen kokoa riippuen annetusta hiiren rullan suunnasta (theDelta)
	if(theDelta > 0)
		itsScaleMaxValue -= 10;
	else
		itsScaleMaxValue += 10;

	// Rajataan muuttujan koko 10 ja 150 v�lille
...
{
	// s��det��n suhteellista kokoa riippuen annetusta hiiren rullan suunnasta (theDelta)
	if(theDelta > 0)
		itsRelativiHeightFactor -= 0.05;
	else
		itsRelativiHeightFactor += 0.05;

	// Rajataan muuttujan koko 0.2 ja 0.6 v�lille
...
// **********************************************************
// ** T�st� alkaa NFmiMTATempSystem::ServerProducer osio ****
// **********************************************************
...
// ****************************************************
// ** T�st� alkaa NFmiMTATempSystem::TempInfo osio ****
// ****************************************************
...
// ******************************************
// ** T�st� alkaa NFmiMTATempSystem osio ****
// ******************************************
...
// N�ille viiva asetuksille pit�� antaa valmiit oletusarvot, koska n�it� ei v�ltt�m�tt� l�ydy asetuksista
...
void NFmiMTATempSystem::Init(NFmiProducerSystem &theProducerSystem, const std::vector<NFmiProducer>& theExtraSoundingProducers, const SoundingViewSettingsFromWindowsRegisty& soundingViewSettingsFromWindowsRegisty)
{
	...
	fInitializationOk = true; // jos ei poikkeuksia lent�nyt laitetaan true:ksi
}
...
static std::string MakeValueStr(const std::vector<double> &theValues)
{
	...
	for(int i = 0; i < ssize; i++)
	{
		...
		if(i<ssize-1) // viimeisen per��n ei laiteta pilkkua
			str += ",";
	}
	return str;
}
...
{
    try
    { // yritet��n hakea asetuksista arvoja, mutta jos niit� ei l�ydy, otetaan poikkeus kiinni ja j�tet��n oletusarvot
...
void NFmiMTATempSystem::InitializeSoundingColors(void)
{
	...
	// ********* initialisoidaan viel� luotaus n�yt�n kaikki viiva systeemit ********
	// n�m� otetaan lopuksi konffi filest�, mutta nyt alustetaan t�st�
...
void NFmiMTATempSystem::StoreSettings(void)
{
	if(fInitializationOk == false)
		return; // jos alustus on ep�onnistunut, ei talletuksia tehd�, koska tuloksena voi olla roskaa
	...
	for(int i=0; i<colorCount; i++)
	{
		// v�rin talletus vaatii hieman kikkailua stringstreamin kanssa
		std::string settingStr(colorText + NFmiStringTools::Convert<int>(i+1)); // colorText-stringiin pit�� saada j�rjestys numero per��n
...
{
    // 4. extra tuottajat, jotka saadaan my�s docilta
...
{
    // 2. malli datat jotka on m��ritelty docin producer listassa
    ...
    for(auto producerIndex = 0u; producerIndex < static_cast<unsigned int>(producers.size()); producerIndex++)
    {
        ...
        {
            NFmiProducerInfo &producerInfo = theProducerSystem.Producer(producerIndex + 1); // +1 johtuu producersystemin 1-pohjaisesta indeksi systeemist�
            NFmiProducer producer = producerInfo.GetProducer();
            const unsigned long nameLengthLimit = 12; // Jos tuottajan normaalinimi menee yli t�m� rajan, k�ytet��n ShortName:a
...
{ 
	...
	// ignoreHighLimit tapaus kiinnostaa vain n�ytt�makron latauksen yhteydess�
...
void NFmiMTATempSystem::Write(std::ostream& os) const
{
    ...
    extraData.Add(::MakeProducerContainerServerUsageString(itsSoundingComparisonProducers)); // 2. uusi string arvo extroissa on valittujen tuottajien server/local data k�ytt�tila tyyliin "0 1 0 0"
	extraData.Add(itsHodografViewData.GenerateSettingsString()); // 3. uusi string arvo extroissa on hodografi s��d�t
	extraData.Add(MakeSelectedProducerStringForViewMacro()); // 4. uusi string arvo extroissa on valitun tuottajan indeksiin liittyv�t arvot
...
// Tehd��n stringi, jossa on valitun tuottajan producer-id ja valitun tuottajan index pilkuilla eroteltuna.
// Oletetaan ett� itsSelectedProducerIndex on 2 (eli 3. valituista multivalinta tuottajista) ja se on Gfs
// tuottaja (id = 54), t�ll�in tehd��n seuraava stringi: "2,54"
// Huom! Jos valittu index on suurempi kuin on valittuja tuottajia, k�ytet��n silloin listan viimeist� tuottajaa,
// mutta originaali indeksi�, joka osoittaa tuottaja listan ulkopuolelle.
...
// Jos textual-sounding sivuikkuna on auki, avataan se my�s legacy n�ytt�makroissa, 
...
// Jos jompikumpi tai molemmat 'teksti' sivuikkunoista on auki, on vanhassa systeemiss� ShowSideView = true
...
{ // toivottavasti olet poistanut kommentit luettavasta streamista!!
	...
	{
		// Version 5.13.12.0 j�lkeisill� versioilla tehty n�ytt�makro hanskataan k�ytt�m�ll� extraData arvoja suoraan
	...
	{
		// Versiolla 5.13.12.0 ja sit� edelt�vill� versioilla tehty n�ytt�makro pit�� tehd� erityis legacy metodeilla
...
void NFmiMTATempSystem::InitFromViewMacro(const NFmiMTATempSystem &theOther, bool disableWindowManipulations)
{
	...
	// Pit�isik� my�s itsSoundingDataServerConfigurations asetukset kopioida t�ss�???
...
// Kun luotaus-dialogista annetaan takaisin valitut tuottajat, ne annetaan pelkk�n� tuottaja listana,
// ilman tietoa onko kyseess� serverilt� haettava data vai ei.
// T��ll� p��tell�� tuottajanimien avulla, mitk� tuottajat oikeasti on kyseess�.
