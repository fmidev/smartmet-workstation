...
// koska t�ll�in pit�� piirt�� erilaisia markereit� n�yt�lle ja 
...
void NFmiViewParamsView::DrawMouseDraggingAction()
{
    if(fMouseDraggingAction)
    {
        ...
        {
            bool useTopLine = itsCurrentDragRowIndex < itsCapturedParamRowIndex; // jos tiputus on l�ht�paikkaa ylemp�n�, k�ytet��n paramRectin yl�viivaa
		...
		if(paramList)
		{
			...
			if(paramList->Index(index))
			{
				...
				if(drawParam)
				{
					...
					{
//						if(itsMapViewDescTopIndex == 0) // en saa t�t� toimimaan oikein kuin p��karttan�yt�ll� (joten est�n drawParam muokkauksen muissa ikkunoissa), muissa parametri menee piiloon, enk� saa ikkunoita p�ivitt�m��n ruutua oikeaan tilaan
						{
                            ...
                            // Ik�v� kyll� yks�is klikin j�lkeen (ennen t�t� toista klikki�, josta syntyy tupla klikkaus) 
		...
		if(paramList)
		{
			...
			if(paramList->Index(index))
			{
				...
				if(drawParam)
				{
					...
					{ // jos shift pohjassa poistetaan parametri n�yt�lt�
						paramList->Remove();
                        return DoAfterParamModeModifications(paramList);
                    }
					else
