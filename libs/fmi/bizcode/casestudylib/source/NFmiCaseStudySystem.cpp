		...
		for(size_t i = 0; i < itsCategoriesData.size(); i++)
		{
			...
			for(size_t j = 0; j < producerDataVector.size(); j++)
			{
				...
				for(size_t k = 0; k < dataFileVector.size(); k++)
				{
					...
					size_t pos = ci_find_substr(dataFile.RelativeStoredFileFilter(), patternWithoutPath); // t‰m‰ etsint‰ tyˆ pit‰‰ tehd‰ case-insensitiivisti, tai muuten kaikkien eri konffeissa olevien juttujen pit‰‰ olla tarkalleen samalla lailla kirjoitettu
					if(pos != std::string::npos)
