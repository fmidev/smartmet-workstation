...
int NFmiCaseStudyDataFile::GetModelRunTimeGapInMinutes(NFmiQueryInfo *theInfo, NFmiInfoData::Type theType, NFmiHelpDataInfo *theHelpDataInfo)
{
	...
	if(theHelpDataInfo && theHelpDataInfo->NonFixedTimeGab())
		return -1; // Jos datan konffissa on erikseen m��r�tty m��rittelem�t�n aikav�li, niin k�ytet��n sit�.
	...
	int modelRunTimeGap = 0; // defaultti on 0, joka p�tee kaikkiin datoihin, mist� ei ole arkistodataa k�yt�ss�, kuten analyysi, havainnot jne.
	if(NFmiDrawParam::IsModelRunDataType(theType))
	{
		...
		else
		{
			...
			if(theInfo)
			{
				...
				else if(prodId == 118) // 118 on EC Kalman-Laps tuottaja, se on my�s 12 tunnin v�lein kun se lasketaan EC:n pohjalta
					modelRunTimeGap = 12*60; // Ecmwf:lle aikav�li on 12h
...
//	int itsDataIntervalInMinutes;	// ei ole hy�ty� tallettaa case-study metadataan? johdettavissa?
//	double itsSingleFileSize;		// ei ole hy�ty� tallettaa case-study metadataan? johdettavissa?
//	double itsTotalFileSize;		// ei ole hy�ty� tallettaa case-study metadataan? johdettavissa?
//	double itsMaxFileSize;			// ei ole hy�ty� tallettaa case-study metadataan? johdettavissa?
//	DataCategory itsCategory;		// ei ole hy�ty� tallettaa case-study metadataan? johdettavissa?
	jsonObject.push_back(json_spirit::Pair(gJsonName_DataType, static_cast<int>(theData.itsDataType)));
//	int itsLevelCount;				// ei ole hy�ty� tallettaa case-study metadataan? johdettavissa?
...
void NFmiCaseStudyDataFile::AddDataToHelpDataInfoSystem(std::shared_ptr<NFmiHelpDataInfoSystem> &theHelpDataInfoSystem, const std::string &theBasePath, NFmiHelpDataInfoSystem& theOriginalHelpDataInfoSystem)
{
	...
	if(itsDataType == NFmiInfoData::kKepaData || itsDataType == NFmiInfoData::kEditingHelpData || itsDataType == NFmiInfoData::kSingleStationRadarData)
		helpDataInfo.FakeProducerId(static_cast<int>(Producer().GetIdent())); // n�ill� datatyypeill� on poikkeus tuottajat, siis eri tuottaja smartmetissa kuin mit� l�ytyy itse datasta
...
static CaseStudyMatchingFiles GetNonModelDataMatchingFiles(const NFmiCaseStudyDataFile& theDataFile, const CtrlViewUtils::FileNameWithTimeList& fileNameWithTimeListAscending, const NFmiMetTime& usedWallClockTime)
{
	...
	// Loopin lopuksi laitetaan viel� viimeisin data listaan ja selaitetaan valituksi, jos ei ole viel� l�ytynyt valittua
...
// Joskus kun tehd��n UpdateNoProducerData -p��ivityksi�, tulee samalle tuottajelle useampi kuin yksi data. T�ll�in 
...
void NFmiCaseStudyProducerData::InitDataWithStoredSettings(NFmiCaseStudyProducerData &theOriginalProducerData)
{
	...
	// Lis�ksi jokaisen l�ytyneen vastin parin tiedot p�ivitet��n
...
void NFmiCaseStudyCategoryData::AddData(NFmiCaseStudyDataFile &theData)
{
	...
	// jos siis ei l�ytynyt tuottajaa, lis�t��n sellainen t�ss�
...
// Etsii l�ytyyk� itsProducersData -vektorista 0-tuottajaa. 
// Jos l�ytyy, poista se vektorista ja palauta. Muuten palauta tyhj� otus.
...
void NFmiCaseStudyCategoryData::UpdateNoProducerData(NFmiHelpDataInfoSystem &theDataInfoSystem, NFmiInfoOrganizer &theInfoOrganizer)
{
    ...
    UpdateOnlyOneDataStates(); // Pit�� p�ivitt�� OnlyOneData statukset, jos l�ytyi uusia datoja jollekin tuottajalle.
}
...
json_spirit::Object NFmiCaseStudyCategoryData::MakeJsonObject(const NFmiCaseStudyCategoryData &theData, bool fMakeFullStore)
{
	...
	if(dataArray.size())
	{ // t�ytet��n objekti vain jos l�ytyi yht��n talletettua tuottajaa
...
void NFmiCaseStudyCategoryData::InitDataWithStoredSettings(NFmiCaseStudyCategoryData& theOriginalCategoryData)
{
	...
	// Lis�ksi jokaisen l�ytyneen producerDatan vastin parin tiedot p�ivitet��n
...
void NFmiCaseStudyCategoryData::PutNoneProducerDataToEndFix()
{
	...
	if(iter != itsProducersData.end())
	{
		// Jos l�ytyi ns. None producer, siirret��n se listan loppuun
...
static void InitCategoryHeaders(std::vector<NFmiCategoryHeaderInitData>& categoryHeaders, const std::set<std::string> &customFolderNames)
{
	...
	for(const auto& customFolderName : customFolderNames)
	{
		...
		// Jos customFolderName:a ei l�ydy listasta, lis�t��n sellainen
...
// ja katsoa onko kyseinen data luettu k�ytt��n. Sitten data pit�� lis�t� systeemiin uudestaan, ett� 
...
static NFmiProducerSystem* GetProducerSystem(NFmiProducerSystemsHolder &theProducerSystemsHolder, NFmiCaseStudyDataCategory theCategory)
{
	if(theCategory == NFmiCaseStudyDataCategory::Model)
		...
		return theProducerSystemsHolder.itsObsProducerSystem; // analyysi tuottajat on sijoitettu my�s obs-tuottaja listaan
...
void NFmiCaseStudySystem::SetProducerName(NFmiProducerSystemsHolder &theProducerSystemsHolder, NFmiCaseStudyProducerData &theProducerData)
{
	...
	if(producerSystem)
	{
		unsigned long index = producerSystem->FindProducerInfo(tmpProd); // palauttaa 0-indeksin, jos ei l�ytynyt
		if(index)
		{
...
// 2. Pit��k� muokata NFmiCaseStudyDataFile:en uusi suhteellinen polku, ett� my�hemmin sit� voidaan k�ytt�� kun dataa ladataan katsottavaksi?
#ifndef UNIX
...
static void AddProducerDataFilesToList(const std::string &theCategoryDirIn, const std::string &theRelativeCategoryDirIn, NFmiCaseStudyProducerData &theProducerData, CaseStudyMatchingFiles &theCopyedFilesInOut, const NFmiMetTime& usedWallClockTime)
{
	...
	{
		if(fileDataVec[i].DataEnabled()) // jos data on disabloitu smartmetin konffeissa, sit� ei my�sk��n yritet� ottaa mukaan case-study -dataan
...
static void StoreProducerData(const std::string &theCategoryDir, const std::string &theRelativeCategoryDir, NFmiCaseStudyProducerData &theProducerData, int theProgressDialogMaxCount, int &theProgressCounter, CWnd *theCopyWindowPos, const std::string& theCropDataAreaString, const NFmiMetTime& usedWallClockTime)
{
	...
	if(NFmiFileSystem::CreateDirectory(producerDir))
	{
		...
		for(size_t i=0; i < fileDataVec.size(); i++)
		{
			...
			if(dataFile.DataEnabled()) // jos data on disabloitu smartmetin konffeissa, sit� ei my�sk��n yritet� ottaa mukaan case-study -dataan
			{
	...
	if(patternWithoutPath.empty() == false)
	{
		for(auto categoryData : itsCategoriesData)
		{
			...
			for(auto &producerData : producersData)
			{
				...
				{
					auto pos = MacroParam::ci_find_substr(dataFile.RelativeStoredFileFilter(), patternWithoutPath); // t�m� etsint� ty� pit�� tehd� case-insensitiivisti, tai muuten kaikkien eri konffeissa olevien juttujen pit�� olla tarkalleen samalla lailla kirjoitettu
					if(pos != MacroParam::ci_string_not_found)
					{ // l�ytyi haluttu data, rakenna nyt siihen sopiva file-filter
...
ModelDataOffsetRangeInHours NFmiCaseStudySystem::MakeTimeOffsetRange(const std::string& str)
{
	...
	if(parts.size() == 1)
	{
		...
		// Vain yksi mahdollinen luku, toinen indeksi on t�ll�in aina 0
