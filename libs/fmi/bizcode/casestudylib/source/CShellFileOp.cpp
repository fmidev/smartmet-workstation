...
static std::string FixPathStr(LPCTSTR szPath)
{
	...
	NFmiStringTools::ReplaceChars(tmpStr, '/', '\\'); // muutetaan mahdolliset v��rinp�in olevat kenoviivat niin ett� SHFileOperation-funktio ei ota nokkiinsa eli mahd. '/' -> '\'
	NFmiStringTools::TrimR(tmpStr, '\\'); // jos kyse oli polusta, lopussa ei saa olla kenoviivaa, koska t�ll�in SHFileOperation-funktio ei toimi XP:ss� (toimisi Win7:ssa ok)
	return tmpStr;
}
...
BOOL CShellFileOp::AddSourceFile ( LPCTSTR szPath )
{
    ...
    try
        {
			std::string tmpStr = ::FixPathStr(szPath);
			m_lcstrSourceFiles.AddTail (CA2T(tmpStr.c_str()));
        }
...
BOOL CShellFileOp::AddDestFile ( LPCTSTR szPath )
{
    ...
    try
        {
			std::string tmpStr = ::FixPathStr(szPath);
			m_lcstrDestFiles.AddTail(CA2T(tmpStr.c_str()));
        }
    catch ( CMemoryException &)
        {
...
bool CShellFileOp::MakeSureDestinationDirectoryExist(void)
{
	if(m_rFOS.wFunc == FO_COPY || m_rFOS.wFunc == FO_MOVE)
	{
		if(m_lcstrDestFiles.GetCount() == 1)
		{ // oletus, kyse on joko hakemiston copy/move:sta tai monien tiedostojen kopioinnista/siirrosta yhteen hakemistoon
			POSITION pos = m_lcstrDestFiles.FindIndex(0);
			std::string tmpStr = CT2A(m_lcstrDestFiles.GetAt(pos));
			return NFmiFileSystem::CreateDirectory(tmpStr);
		}
	}
	return true;
}
...
// of TCHARs.
//////////////////////////////////////////////////////////////////////////
// Marko: Enen Go:n kutsua pit�� varmistaa ett� kohde polut ovat olemassa, muuten SHFileOperation ei toimi. 
// En voi varmistaa sit� t��ll�, koska en tied� onko annetun kohdepolun p��ss� hakemisto vai tiedosto.
// "C:\\aaa\\bbb\\ccc" , ei voi olla varma, onko 'ccc' hakemisto vai tiedoston nimi, joka on 'bbb'-hakemistossa.
// Voin tehd� kuitenkin kaksi poikkeusta (ks. MakeSureDestinationDirectoryExist): 
// 1. Kun kyse on kopioinnista (tai movesta) ja on monia l�hde-tiedostoja, mutta vain yksi kohde polku.
// 2. Kun kyse on kopioinnista (tai movesta) ja on yksi l�hde-polku ja yksi kohde polku (oletus t�t� ohjelmaa ei kutsuta kopioimaan yht� tiedostoa vaan t�ss� on aina kyse hakemisto-kopiosta).
...
int    nAPIRet;
	...
	MakeSureDestinationDirectoryExist();
...
#pragma warning( push )
#pragma warning( disable : 4996 ) // _tcscpy-funktion aiheuttama 'turvallisuus' varoitus
