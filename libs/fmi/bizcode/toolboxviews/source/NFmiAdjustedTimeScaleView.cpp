...
{    
  ...
  baseScaleRect.Height(baseScaleRect.Height() + theMaxWidth/20.); // korotetaan hieman SmartMEtin vihre�n laatikon korkeutta
...
// lasketaan laatikolle viel� minimi koko, ettei katoa ihan kokonaan n�yt�lt�
...
// Laskee 'normaali' aika-akselin�yt�n leveyden suhteessa currenttiin tilanteeseen.
// K�ytet��n harventamaan liian kaposten n�ytt�jen lukemia ja tickmarkkeja.
// Normaali n�yt�n leveys tarkoittaa leveytt�, mill� akselit n�ytt�v�t muuten
// ilamn t�t� harvennusta ihan ok:lta.
double NFmiAdjustedTimeScaleView::GetNormalToCurrentViewPixelRatio(void)
{
	// t�m� luku kertoo aika-akselin suht noormaalin leveyden (pikseleiss�), 
	// jolloin akselin luvut ja tickmarkit ovat suht. selkeit�. t�m�n
	// luvun avulla on tarkoitus alkaa harventaa lukuja ja tickmarkkeja (lis��)
	// jos aika-akseli n�yt�n leveys on riitt�v�n verran kapeampi kuin t�m�.
...
// lasketaan, mill� jaolliset tunnit piirret��n aikaikkunaan
...
// lasketaan, mill� jaolliset tunteihin piirret��n aikaikkunaan merkill�
// Tarkoitus on harventaa piirrett�vi� tickmarkkeja, riippuen
// aika-akselin koosta/timestepista eli theStepNumber:ista
// ja n�yt�n koosta
...
// Laskee aikaikkunan todellisen leveyden ja aikav�lin avulla sopivat major- ja minor -tick markkien resoluutiot minuuteissa.
// Major-tick mark on se merkki johon tulee tunti lukema.
// Minor-tick on merkki johon tulee viiva (ellei sse ole my�s major samalla)
// Kun Major-step on laskettu, minor on aina yksi pyk�l� siit� pienempi tai jos ollaan jo alarajalla, se on sitten sama.
void NFmiAdjustedTimeScaleView::GetTimeSteps(NFmiDrawingEnvironment &theDrawingEnvi, int &theMajorStepInMinutesOut, int &theMinorStepInMinutesOut)
{
	...
	if(indexCounter >= itsTickMarkStepsLimitsInMinutes.size())
		indexCounter = itsTickMarkStepsLimitsInMinutes.size()-1; // jos ei l�ytynyt sopivaa steppi�, laitetaan sitten tuo viimeinen steppi p��lle
	...
	if(indexCounter > 0)
		...
		theMinorStepInMinutesOut = itsTickMarkStepsLimitsInMinutes[0]; // pakka laitta t�h�nkin ensimm�inen steppi vaihtoehto
}
...
{
	NFmiDrawingEnvironment theEnvironment;
	// piirret��n ensin 'sein�kello' siten ett� piirret��n asteikon pohja vaalen vihre�ll� nykyhetkest� tulevaisuuteen.
	NFmiMetTime currentTime(1);
	NFmiRect wallClockRec = GetFrame();
	wallClockRec.Top(wallClockRec.Top() + itsToolBox->SY(1)); // siirret��n yl�reunaa yhden pikselin verran alasp�in
	wallClockRec.Bottom(wallClockRec.Bottom() - itsToolBox->SY(1)); // siirret��n alareunaa yhden pikselin verran yl�sp�in
	...
	{ // piirret��n sein�kello v�rj�ys, koska 'tulevaisuus' aikaa tulee ainakin jonkin verran n�kyviin
		theEnvironment.SetFillColor(NFmiColor(0.86f,0.92f,0.85f));
		theEnvironment.DisableFrame();
		theEnvironment.EnableFill();
		NFmiRect timeScaleRect = CalcTotalTimeScaleArea(); // rajoitetaan piirto aikaskaalaan alueelle ja siit� viel� pikselin verran sis��n
...
enum TimeLabelPrintMode
{
	...
	kFmiLabelModeFirstMainTime = 1, // ensimm�inen 00 tai 12
	kFmiLabelModeFirstSynopTime = 2, // ensimm�inen 00, 03, 06, 09, 12, 15, 18, 21
...
//		while(theScale->Next(cursor)) // theScale -> next -looppi antaa cursor:ille arvot: 0, stepInMinutes, 2*stepInMinutes, 3*stepInMinutes, jne.
		...
		for(cursor = theScale->StartValue(); cursor <= endOfCursor; cursor += usedStepInMinutes, aTime.ChangeByMinutes(static_cast<long>(usedStepInMinutes)))
		{
			...
			// 1. m��r�� piirtov�ri, joka toinen p�iv� punainen ja joka toinen sininen. Ota v�ri suoraan p�iv�m��r�n p�iv�n numeron mukaan, vaikka siin� on tiettyj� huonoja puolia.
			int odd = aTime.GetDay()%2;
			theEnvironment.SetFrameColor(NFmiColor(odd*1.0f, 0.0f,(1-odd)*1.0f));

			// 2. Piirr� tick-mark (= minor aika merkki, joka voi olla my�s major-merkin kohdalla)
			...
			// 3. Piirr� tunti/minuutti-lukema (= major aika merkki)
			if(GetMinutesOfTheDay(aTime) % majorStepInMinutes == 0)
			{
				if(majorStepInMinutes <= 24*60 || aTime.DifferenceInMinutes(lastPrintedMajorTickMark) >= majorStepInMinutes)
				{
					...
					// 4. Piirr� p�iv�ys aikaleima ja viikop�iv� Jos mahdollista laita p�iv�m�� klo 12 utc kohtaan, 
					// mutta jos majorStep on suurempi kuin 12h, laita leima 00-kohtaan.
					if(datePrinted)
						continue;
					bool printDateNow = false; // t�ss� hanskataan pri erikois tapausta
					...
					if(printDateNow || (majorStepInMinutes <= 12*60 && aTime.GetHour() == 12) || (majorStepInMinutes > 12*60 && aTime.GetHour() == 0))
					{
						...
						// piirret��n my�s viikon p�iv� p�iv�yksen alle
