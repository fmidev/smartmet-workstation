...
{ 
	...
	if(is)
		is >> itsDataIdent;
	if(is)
		itsDrawParam->Param(itsDataIdent); // pit�� asettaa my�s parametri kohdalleen drawParamiin, muuten koiranpennut rupee juhlimaan
	if(is)
		is >> itsLevel;
	if(is)
		itsDrawParam->Level(itsLevel); // my�s level pit�� asettaa t�ss� heti
	if(is)
	{
		int tmp = 0;
		is >> tmp;
...
void NFmiViewSettingMacro::TimeViewRow::Write(std::ostream& os) const
{
	...
	// 1. lis�ttyn� ominaisuutena on lista side-parametereista yhten� stringin� (HUOM! pakko lis�t� my�s tyhj� lista)
	std::stringstream stringOut;
	NFmiDataStoringHelpers::WriteContainer(itsSideParameters, stringOut, string("\n"));
	// Huom! talletettavasta stringist� pit�� my�s poistaa kaikki kommentit ensin, muuten homma ei toimi!!!
...
static NFmiViewSettingMacro::MapRow MakeMapRow(NFmiDrawParamList *theDrawParamList, NFmiMacroParamSystem& theMacroParamSystem)
{
	...
	for(theDrawParamList->Reset(); theDrawParamList->Next(); )
	{
		...
		{ 
			...
			// hieman erilaisena ja oikean suht.polun saa vain MacroParamSystemist� l�ytyv�st� macroParamin DrawParamista
			// eika dokumentista l�ytyv�st� DrawParamListasta.
...
void NFmiViewSettingMacro::TimeView::Write(std::ostream& os) const
{
    ...
    // Lis�t��n apudata2 ja 3 ja 4 asetukset n�ytt�makroon
...
{ 
    ...
    // Apudata2 ja 3 asetukset n�ytt�makrosta
    fShowHelpData2 = false;
	...
	// Oletusarvoisesti laitetaan latlon-point puuttuvaksi, jolloin j�tet��n nykyinen k�yt�ss� oleva piste voimaan.
...
{ 
	...
	{
		// luetaan uudet ver 2.0 ja my�h. jutut
		if(is.fail())
			throw runtime_error(exceptionErrorMessage);
		is >> itsMTATempSystem;
...
// Jos jollekin toisille n�yt�ille halutaan viel� lis�t� macroParam tuki, 
// t�h�n pit�� lis�t� kyseisen n�yt�n macroParam polkujen alustukset.
...
// tutkii miss� n�ytt� riviss� on viimeiset parametrit. T�ll� pyrit��n s��st�m��n
...
void NFmiViewSettingMacro::MapViewDescTop::Write(std::ostream& os) const
{
    ...
    // Talletetaan rivej� vain niin pitk�lle kuin sielt� l�ytyy jotain talletettavaa (nyt siis rivej� voi potentiaalisesti olla aina 50 kpl)
    int mapRowWriteSize = ::CalcStoredMapRowCount(itsMapRowSettings); 
    NFmiDataStoringHelpers::WriteContainer(itsMapRowSettings, os, string("\n"), mapRowWriteSize);

	os << "// vector<MapRow> ExtraMapRowSettings" << endl;
    std::vector<MapRow> emptyLegacyMapRowSettings; //  Pakko tallettaa tyhj� vektori n�ytt�makroon taaksep�in yhteensopivuuden takia
...
{ 
    ...
    std::vector<MapRow> legacyExtraMapRowSettings; //  Pakko lukea n�ytt�makroista mahdolliset extraParamit taaksep�in yhteensopivuuden takia
