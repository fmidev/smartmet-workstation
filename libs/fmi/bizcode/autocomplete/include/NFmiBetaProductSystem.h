...
public:
    ...
    bool CheckTimeRelatedInputs(const NFmiMetTime &theStartingTime, const std::string &theTimeLengthInHoursString, const std::string &theTimeStepInMinutesString, bool theUseUtcTimesInTimeBox, FmiDirection theTimeBoxLocation); // K‰y l‰pi kaikki tarkastelut ja tekee virheilmoituksia
...
public:
    ...
    public:
        ...
        std::string itsTriggerDataString; // t‰ss‰ on listassa triggeri datojen lista, miss‰ haluttu data on kerrottu sen fileFilterill‰ ja ne on pilkulla eroteltuna.
...
private:
    static const std::string itsRunTimeStepInHoursTitle; // Time-step-hours -kontrolliin liittyv‰ vakio teksti. T‰m‰n avulla tehd‰‰n info/virhe tekstej‰ ja t‰m‰ tulee myˆs CFmiBetaAutomationDialog -dialogin teksteihin
    static const std::string itsFirstRunTimeOfDayTitle; // First-run-of-day -kontrolliin liittyv‰ vakio teksti. T‰m‰n avulla tehd‰‰n info/virhe tekstej‰ ja t‰m‰ tulee myˆs CFmiBetaAutomationDialog -dialogin teksteihin
    std::string itsBetaProductPath;
    std::string itsOriginalBetaProductPath; // T‰h‰n talletetaan se polku, mink‰ k‰ytt‰j‰ on antanut dialogissa, t‰m‰ talletetaan myˆs lopulta tiedostoon
    bool fBetaProductPathStatus;
    std::string itsBetaProductPathStatusString;
    NFmiTriggerModeInfo itsTriggerModeInfo; // Mik‰ laukaisee t‰m‰n tuotteen tuotannon
    NFmiTimeModeInfo itsStartTimeModeInfo; // Miten m‰‰r‰t‰‰n t‰m‰n tuotteen alkuaika
