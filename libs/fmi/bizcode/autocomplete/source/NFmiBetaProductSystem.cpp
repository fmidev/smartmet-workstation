...
NFmiMetTime NFmiBetaProductAutomation::NFmiTriggerModeInfo::CalcNextDueTimeWithFixedTimes(const NFmiMetTime& theLastRunTime) const
{
    ...
    {
        // Jos ei l�ydy t�lle p�iv�lle en�� aikoja, tehd��n listan 1. ajasta seuraavan p�iv�n aika
...
NFmiMetTime NFmiBetaProductAutomation::NFmiTriggerModeInfo::CalcNextDueTimeWithTimeSteps(const NFmiMetTime& theLastRunTime) const
{
    ...
    // Jos ei l�ytynyt en�� t�lle p�iv�lle aikoja, tehd��n seuraavan p�iv�n 1. aika t�ss�
...
bool NFmiBetaProductAutomation::NFmiTriggerModeInfo::HasDataTriggerBeenLoaded(const std::vector<NFmiDefineWantedData>& triggerDataList, const std::vector<std::string>& loadedDataTriggerList, NFmiInfoOrganizer& infoOrganizer, const std::string& automationName, int& postponeTriggerInMinutesOut, bool betaProductCase)
{
    if(!triggerDataList.empty())
    {
        for(const auto& triggerData : triggerDataList)
        {
            ...
            {
                // NFmiExtraMacroParamData::FindWantedInfo metodille pit�� sallia etsi� my�s asemadatoja (3. parametri true).
...
// Siirr� automaatiota yhden pyk�l�n listassa yl�s/alas.
// Suunta yl�s tarkoittaa Beta-automaatio dialogin listassa 
// visuaalisesti yl�s eli kohti pienemp�� indeksi�.
// T�ss� tehd��n swap kahden vector:issa olevan itemin v�lill�.
...
bool NFmiBetaProductionSystem::Init(const std::string &theBaseRegistryPath, const std::string& theAbsoluteWorkingDirectory, const std::string& possibleStartingBetaAutomationListPath)
{
    ...
    // otetaan Beta-product base-directory oletus arvoksi n�ihin kolmeen polkuun
