...
// Toolmasterin laskemissa polygoneissa on joskus pieni vertikaalisuunnassa tapahtuva vika. Tämä funktio etsii seuraavanlaisia merkkeja:
// 1. polygonsCoordinatesY parametrista löytyy yksittäinen arvo joka on tarpeeksi lähellä (normi epsilon tarkastelu) bottomRowCoordinateY arvoa
// 2. Tälläisiä yksittäisiä kohtia voi olla yhdessä polygonissa useita ja ne kaikki pitää hoitaa.
// 3. Jos tämä yksittäisen arvon vierestä löytyy toinen arvo joka on lähellä bottomRowCoordinateY arvoa, mutta huomattavasti isommalla epsilonilla etsittynä, 
//    pitää tätä vieressä olevaa y-koordinaattia korjata niin että se on pohjarivin korkeudella.
// 4. Jos näiden kahden vierekkäisen pisteen x-koordinaateissa on iso ero (polygoni on leveä ja pohjalta lähes horisontaali), on kyse mitä luultavimmin Toolmaster vika joka pitää korjata.
// Funktio palauttaa vektorin jossa on tehty korjaukset, jos mitään korjauksia on tehty, muuten palautetaan tyhjä vektori.
