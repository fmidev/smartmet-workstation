    ...
    if( aiTotalRows > 0)
    {
        ...
        for( int i1=0; i1 < aiTotalRows; i1++)
        {
            ...
            if( pGridTreeCell != NULL)
            {
                ...
                pGridTreeCell->SetLevelAndHide( *apucTreeLevelAry | 0x80); // marko (keskustelu palstalta vinkki, miten tree-näkymän saa päälle) -> | 0x80 
            }
...
// returns:  row where elements inserted; -1 on error
{
    ...
    for( i1=0; i1 < aiNbrElements; i1++)
    {
        ...
        if( m_pGrid->InsertRow( _TEXT(""), iRowForAppend) < 0)
        {
            aiRow = -1; // error
            break;
        }
