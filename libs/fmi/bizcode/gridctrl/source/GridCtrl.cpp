...
void CGridCtrl::OnDraw(CDC* pDC)
{
	...
	pen.DeleteObject(); // TÄMÄ ON MARKON LISÄ!!!!!
                          ...
                          int low, int high)
{
    ...
    while (lo <= hi)
    {
        ...
        if (lo <= hi)
        {
            ...
            {
//				for (int col = 0; col < GetColumnCount(); col++)
				for (int col = GetFixedColumnCount(); col < GetColumnCount(); col++) // MARKO VIRITYS, fixed columneja ei sortata
                {
...
static void SetFocusFunction(CWnd *theFocusWnd)
{
	if (theFocusWnd && ::IsWindow(theFocusWnd->GetSafeHwnd()))
		theFocusWnd->SetFocus(); 
}
