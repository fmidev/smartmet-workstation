...
void CGridCellCheck::OnClick(CPoint PointCellRelative)
{
    ...
    // Marko: CGridCtrl and CGridCellCheck item in it makes following irritating behaviour:
    // 1. CGridCtrl area is not fully covered by grid-control rows
    // 2. First user clicks with mouse on some cell that contains CGridCellCheck -item
    // 3. User then click on empty area of grid-control
    // 4. If application is in debug mode, there will be assert that stops execution, because empty cell is checked if its editable.
    if(!cell.IsValid()) // So this is to prevent assert on empty cell in debug version
        return;
	...
	if (GetCheckPlacement().PtInRect(PointCellRelative))
	{
		...
		if(itsCallbackFunction)
			itsCallbackFunction(cell.col, cell.row);
	}
}
