...
#ifndef UNIX
#pragma warning( disable : 4786 4996) // disable warning 4385. (liian pitk� luokan nimi)
#endif
...
void NFmiEditMapView::Draw(NFmiToolBox * theGTB)
{
    ...
    try
    {
        // HUOM! jos GDI tulee laajempaan k�ytt��n, poista init+clean t�st� ja siirr� ne Draw-metodiin!!!!!!!
...
{
    itsLastSendTimeTextRect = NFmiRect(); // nollataan varmuuden vuoksi t�m� laatikko aluksi
    ...
    if(isMainMapView && notPrinting && notBetaGenerationRunning && showLastTimeOnView)
    {
        ...
        NFmiPoint textOriginPoint = itsViewGrid->GetFrame().TopRight(); // Tekstien piirto alkaa hilan�yt�n oikeasta yl�kulmasta
#ifndef UNIX
        ...
        Gdiplus::Rect rectInPixels(
            static_cast<int>(textOriginPointInPixel.X - textBoundingBox.Width), // laatikkoa siirret��n vasemmalle oikeasta reunasta t�ss�
...
void NFmiEditMapView::LogWarningForTooLongMapViewDrawTime(NFmiMilliSecondTimer &theTimer)
{
    ...
    { // Jos karttan�yt�n piirto kest�� yli m��r�tyn ajan, laitetaan lokiin varoitus
...
{
    // Ei jatketa, jos on k�ynniss� jonkun muun n�yt�n printtaus,printatessa muiden ruutujen p�ivitys voi aiheuttaa ongelmia...
	if(itsCtrlViewDocumentInterface->Printing() && !IsPrintedMapViewDesctop())
		return ;
	if(itsCtrlViewDocumentInterface->DataModificationInProgress()) // Uusien datan muokkaus rutiinien kanssa ruutujen p�ivitys voi aiheuttaa ongelmia 
...
static bool DoesLabelsFitOnMapVertically(double theMapAreaHeightInMMIn, size_t theLabelCountIn, double labelHeight, double labelGab)
{
	if(theLabelCountIn > 1)
		return (theMapAreaHeightInMMIn / theLabelCountIn) > (labelHeight + labelGab - (labelGab/theLabelCountIn)); // huom1 labelGabeja on yksi v�hemm�n kuin labeleita
	else
	...
	if(doesLabelsFit == false)
	{
		...
		for( ; ::DoesLabelsFitOnMapVertically(theMapAreaHeightInMMIn, theLabelCountIn, labelHeight, labelGab) == false; )
		{
			...
			if(labelGab < gMinLabelGapInMM || labelHeight < gMinLabelHeightInMM)
				break;  // t�m� on virhe, t�h�n ei pit�isi menn�
		}
	}
...
void NFmiEditMapView::DrawVerticalAnimationControl(void)
{
    ...
    // piirret��n lopuksi viel� Play-label
    usedFillColor = fillColor;
...
static double MMtoRelativeLength(double lengthInMM, NFmiToolBox &theToolBox, CtrlViewUtils::GraphicalInfo &theGraphicalInfo, bool doX)
{
	...
	else // tehd��n sitten y sunnassa laskut
	{
...
void NFmiEditMapView::DrawVerticalAnimationControlInit(void)
{
	...
	if(animData.ShowTimesOnTimeControl() && animData.ShowVerticalControl())
	{
		...
		{ // pit�� harventaa labeleiden lukum��r��, niin ett� ne pienimmill��n voisivat mahtua karttaosioon
			...
			do
			{
				...
				if(newStepInMinutes == usedStepInMinutes) // jos samat, ollaan jo p��sty suurimpaan aika-askeleeseen
					return ; // ei toimi, aikoja on liikaa piirrett�v�ksi jopa suurimmalla aika askeleella (2 vrk), lopetetaan, luultavasti virhetilanne
			...
			NFmiMetTime lastTime(times.FirstTime()); // uusi viimeinen aika pit�� laskea
...
// Jos tehd��n operaatioita hiirell� karttan�yt�ll� ja
// hiiri on t�m�n EditMapView:in alueella, aktivoidaan t�m�n 
// karttan�yt�n indeksi ns. aktiiviseksi karttan�ytt� aikaindeksiksi.
...
bool NFmiEditMapView::LeftButtonUp(const NFmiPoint & thePlace, unsigned long theKey)
{
    ...
    { // Hiiren kaappaus pit�� tarkastella ensin
    ...
    if(!status)
    {
        // muuten tehd��n normaali tarkastelut 
...
bool NFmiEditMapView::LeftDoubleClick(const NFmiPoint &thePlace, unsigned long theKey) // Marko lis�si 14.12.2001
{
    ActivateMapView(thePlace);
	if(itsViewGrid)
...
bool NFmiEditMapView::LeftButtonDown(const NFmiPoint & thePlace, unsigned long theKey)
{
	...
	if(IsIn(thePlace))
	{
		...
		if(itsTimeControlView && itsTimeControlView->LeftButtonDown(thePlace, theKey))
			return false; // lopetetaan jos oli sis�ll� ja palautuu false (eli ei tarvetta p�ivitt�� ruutua jos hiiri on vasta painettu alas)
		else
...
{ // HUOM!! ei pid� tutkia onko hiiri ikkunan sis�ll� vaan onko hiiri kaapattu!
    ...
    if(itsCtrlViewDocumentInterface->MouseCaptured())
    {
        if(itsTimeControlView && itsTimeControlView->IsMouseCaptured())
        {
            ...
            return true; // t�m� pakottaa ruudun p�ivityksen, kun aikaikkunaa raahataan
        }
        else if(itsViewGrid->IsMouseDraggingOn() || itsViewGrid->IsIn(thePlace))
            status = itsViewGrid->MouseMove(thePlace, theKey); // otetaan status talteen, sit� k�ytet��n my�hemmin
        if(status)
            fMouseMovedInCapture = true;
    }
    ...
    if(itsViewGrid->IsIn(thePlace))
    {
        ...
        if(itsCtrlViewDocumentInterface->MouseCaptured())
        {
            if(status && itsCtrlViewDocumentInterface->MiddleMouseButtonDown() && itsCtrlViewDocumentInterface->MapMouseDragPanMode())
                return true; // jos ollaan "mouse drag pan" -moodissa, pit�� kartta n�ytt� p�ivitt��
        }
        ...
        { // tehd��n ajan s��d�t
            ...
            if(index == itsVerticalAnimationInfo.itsUsedLabelCount - 1)
            {
                ...
                return false; // Palauta false, koska edell� tehtiin jo ruutujen p�ivitys oikeilla loki teksteill�
            }
            else if(itsVerticalAnimationInfo.itsUsedTimes.SetTime(index))
            {
                ...
                if(newTime != oldTime)
                {
                    // T�m� laittaa optimoidun n�ytt�jen p�ivityksen p��lle, joten seuraavan rivin RefreshApplicationViewsAndDialogs voi olla ilman n�ytt� maskia
...
void NFmiEditMapView::Update()
{
	...
	if(itsViewGrid)
	{
		...
		itsViewGrid->Update();
		UpdateTimeControlView(); // drawparamlistin pit�� olla 'likainen' ennen kuin updatetaan t�t�!!!
	}
}
...
// etsii 1. mahd. synop-plot -n�yt�n
NFmiSynopPlotView* NFmiEditMapView::GetSynopPlotViewFromToolTipPos(bool fDrawSoundingPlot, bool fDrawMinMaxPlot, bool fDrawMetarPlot)
{
	...
	if(drawParamList)
	{
		// ensin etsit��n normaalia synop-plot paramia
...
NFmiString NFmiEditMapView::Value2String(float theValue, int theDigitCount, FmiInterpolationMethod theInterpolationMethod, FmiParamType theParamType)
{
	...
	else if(theDigitCount == 0) // lis�t��n linear + 0-tapauksessa kuitenkin yksi desimaali
...
// k�y asettamassa kaikkiin mahdollisiin n�ytt�ihin oikeat ajat
...
std::string NFmiEditMapView::ComposeToolTipText(const NFmiPoint& theRelativePoint)
{
	...
	std::string str = GetToolTipText().CharPtr(); // t��lt� tulee tiettyjen ty�kalujen tooltip tekstit
	if(str.empty() == false)
		return str;

	// jos noilta ty�kaluilta ei tullut mit��n, kysyt��n muilta komponenteilta teksti�
