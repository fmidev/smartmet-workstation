...
static NFmiColor GetTimeTextcolor(const NFmiMetTime &theTime, bool fIsLastEditedDataSendHasComeBack)
{
    if(theTime == NFmiMetTime::gMissingTime)
        ...
        return NFmiColor(0.129f, 0.568f, 0.005f, 1.f); // vihreä, jos data on jo tullut takaisin
    else
        return NFmiColor(0.99f, 0.64f, 0.f, 1.f); // keltainen, jos data ei ole vielä tullut takaisin
//    else
...
void NFmiEditMapView::DrawVerticalAnimationControl(void)
{
    ...
    if(itsCtrlViewDocumentInterface->AnimationData(itsMapViewDescTopIndex).AnimationOn())
        usedFillColor = selectedFillColor;
    CtrlView::DrawPath(*itsGdiPlusGraphics, labelPath, frameColor, usedFillColor, true, true, true, 1); // pitää kysyä Play statusta, että filli color saadaan oikein
