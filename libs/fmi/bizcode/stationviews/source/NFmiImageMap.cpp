...
Gdiplus::Bitmap* NFmiImageMap::GetRightSizeImage(double drawedSymbolSizeInPixels, bool isPrinting, const std::string &theSymbolCode)
{
    if(!isPrinting)
    {
        ...
        {
            if(smallerSymbolImage->GetHeight()*1.3 >= drawedSymbolSizeInPixels) // Jos pienempi kuvista on tarpeeksi lähellä haluttua piirtokoko tai pienempi, käytetään pientä kuvaa
