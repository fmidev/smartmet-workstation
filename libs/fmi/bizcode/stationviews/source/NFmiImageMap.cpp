...
// T�h�n luokkaan luetaan Mirwa analyyseiss� k�ytettyj� kuvia.
...
void NFmiImageMap::Initialize(const std::string &theImageBaseFolder, const std::string &theInitFile)  // HUOM! heitt�� poikkeuksia ep�onnistuessaan
{
    fInitialized = true;
    itsSymbolMap.clear();
    ...
    // init tiedoston nimen voi antaa tyhj�n�, jolloin ei tehd� mit��n.
    if(!theInitFile.empty())
    {
	    ...
	    if(NFmiFileSystem::ReadFile2String(theInitFile, fileContent))
	    {
		    ...
		    const int maxBufferSize = 1024; // kuinka pitk� yhden rivin maksimissaan oletetaan olevan
		    std::string buffer;
		    int i = 0;
		    int counter = 0;
		    do
...
std::pair<std::string, NFmiImageMap::ImagesHolder> NFmiImageMap::GetImageHolderInfo(const std::string &theConfFileLine, const std::string &theConfFileName)
{
    ...
    if(!line.empty())
    {
        ...
        else if(parts.size() == 2 || parts.size() > 3) // tehd��n poikkeus, jos rivilt� on l�ytynyt ';'-merkkej�, mutta ei tasan kahta kappaletta
        {
    ...
    return std::pair<std::string, ImagesHolder>(); // rivi oli tyhj�
}

// 1. Jos ei printata (= ei tarvita v�ltt�m�tt� isoa/paras laatuista kuvaa), katsotaan l�ytyyk� pienempi kuva
// 2. Jos l�ytyy ja sen pikseli koko vastaa haluttua kuvan koko, palautetaan se.
// 3. Kaikissa muissa tapauksissa yritet��n palauttaa iso kuva (l�ytyi se tai ei)
