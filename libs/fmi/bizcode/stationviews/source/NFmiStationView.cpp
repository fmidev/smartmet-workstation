...
bool NFmiStationView::IsSpaceOutDrawingUsed()
{
	...
	if(spaceOutFactors.X() > 1 || spaceOutFactors.Y() > 1)
	{
		...
		if(itsInfo->Grid() && !stationMacroParamData) // asema dataa ei yritetä harventaa
		{
			return true;
		}
	}
...
float NFmiStationView::GetMacroParamTooltipValueFromCache(const NFmiExtraMacroParamData& extraMacroParamData)
{
	...
	if(itsCtrlViewDocumentInterface->MacroParamDataCache().getCache(itsMapViewDescTopIndex, realRowIndex, itsViewRowLayerNumber, usedTime, itsDrawParam->InitFileName(), macroParamLayerCacheDataType))
	{
		...
		else
		{
			...
			auto interpolatedValue = DataMatrixUtils::InterpolatedValue(dataMatrix, LatLonToViewPoint(latlon), itsArea->XYArea(), kFmiTemperature);
			...
			return DataMatrixUtils::InterpolatedValue(dataMatrix, LatLonToViewPoint(latlon), itsArea->XYArea(), kFmiTemperature, false, kNearestPoint);
		}
	}
...
void NFmiStationView::FinalFillDataMatrix(boost::shared_ptr<NFmiFastQueryInfo> &theInfo, NFmiDataMatrix<float> &theValues, const NFmiMetTime &usedTime, bool useCropping, int x1, int y1, int x2, int y2, NFmiGrid* optimizedDataGrid)
{
	if(optimizedDataGrid)
		theValues = theInfo->GridValues(*optimizedDataGrid, usedTime, itsTimeInterpolationRangeInMinutes);
	else if(useCropping)
