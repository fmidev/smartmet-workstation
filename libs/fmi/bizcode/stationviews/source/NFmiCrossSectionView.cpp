...
void NFmiCrossSectionView::Draw(NFmiToolBox *theGTB)
{
	...
	itsShowDifferenceTimeShift = static_cast<int>(::round(itsCtrlViewDocumentInterface->TimeControlTimeStep(0))); //otetaan aikasiirron pituus p‰‰karttan‰ytˆst‰
			...
			if(dpList->NumberOfItems() > 0)
			{
				...
				for(dpList->Reset(); dpList->Next(); )
				{
					...
					if(itsDrawParam->ShowDifference())
					{
						...
						float value2 = GetLevelValue(info, p, latlon, tmpTime); // tee log(p) interpoloinnit qinfoon ja k‰yt‰ t‰ss‰!!!!
					...
					else if(itsDrawParam->ModelRunDifferenceIndex() < 0)
					{
						...
						{
							float value2 = GetLevelValue(wantedModelRunInfo, p, latlon, aTime); // tee log(p) interpoloinnit qinfoon ja k‰yt‰ t‰ss‰!!!!
							value = ::CalcDiffValue(value, value2);
						}
					}
	...
	for(theTimes.Reset(); theTimes.Next(); )
	{
		if(theInfo->Time(theTimes.CurrentTime()))
		{
			...
			{ // dataa lˆytyi asemalta johonkin aikaan, t‰ytet‰‰n matriisi silt‰ kohdalta, ja merkit‰‰n viimeksi lˆytynyt aika-indeksi talteen
				FmiParameterName parId = static_cast<FmiParameterName>(theDrawParam->Param().GetParamIdent());
				if(theInfo->Param(parId) || parId == kFmiHumidity || ::DoWindVectorSpecialData(theInfo, theDrawParam)) // humidity lasketaan vaikka sit‰ ei lˆydy infosta
				{
