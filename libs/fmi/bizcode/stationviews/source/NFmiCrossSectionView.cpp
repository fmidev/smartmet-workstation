...
static std::shared_ptr<NFmiArea> s_emptyArea;
static std::shared_ptr<NFmiDrawParam> s_emptyDrawParam;
...
void NFmiCrossSectionView::DrawParamView(NFmiToolBox * theGTB)
{
    if(itsCtrlViewDocumentInterface->BetaProductGenerationRunning())
    {
        ...
        // Jos se otetaan muuallakin k�ytt��n, alustus ja siivous pit�� siirt�� NFmiCrossSectionView::Draw metodiin.
...
void NFmiCrossSectionView::Draw(NFmiToolBox *theGTB)
{
	...
	//************** QUICKFIX!!!!! *******************
	std::shared_ptr<NFmiDrawParam> oldDrawParam = itsDrawParam;
	...
	if(dpList)
	{
		...
		{
			itsExistingLabels.clear(); //EL
			itsDrawParam = dpList->Current(); // asetetaan todella k�ytetty drawParam k�ytt��n
			...
			{ // voi menn� t�h�n esim. jos on tiputtanut ensin editoriin level dataa ja katsoo
			  // editoituna datana vaikka l�mp�tilaa. Sitten laitetaan editori operatiiviseen
			  // tilaan painamalla Lataa-nappia, jolloin editoitava data on pinta dataa (nyt
			  // se katsottu l�mp�tila drawParam pit�� poistaa)
			  // itsCrossSectionDrawParam voi olla 0-pointteri, jos kyseess� viallinen MacroParam

				//dpList->Remove(true); // Ei poisteta en�� jos dataa ei l�ydy
				continue;
			}
...
{
    return itsCtrlViewDocumentInterface->ActiveMapTime(); // haetaan aktiivisen karttan�yt�n current time
}
...
NFmiMetTime NFmiCrossSectionView::GetCrossSectionTime(const NFmiPoint &theRelativePlace)
{
	...
	else if(crossSectionSystem->GetCrossMode() == NFmiCrossSectionSystem::kTime || obsForMode)
	{
		...
		if(times.SetTime(static_cast<unsigned long>(timeIndex)))
			...
			return times.FirstTime(); // JOKIN MENI OIKEASTI PIELEEN; T�M� ON H�T� PASKA
	}
	...
	try
	{
		...
		if(dpList)
		{
			if(dpList->NumberOfItems() > 0)
			{
				...
				{
					itsDrawParam = dpList->Current(); // asetetaan todella k�ytetty drawParam k�ytt��n
					std::shared_ptr<NFmiFastQueryInfo> info = itsCtrlViewDocumentInterface->InfoOrganizer()->Info(itsDrawParam, true, true, fGetCurrentDataFromQ2Server);
					if(info == 0)
					...
					else if(crossMode == NFmiCrossSectionSystem::kObsAndFor)
					{
                        ...
                        // 1. On useita parametreja k�yt�ss� (koska vain viimeisen piirretyn layerin arvot j��v�t k�ytettyyn matriisiin)
                        // 2. Jos parametri on piilotettu (t�ll�in matriisiin ei lasketa arvoja ollenkaan)
						NFmiPoint projectedPoint(itsDataViewFrame.Project(theRelativePoint));
						value = DataMatrixInterpolation::InterpolatedValue(itsIsolineValues, projectedPoint, itsParamId);
					}
...
bool NFmiCrossSectionView::IsCrossSectionMacroParamOk(std::shared_ptr<NFmiDrawParam>& theUsedDrawParam)
{
	NFmiExtraMacroParamData extraMacroParamData;
	NFmiIsoLineData isoLineData;
	CrossSectionTooltipData tooltipData;
...
float NFmiCrossSectionView::GetLevelValue(std::shared_ptr<NFmiFastQueryInfo> &theInfo, float P, const NFmiPoint &theLatlon, const NFmiMetTime &theTime, bool doMetaParamCheck)
{
    if(theInfo)
    {
...
float NFmiCrossSectionView::GetLevelValueForMetaParam(std::shared_ptr<NFmiFastQueryInfo> &theInfo, float P, const NFmiPoint &theLatlon, const NFmiMetTime &theTime)
{
...
int NFmiCrossSectionView::CalcHorizontalPointCount(void)
{
	...
	{ // jos ollaan aika-leikkaus moodissa, pit�� katsoa kuinka monta aikaa leikkauksessa on
		std::shared_ptr<NFmiFastQueryInfo> info = itsCtrlViewDocumentInterface->InfoOrganizer()->Info(itsDrawParam, true, true);
		if(info)
		{
...
{
	// jos on esim. ladattu viewMacro k�ytt��n, pit�� paine asteikon rajat p�ivitt��
	NFmiCrossSectionSystem::ExtraRowInfo &extraRowInfo = itsCtrlViewDocumentInterface->CrossSectionSystem()->GetRowInfo(itsViewGridRowNumber - 1);  // itsViewGridRowNumber alkaa 1:st�, mutta GetRowInfo:n indeksit alkavat 0:sta.
...
// Jostain syyst� karttan�yt�ille transparency piirtoon liittyv� bitmap pit�� deletoida (muuten vuotaa),
// mutta poikkileikkausn�yt�ss� sit� ei saa tehd�, muuten kaatuu (ei vuoda, vaikka ei deletoida).
...
void NFmiCrossSectionView::DrawCrossSection(void)
{
	...
	if(fGetCurrentDataFromQ2Server)
		return ; // ei viel� q2server tukea poikkileikkausn�yt�ss�!!!!
	...
	// HUOM! T�M� ON VIRITYS!!!! -alkaa
	// Tuulensuunta-parametri on saatettu laittaa suuntanuoli asetukseen ja se toimiikiin hyvin karttan�yt�ll�. 
	// Mutta minusta olisi h�m��v�� jos tuuulen suunta piirret��n nuolella poikkileikkausn�yt�ss� ,koska se ei osoittaisi tuulen suuntaa
	// n�yt�ll� vaan maantieteellisesti. WindBarb on eri asia ja se piirret��n kuten kartalla, mutta siihen ollaan totuttu...
	// Lis�ksi nyt kun valitsin tuulen suunnan n�yt�lle, ei tapahdu mit��n, koska piirto ei tue suunta-nuolen piirtoa.
...
void NFmiCrossSectionView::DrawCrosssectionWithToolMaster(NFmiIsoLineData& theIsoLineData)
{
	...
	NFmiPoint grid2PixelRatio(0, 0); // t�t� ei k�ytet� viel� toistaiseksi poikkileikkaus n�yt�ss�, siksi alustetaan 0:ksi.
#ifndef UNIX
...
// T�ytet��n isoviivapiirtoa varten xy-piste matriisi, jossa pisteet sijaitsevat n�yt�n suhteellisessa avaruudessa.
...
void NFmiCrossSectionView::FillXYMatrix(NFmiIsoLineData &theIsoLineData, NFmiDataMatrix<NFmiPoint> &theCoordinates, std::vector<float> &thePressures)
{
	...
	if(!itsCtrlViewDocumentInterface->TrajectorySystem()->ShowTrajectoriesInCrossSectionView())
	{
		...
		// aikan�ytt��, mik� on valittu. Osa j�� tyhj�ksi tai ei tule n�kyviin. Dataa haetaan vain
...
std::shared_ptr<NFmiArea> NFmiCrossSectionView::GetZoomedArea(void)
{
	return itsCtrlViewDocumentInterface->GetMapHandlerInterface(0)->Area(); // tassa haetaan vain p��karttan�yt�n area
}
...
void NFmiCrossSectionView::FillCrossSectionMacroParamData(NFmiDataMatrix<float> &theValues, NFmiIsoLineData &theIsoLineData, std::vector<float> &thePressures, std::shared_ptr<NFmiDrawParam>& theUsedDrawParam, CrossSectionTooltipData *possibleTooltipData, NFmiExtraMacroParamData* possibleExtraMacroParamData)
{
...
static void FillCrossSectionMatrixWithSoundingData(NFmiDataMatrix<float> &theValues, std::shared_ptr<NFmiFastQueryInfo> &theInfo, std::shared_ptr<NFmiDrawParam> &theDrawParam, std::vector<float> &thePressures, int theTimeIndex)
{
...
static void FillBeforeStartTimeWithCrossSectionValues(std::shared_ptr<NFmiFastQueryInfo> &theInfo, NFmiTimeBag &theTimes, std::shared_ptr<NFmiDrawParam> &theDrawParam, std::vector<float> &thePressures, int &theBeforeStartIndex, NFmiDataMatrix<float> &theBeforeStartValues)
{
	if(theInfo->FindNearestTime(theTimes.FirstTime(), kBackward, 360))
	{
		do
		{
			...
			{ // dataa l�ytyi asemalta johonkin aikaan, t�ytet��n vektori silt� kohdalta, ja lasketaan aika-indeksi talteen
...
static void FillCrossSectionMatrixWithObservedSoundings(NFmiDataMatrix<float> &theValues, std::shared_ptr<NFmiFastQueryInfo> &theInfo, std::shared_ptr<NFmiDrawParam> &theDrawParam, NFmiTimeBag &theTimes, std::vector<float> &thePressures, int &theFirstForecastTimeIndex, std::vector<NFmiMetTime> &theObsForModeFoundObsTimes, const NFmiFastInfoUtils::MetaWindParamUsage &metaWindParamUsage)
{
	...
	for(theTimes.Reset(); theTimes.Next(); )
	{
		...
		{
			if(NFmiSoundingData::HasRealSoundingData(theInfo))
			{ // dataa l�ytyi asemalta johonkin aikaan, t�ytet��n matriisi silt� kohdalta, ja merkit��n viimeksi l�ytynyt aika-indeksi talteen
				FmiParameterName parId = static_cast<FmiParameterName>(theDrawParam->Param().GetParamIdent());
				if(theInfo->Param(parId) || parId == kFmiHumidity || metaWindParamUsage.ParamNeedsMetaCalculations(theDrawParam->Param().GetParamIdent())) // humidity lasketaan vaikka sit� ei l�ydy infosta
				{
					...
					dataFoundTimes[theFirstForecastTimeIndex] = 1; // asetetaan my�s taulukkoon ett� t�lle ajalle on l�ytynyt dataa
	...
	{ // on l�ytynyt luotauksia, mutta 1. timebagin aikaan (= indeksi 0) ei ollut mit��n luotausta.
	  // Nyt pit�� etsi� mahd. l�ytyv� edellinen luotaus aikaikkunan ulkopuolelta, ett�
	...
	{
		theFirstForecastTimeIndex = theFirstForecastTimeIndex + 1; // ennuste dataa aletaan t�ytt�m��n viimeisen l�ydetyn havainto data ajan j�lkeen
		...
		theFirstForecastTimeIndex = 0;// (theTimes.GetSize() / 2) + 1;  // jos ei l�ytynyt kuitenkaan havainto dataa, ennusteita aletaan t�ytt�m��n vasta puolesta v�list�, ett� k�ytt�j� huomaa  puutteen
}
...
int NFmiCrossSectionView::FillObsPartOfTimeCrossSectionData(NFmiDataMatrix<float>& theValues, NFmiIsoLineData& theIsoLineData, std::vector<float>& thePressures)
{
	...
	std::shared_ptr<NFmiFastQueryInfo> soundingInfo = itsCtrlViewDocumentInterface->InfoOrganizer()->GetPrioritizedSoundingInfo(NFmiInfoOrganizer::ParamCheckFlags(true));
	if(soundingInfo && soundingInfo->NearestLocation(NFmiLocation(point), 500 * 1000))
	{
		...
		{ // luultavasti haluttua parametria ei l�ytynyt
			firstForecastTimeIndex = 0; //(times.GetSize() / 2) + 1;  // jos ei l�ytynyt havainto dataa, parametria tai asemaa tarpeeksi l�helt�, ennusteita aletaan t�ytt�m��n vasta puolesta v�list�, ett� k�ytt�j� huomaa  puutteen
		}
	}
	else
	{
		crossSectionSystem->ObsForModeLocation(NFmiLocation()); // jos asemaa ei l�ytynyt, laitetaan ns. default location, miss� asema id on 0
		firstForecastTimeIndex = 0; //(times.GetSize() / 2) + 1;  // jos ei l�ytynyt havainto dataa, parametria tai asemaa tarpeeksi l�helt�, ennusteita aletaan t�ytt�m��n vasta puolesta v�list�, ett� k�ytt�j� huomaa  puutteen
	}
...
bool CalcCrossSectionMetaWindParamMatrix(const std::shared_ptr<NFmiFastQueryInfo> &info, NFmiDataMatrix<float> &theValues, std::vector<float> &thePressures, unsigned long wantedParamId, const NFmiFastInfoUtils::MetaWindParamUsage &metaWindParamUsage, GetDataFunction getDataFunction)
{
    if(metaWindParamUsage.HasWsAndWd())
    {
...
static bool IsContourDrawUsed(const std::shared_ptr<NFmiDrawParam> &drawParam)
{
...
static NFmiDataMatrix<float> MakePressureLevelBasedPressureMatrix(size_t xSize, NFmiFastQueryInfo &usedInfo)
{
	...
	std::vector<float> pressureLevelValuesBS = usedInfo.CalcPressureLevelDataPressures();
	...
	{
		// paineet pit�� k��nt��, koska ne k��nnet��n my�s datan haussa
...
static bool DoTimeBagSpacingOut(CtrlViewDocumentInterface *theCtrlViewDocumentInterface, std::shared_ptr<NFmiDrawParam> &theDrawParam)
{
...
// t�ll�in timebagi, jossa onkin yhden tunnin aika-askel (=> v�hemm�n turhaa ty�t� ja nopeampi ruudun p�ivitys)
NFmiTimeBag NFmiCrossSectionView::GetUsedTimeBagForDataCalculations(void)
{
	...
	if(crossMode == NFmiCrossSectionSystem::kObsAndFor || crossMode == NFmiCrossSectionSystem::kTime)
	{
		// jos p��karttan�yt�n (sielt� otetaan perus aika-askel) aika-askel on suurempi kuin nykyisen k�yt�ss� ollen timebagin,
		// oletetaan ett� ollaan raahaamassa poikkileikkausn�yt�n aika-ikkunaa
...
// theDir joko arvo kUp siirt�� asteikkoa halutulla m��r�ll� yl�sp�in muut arvot alasp�in
...
bool NFmiCrossSectionView::ChangePressureScale(FmiDirection theDir, bool fChangeUpperAxis, bool ctrlKeyDown)
{
	...
	{
		// kun asteikko on vedetty tarpeeksi lyhyeksi, muutetaan muutos arvoa pienemm�ksi, ett� voidaan tehd� tarkempia s��t�j�
...
// Lasketaan itse data n�yt�n koko korkeus akselin avulla.
...
//	if(itsDoc->GetMTATempSystem().ShowKilometerScale()) // jos olisi s��t� ett� piirret��nk� km-asteikko vai ei, olisi ehto laitettava t�h�n
//		extraOffsetInPixels = static_cast<long>(fontSize*1.6); // siirret��n asteikkoa, jos my�s kilometri asteikko on n�kyviss�
...
// T�t� timebagia k�ytet��n my�s tuuliviirien piirron harvennuksessa.
...
// piirt�� my�s kaikki ajanhetket, mist� on havaittu luotaus
void NFmiCrossSectionView::DrawObsForModeTimeLine(void)
{
	if(itsCtrlViewDocumentInterface->TrajectorySystem()->ShowTrajectoriesInCrossSectionView() == false)
	{
		if(itsCtrlViewDocumentInterface->CrossSectionSystem()->GetCrossMode() == NFmiCrossSectionSystem::kObsAndFor && itsFirstForecastTimeIndex >= 0)
		{
			...
			// piirr� my�s viiva havaittujen luotauksien kohdalle
...
// Piirt�� standardi ilmakeh�n mukaiset kilometri palkit ja tekstit n�ytt�ruudun 
...
//	if(itsDoc->GetMTATempSystem().ShowKilometerScale())
	{
		...
		// k�yt�ss� ainakin 2, mik� tarkoittaa kymmenien metrien tarkkuutta
...
std::vector<float> NFmiCrossSectionView::CalcCurrentLevelPressures(std::shared_ptr<NFmiFastQueryInfo> &theInfo)
{
...
std::shared_ptr<NFmiFastQueryInfo> NFmiCrossSectionView::GetFirstHybridInfo(void)
{
	NFmiDrawParamList *dpList = itsCtrlViewDocumentInterface->CrossSectionViewDrawParamList(itsViewGridRowNumber);
	if(dpList)
	{
		for(dpList->Reset(); dpList->Next(); )
		{
			std::shared_ptr<NFmiDrawParam> drawParam = dpList->Current();
			std::shared_ptr<NFmiFastQueryInfo> hybridInfo = itsCtrlViewDocumentInterface->InfoOrganizer()->Info(drawParam, true, false);
...
{
	std::shared_ptr<NFmiFastQueryInfo> hybridInfo = GetFirstHybridInfo();
...
static bool DrawBatymetria(NFmiDrawParamList &theDrawParamList, NFmiInfoOrganizer &theInfoOrganizer)
{
	if(theDrawParamList.Index(1)) // etsit��n 1. drawParam
	{
		...
		std::shared_ptr<NFmiFastQueryInfo> firstDataInfo = theInfoOrganizer.Info(currentDrawParam, true, true);
		if(firstDataInfo && firstDataInfo->SizeLevels() > 1) // datassa pit�� olla useita leveleit�
		{
			for(firstDataInfo->ResetLevel(); firstDataInfo->NextLevel(); )
			{
				if(firstDataInfo->Level()->LevelValue() < 0)
					return true; // jos datan yksikin level value on negatiivinen, t�ll�in oletetaan ett� tarvitaan batymetria piirtoa
			}
		}
	}
	return false;
...
// Piirret��n maanpinta seuraavin prioriteetti s��nn�in:
// 0. Jos 1. piirrett�v� data vaatii batymetriaa (= level heigth sis�lt�� negatiivisia arvoja), piirret��n maanpinta topo-datasta.
// 1. Jos n�yt�ss� mallidataa, jolta l�ytyy pintadatasta kFmiPressureAtStationLevel -parametri, 
//    k�ytet��n sit� m��ritt�m��n maanpintaa (paine-asteikolle)
// 2. Jos n�yt�ss� mallipinta dataa, piirr� mallipintadatan alinkerros maanpintana (paine-asteikolle)
// 3. K�y drawParam-Listaa l�pi kunnes l�ytyy data, jolta l�ytyy pinnan piirto kohdasta 1 tai 2.
...
{ 
	...
	if(dpList)
	{
		...
		if(drawBatymetria == false)
		{
			...
			{
				std::shared_ptr<NFmiDrawParam> drawParam = dpList->Current();
				std::shared_ptr<NFmiFastQueryInfo> groundInfo = itsCtrlViewDocumentInterface->InfoOrganizer()->FindInfo(NFmiInfoData::kViewable, *(drawParam->Param().GetProducer()), true);
				...
				// Haetaan paras verkikaali data, mik� mallista l�ytyy (1. hybrid-data, 2. pressure-data), ja jos se oli hybrid, piirret��n sen alin kerros
				std::shared_ptr<NFmiFastQueryInfo> hybridInfo = itsCtrlViewDocumentInterface->InfoOrganizer()->FindSoundingInfo(*(drawParam->Param().GetProducer()));
				if(hybridInfo && hybridInfo->DataType() == NFmiInfoData::kHybridData)
				{
...
// piirret��n karkea maanpinta poikkileikkaus n�ytt��n
...
// kuin toolboxin clippaus laitetaan falseksi. T�ll�in on kutsuttu QuickInitMainXYPoints
...
// Jos annetusta vektorista l�ytyy yksikin ei missing arvo, palauta true, muuten false
...
// Piirret��n mallista saatu karkea maanpinta profiili n�ytt��n.
// Palauttaa true, jos piirretty jotain, muuten false.
bool NFmiCrossSectionView::DrawModelGroundLevel(std::shared_ptr<NFmiFastQueryInfo> &theInfo, NFmiDrawingEnvironment &theEnvi, bool fUseHybridCalculations)
{
	if(theInfo)
	{
        ...
        // Jos datassa ei ollutkaan haluttua parametria, t�ll�in itsModelGroundPressures on tyhj� ja ei voida piirt�� mit��n.
...
void NFmiCrossSectionView::CalcGroundHeights(void) //NFmiSmartInfo *theInfo)
{
	...
	std::shared_ptr<NFmiFastQueryInfo> topoInfo = itsCtrlViewDocumentInterface->InfoOrganizer()->FindInfo(NFmiInfoData::kStationary);
...
void NFmiCrossSectionView::CalcModelGroundPressures(std::shared_ptr<NFmiFastQueryInfo> &theInfo)
{
...
void NFmiCrossSectionView::CalcModelPressuresAtStation(std::shared_ptr<NFmiFastQueryInfo> &theInfo)
{
	itsModelGroundPressures.clear();
