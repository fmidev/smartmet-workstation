...
void NFmiIsoLineView::EndTransparentDraw(void)
{
    ...
    // Kun CDC:ll� olevaan bitmapiin on piirretty, ota oikea CDC k�ytt��n
    ...
    if(fDrawUsingTransparency)
    {
        ...
        // HUOM! kuvan saa piirt�� vain oman n�yt�n p��lle, muuten jos ollaan moni ruutuisessa kokonaisn�yt�ss�,
        // tulee alpha piirtoa eri ruutujen p��lle useina kerroksina ja t�ll�in ensin piirrettyjen ruutujen
...
// T�m� funktio palauttaa true, jos pelk�n zoomatun alueen datan k�ytt� ja piirto on mahdollista.
// Jos mahdollista palauttaa my�s uuden zoomed-area-rectin ja zoomatun alueen hilan 'boundingbox' indeksit.
// Zoomatun datan k�ytt� mahdollista jos:
// 1. teht�v� s��st� hilapisteiss� on riitt�v�.
...
// Osuvatko data j� n�yt�n alue ollenkaan p��llekk�in.
...
// Tuulen suunta on kulma joka alkaa pystysuunnasta (0/360 astetta) ja menee my�t�p�iv��n ymp�ri. 
...
{
    float mm_per_pixel = 1000.f / 260.f; // arvio omasta n�yt�st�ni
...
static void SetSimpleColorContourEnvi(NFmiIsoLineData& theIsoLineData, NFmiDrawingEnvironment &theEnvi, float theLowerLimit, float theUpperLimit)
{
    ...
    {
        theEnvi.DisableFrame(); // T�M� RAJAVIIVAN PIIRRON ESTO PIT�� KATSOA jotenkin toolboxista, kun ei toimi
    }
...
void NFmiIsoLineView::DrawIsoLinesWithImagine(void)
{
    ...
    { // jos l�ytyi cachesta, laitetaan ne koordinaateiksi
...
static Imagine::NFmiPath CalcContourPath(float theValue1, float theValue2, NFmiIsoLineData& theIsoLineData, NFmiDataMatrix<float> &theValues, NFmiDataMatrix<NFmiPoint> &theCoordinates, Imagine::NFmiDataHints &theHelper, const boost::shared_ptr<NFmiArea> &theMapArea, bool fLinesOnly)
{
    ...
    if(theIsoLineData.itsXNumber * theIsoLineData.itsYNumber > gMaxGridPointsThatImagineWillSubTriangulate) // ei tehd� subtriangle jakoa jos hilapisteit� tarpeeksi paljon muutenkin
        tree.SubTriangleMode(false);								// eli n. 30 x 30 (=900) hilasta yl�sp�in ei tehd� alikolmioita
...
void NFmiIsoLineView::AddLabelsToDrawList(Imagine::NFmiPath &thePath, float theIsoLineValue, const NFmiRect &theRelativeRect, LabelBox &theLabelBox, const NFmiPoint &theOffSet)
{
    ...
    // irroita my�s ghostlinet (skippaa ne)
...
bool NFmiIsoLineView::MoveLabelBoxToBestPossiblePlace(const NFmiPathData &thePath, const NFmiRect &theRelativeRect, LabelBox &theLabelBox)
{
    ...
    if(thePath.size() >= 2)
    {
        ...
        NFmiPoint sparePoint(centerPoint); // t�m� otetaan k�ytt��n jos muuta paikkaa ei l�ydy
        ...
        for(int i = 0; i < 2; i++)
        {
            ...
            for(int j = beginIndex; j != endIndex; j += traverseDirection)
            {
                ...
                if(theRelativeRect.IsInside(theLabelBox.LabelBoxRect()))
                {
                    ...
                    { // l�ytyi hyv� paikka, otetaan se talteen ja lopetetaan
        ...
        {
            theLabelBox.Center(sparePoint); // jos ei l�ytynyt 't�ydellist�' paikaa, k�ytet��n se varapaikka
            StoreLabel(theLabelBox);
            return true;
        }
    }
    ...
    for(long i = 0; iter != endIter; ++iter, i++)
    {
        ...
        for(long j = 0; iter2 != endIter; j++)
        {
            ...
            else if(CoordComparison(p1[p1.size() - 1], p2[0]))
            {
                ...
                ++iter2;
            //else ei osaa viel� verrata pelkk�st��n alkuja tai loppuja  koska se vaatii viel� polun k��nt��
        }
    }
}
...
// piirret��nk� toolmasterilla vai imaginella. Piirret��n toolmasterilla toistaiseksi
// nopeuden ja n��n takia jos voi.
// Eli palauta false = piirr� toolmasterilla jos se on k�yt�ss� ja
...
// T�m�n ei tarvitse palauttaa tietoa ett� k�sittelik� t�m� asiat, koska t�m� aina k�sittelee loput tapaukset, mit� ei edell� saatu k�sitelty�.
...
// FillGridRelatedData metodin paloittelu osiin, tapaus jossa on k�yt�ss� Visualization optimization systeemi.
// T�ll�in lasketaan dataan ja kartta-alueeseen sopiva harvennettu hila ja data t�ytet��n siihen.
// Palauttaa bool arvon: k�sittelik� t�m� funktio kyseisen datan tapauksen, eik� tarvitse tehd� en�� muuta.
...
bool NFmiIsoLineView::FillGridRelatedData_BetterVisualizationChecks(NFmiIsoLineData& isoLineData, NFmiRect& zoomedAreaRect, bool& fillGridDataStatus)
{
    ...
    if(!fGetCurrentDataFromQ2Server && dataUtilitiesAdapter->isModifiedDataDrawingPossible())
    {
        if(dataUtilitiesAdapter->isThereAnythingToDraw())
        {
            ...
            // Mutta pit�� varmistaa ett� data interpoloidaan oikealta ajalta my�s klimatologisilta datoilta (kuten Era-5, tms.)
