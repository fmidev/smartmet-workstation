...
bool NFmiIsoLineView::FillIsoLineVisualizationInfo(boost::shared_ptr<NFmiDrawParam> &theDrawParam, NFmiIsoLineData* theIsoLineData, bool fToolMasterUsed, bool fStationData)
{
    ...
    auto viewType = fStationData ? theDrawParam->StationDataViewType() : theDrawParam->GridDataPresentationStyle();
    if(viewType == NFmiMetEditorTypes::View::kFmiColorContourView || viewType == NFmiMetEditorTypes::View::kFmiColorContourIsoLineView || viewType == NFmiMetEditorTypes::View::kFmiQuickColorContourView)
    {
    ...
    if(viewType == NFmiMetEditorTypes::View::kFmiIsoLineView || viewType == NFmiMetEditorTypes::View::kFmiColorContourIsoLineView)
    {
...
void NFmiIsoLineView::FillCustomColorContourInfo(boost::shared_ptr<NFmiDrawParam> &theDrawParam, NFmiIsoLineData* theIsoLineData, bool fStationData, bool fToolMasterUsed)
{
    ...
    auto presentStyle = fStationData ? theDrawParam->StationDataViewType() : theDrawParam->GridDataPresentationStyle();
    if(presentStyle == NFmiMetEditorTypes::View::kFmiColorContourView && theDrawParam->UseSeparatorLinesBetweenColorContourClasses())
        theIsoLineData->fUseSeparatorLinesBetweenColorContourClasses = true;
    if(presentStyle == NFmiMetEditorTypes::View::kFmiQuickColorContourView)
        theIsoLineData->fUseColorContours = 2; // 2 asettaa quick contourin p‰‰lle
    if(presentStyle == NFmiMetEditorTypes::View::kFmiColorContourIsoLineView)
...
void NFmiIsoLineView::FillIsoLineInfoCustom(boost::shared_ptr<NFmiDrawParam> &theDrawParam, NFmiIsoLineData* theIsoLineData, bool fStationData)
{
    ...
    auto vieStyle = fStationData ? theDrawParam->StationDataViewType() : theDrawParam->GridDataPresentationStyle();
    bool colorContourAndIsolines = vieStyle == NFmiMetEditorTypes::View::kFmiColorContourIsoLineView;
...
bool NFmiIsoLineView::FillIsoLineGridData(NFmiDataMatrix<float> &theValues, NFmiIsoLineData& theIsoLineData)
{
    ...
    if(CalcViewFloatValueMatrix(theValues, 0, 0, 0, 0) == false) // imaginella piirrolla ei ole viel‰ croppi optimointia
        return false;
...
void NFmiIsoLineView::DrawIsoLinesWithImagine(void)
{
    ...
    isoLineData.Init(xCount, yCount, 500); // maksimissaan 500 eriarvoista isoviivaa suostutaan piirt‰m‰‰n (muuten rupeaa jo kest‰m‰‰nkin liian kauan)
...
void NFmiIsoLineView::ParamGradientShade(boost::shared_ptr<NFmiFastQueryInfo> &theInfo, NFmiDataMatrix<float> &theValues, NFmiDataMatrix<NFmiPoint> &theCoords)
{
    // 15.9.04/EL
    // KESKEN - EI TOIMI!
    // TƒMƒ ON VAIN TESTIMETODI GRADIENTIN ESITTƒMISEKSI TOOLBOXIN KAUTTA (!)
    // TODO: itse parametri pit‰isi asettaa ...
    ...
    float yIncr = 1.f / (float)ny; // Pseudopixeleiss‰
    float xIncr = 1.f / (float)nx; // Pseudopixeleiss‰
    ...
    for(double y = yStart; y <= yEnd; y += yIncr)
        for(double x = xStart; x <= xEnd; x += xIncr)
        {
            ...
            // TODO: TƒHƒN GRADIENTIN LASKEMINEN NAAPUREISTA - NYT ON KƒYT÷SSƒ VAIN YKSI PISTE ...
...
bool NFmiIsoLineView::initializeIsoLineData(NFmiIsoLineData &theIsoLineData)
{
    ...
    theIsoLineData.Init(nx, ny, 500); // 500 on maksimi m‰‰r‰ mit‰ eri arvosia isoviivoja suostutaan piirt‰m‰‰n
    for(int j = 0; j < ny; j++)
    {
        for(int i = 0; i < nx; i++)
