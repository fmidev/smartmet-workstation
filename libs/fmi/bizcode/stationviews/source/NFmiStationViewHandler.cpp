...
void NFmiStationViewHandler::DoBasicDrawing(NFmiToolBox * theGTB, const NFmiRect &theMapFrame)
{
    ...
    // piirret‰‰n n‰m‰ kahdesti, sek‰ heti datan p‰‰lle, ett‰ DrawOverBitmapThings-metodissa
    // jolloin ne tulevat n‰kyviin myˆs t‰lletett‰viin kuviin.
    //----------------------------------------------------------------------
    DrawSelectedMTAModeSoundingPlaces();
    DrawSoundingPlaces();
    DrawCrossSectionPoints();
    DrawTimeText(); // piirret‰‰n t‰ss‰ myˆs aikatekstit
                    //----------------------------------------------------------------------
...
void NFmiStationViewHandler::Update(void)
{
	if(itsViewList)
	{
		...
		if(drawParamList && (mapHandler->IsMapDirty() || drawParamList->IsDirty()))
		{
...
bool NFmiStationViewHandler::LeftButtonDown(const NFmiPoint& thePlace, unsigned long theKey)
{
	...
	{
        auto crossSectionSystem = itsCtrlViewDocumentInterface->CrossSectionSystem();
        if(ShowParamHandlerView() && itsParamHandlerView->IsIn(thePlace)) // param-n‰ytˆn on napattava ensimm‰iseksi hiiren toiminnot!!!!!!!!
		{
...
{
	itsCtrlViewDocumentInterface->SelectLocations(itsMapViewDescTopIndex, theInfo, itsMapArea, theLatLon, itsTime, theSelectionCombineFunction, theMask, fRedrawMapAfterMTATempClear, fMakeMTAModeAdd, fDoOnlyMTAModeAdd);
}
...
bool NFmiStationViewHandler::LeftButtonUp(const NFmiPoint & thePlace, unsigned long theKey)
{
    ...
    if(itsViewList && GetFrame().IsInside(thePlace))
	{
        // Uuden ParamAdding-dialogin tarvitsemia asetuksia laitetaan t‰ll‰ uudella funktiolla.
		...
		if(ShowParamHandlerView() && itsParamHandlerView->IsIn(thePlace)) // napattava ensimm‰iseksi hiiren toiminnot!!!!!!!!
		{
			return itsParamHandlerView->LeftButtonUp(thePlace, theKey);
		}
...
bool NFmiStationViewHandler::MouseWheel(const NFmiPoint &thePlace, unsigned long theKey, short theDelta)
{
	if(itsViewList && GetFrame().IsInside(thePlace))
	{
		...
		if((theKey & kCtrlKey) && (theKey & kShiftKey))
		{
            ...
            if(status)
                itsCtrlViewDocumentInterface->MapDirty(itsMapViewDescTopIndex, true, true);
            return status; // jos yksikin level vaihtui, p‰ivitet‰‰n ikkunat
		...
		else if(theKey & kShiftKey) // jos shift-nappi pohjassa muutetaan hybrid-datojen leveli‰ ylˆs/alas tai satel datan kanavaa
		{
			bool status = false;
			for(itsViewList->Reset(); itsViewList->Next(); ) // hybrid data muutos ensin
			{
