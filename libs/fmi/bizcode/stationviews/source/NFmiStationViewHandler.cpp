...
void NFmiStationViewHandler::DrawNormalColorContourLegendBackground(LegendDrawingMeasures& legendDrawingMeasures, const Gdiplus::PointF& lastLegendBottomRightCornerInPixels)
{
    auto& colorContourLegendSettings = itsCtrlViewDocumentInterface->ColorContourLegendSettings();
...
void NFmiStationViewHandler::DrawNormalColorContourLegendClassColorRect(const NFmiColorRectSettings &drawSettings, const Gdiplus::Rect &rectInPixels, const NFmiColor &color, bool transparentColor)
{
    auto pixelsPerMM = itsCtrlViewDocumentInterface->GetGraphicalInfo(itsMapViewDescTopIndex).itsPixelsPerMM_y;
...
void NFmiStationViewHandler::DrawNormalColorContourLegendClassColors(const LegendDrawingMeasures& legendDrawingMeasures, const NFmiColorContourLegendValues& colorContourLegendValues, const Gdiplus::PointF& lastLegendBottomRightCornerInPixels)
{
    auto& colorContourLegendSettings = itsCtrlViewDocumentInterface->ColorContourLegendSettings();
    ...
    for(size_t index = 0; index < classColors.size(); index++)
    {
        ...
        {
            DrawNormalColorContourLegendClassColorRect(colorContourLegendSettings.invsibleColorRectSettings(), colorRectInPixels, color, true);
        }
        else
        {
            DrawNormalColorContourLegendClassColorRect(colorContourLegendSettings.backgroundRectSettings(), colorRectInPixels, color, false);
        }
    }
}

void NFmiStationViewHandler::DrawNormalColorContourLegendClassValueTexts(const LegendDrawingMeasures& legendDrawingMeasures, const NFmiColorContourLegendValues& colorContourLegendValues, const Gdiplus::PointF& lastLegendBottomRightCornerInPixels)
{
    const auto &colorContourLegendSettings = itsCtrlViewDocumentInterface->ColorContourLegendSettings();
    ...
    for(const auto& classLimitText : colorContourLegendValues.classLimitTexts())
    {
        ...
        auto relativeTextPosition = CtrlView::GdiplusPoint2Relative(itsToolBox, textLocationInPixels);
        CtrlView::DrawTextToRelativeLocation(*itsGdiPlusGraphics, textColor, legendDrawingMeasures.usedFontSizeInMM, classLimitText, relativeTextPosition, pixelsPerMM, itsToolBox, fontName, kRight, Gdiplus::FontStyleRegular);
    }
    ...
    auto textLocationInPixels = Gdiplus::Point(currentTextPosXInPixels, currentTextPosYInPixels);
    auto relativeTextPosition = CtrlView::GdiplusPoint2Relative(itsToolBox, textLocationInPixels);
    itsGdiPlusGraphics->SetClip(legendDrawingMeasures.backgroundRectInPixels);
    CtrlView::DrawTextToRelativeLocation(*itsGdiPlusGraphics, textColor, legendDrawingMeasures.usedFontSizeInMM, colorContourLegendValues.name(), relativeTextPosition, pixelsPerMM, itsToolBox, fontName, kLeft, Gdiplus::FontStyleRegular);
...
static void DrawPolyLineList(NFmiToolBox *theGTB, list<NFmiPolyline*> &thePolyLineList, const NFmiPoint &theOffSet, NFmiCtrlView *ctrlView)
{
	NFmiPoint scale;
...
static void PushOldPolyLineAndStartNew(std::list<std::vector<NFmiPoint>> &relativePolyLineList, std::vector<NFmiPoint> &currentPolyLine, const NFmiPoint &newPoint)
{
...
static std::list<std::vector<NFmiPoint>> ConvertPath2RelativePolyLineListGdiplus(Imagine::NFmiPath& thePath, bool relative_moves, bool removeghostlines)
{
...
static void DrawPolyLineListGdiplus(Gdiplus::Graphics &gdiplusGraphics, NFmiToolBox *toolbox, const std::list<std::vector<NFmiPoint>>& polyLineListGdiplus, const NFmiPoint &relativeOffSet, NFmiCtrlView *ctrlView, const NFmiColor &lineColor, int lineThickness)
{
...
static void DrawLandBorders(NFmiStationViewHandler *mapView, NFmiToolBox *toolbox)
{
    auto ctrlViewDocumentInterface = mapView->GetCtrlViewDocumentInterface();
    int mapViewDescTopIndex = mapView->MapViewDescTopIndex();
    auto mapArea = mapView->MapArea();
...
static void fillMapWithTransparentColor(CDC *theUsedCDC, const CRect& mfcRect)
{
...
static void DrawLandBordersToCacheBitmap(NFmiStationViewHandler *mapView, NFmiToolBox *toolbox)
{
    int mapViewDescTopIndex = mapView->MapViewDescTopIndex();
...
static void DrawLandBordersFromCacheBitmap(NFmiStationViewHandler *mapView, NFmiToolBox *toolbox)
{
    CtrlViewUtils::CtrlViewTimeConsumptionReporter::makeSeparateTraceLogging(__FUNCTION__, mapView);
    int mapViewDescTopIndex = mapView->MapViewDescTopIndex();
    auto ctrlViewDocumentInterface = mapView->GetCtrlViewDocumentInterface();
    ...
    auto destPixels = CtrlView::Relative2GdiplusRectF(toolbox, mapView->MapArea()->XYArea());
...
static void DrawLandBordersWithBitmap(NFmiStationViewHandler *mapView, NFmiToolBox *toolbox)
{
...
void NFmiStationViewHandler::DrawData(NFmiToolBox* theGTB)
{
    ...
    if(itsCtrlViewDocumentInterface->Printing())
        ::DrawLandBorders(this, theGTB);
    else
        ::DrawLandBordersWithBitmap(this, theGTB);
}
...
void NFmiStationViewHandler::DrawTimeText(void)
{
	...
	{
		if(itsCtrlViewDocumentInterface->ShowTimeString(itsMapViewDescTopIndex)) // toteuta t‰m‰ dokumenttiin!!!!!
		{
			if(DrawTimeTextInThisMapViewTile())
			{
				...
				else // else:ss‰ vanhan tyylinen aikateksti printtaus
				{
...
void NFmiStationViewHandler::InitParamHandlerView(void)
{
	...
	itsParamHandlerView = new NFmiParamHandlerView(itsMapViewDescTopIndex
												  ...
												  ,true); // true = n‰ytet‰‰n mask-osio (mutta vain 1. rivill‰)
...
void NFmiStationViewHandler::UpdateParamHandler(void)
{
	if(itsParamHandlerView)
	{
		...
		itsParamHandlerView->Update(itsParamHandlerViewRect, itsToolBox);
		itsParamHandlerViewRect = itsParamHandlerView->GetFrame(); // rect:i saattaa muuttua Update:n yhteydess‰
	}
}
...
void NFmiStationViewHandler::DrawSelectedLocations(void)
{
    ...
    {   // Tarvittaessa luodaan yksi n‰yttˆ, jonka avulla piirret‰‰n valitut pisteet
        NFmiDrawingEnvironment envi;
        NFmiStationView stationView(itsMapViewDescTopIndex, this->MapArea(), itsToolBox, &envi, itsDrawParam, kFmiTemperature, NFmiPoint(0, 0), NFmiPoint(1, 1), itsViewGridRowNumber, itsViewGridColumnNumber);
...
void NFmiStationViewHandler::SetViewListArea(void)
{
	if(itsViewList && itsMapArea)
	{
		for(itsViewList->Reset(); itsViewList->Next(); )
		{
			((NFmiAreaView*)itsViewList->Current())->Area(itsMapArea);
		}
	}
}
...
{
	std::string str;
    // Parametri laatikon p‰‰lt‰ ei tehd‰ tooltippi‰, koska jotkut tooltipien laskut ovat super hitaita ja saattavat h‰irit‰ parametrin valinta popupin avautumista.
    // Tehd‰‰n param-boxille oma tooltip jos tarvis...
