...
void NFmiStationViewHandler::DoBasicDrawing(NFmiToolBox * theGTB, const NFmiRect &theMapFrame)
{
    ...
    DrawData(theGTB);
    DrawOverMap(theGTB, theMapFrame); // piirret��n haluttaessa kartan ja datan p��lle l�pin�kyv� esim. kaupunkien nimet/tiest� kartta
...
void NFmiStationViewHandler::DoCacheDrawing(NFmiToolBox * theGTB, const NFmiRect &theMapFrame, CDC *destinationDc, int cacheRowIndex)
{
    ...
    else
    {
        ...
        viewCache->DeleteObject();  // en tied� pit��k� ep�onnistuneen luonnin j�lkeen tuhota objectia
        delete viewCache;
        if(firstTime)
        {
            firstTime = false;
...
// Lasketaan eri karttan�ytt�moodeissa t�m�n olion k�ytt�m� cache rivi.
// cacheRowIndex luvut alkavat 0:sta ja menev�t 49:��n.
// itsViewGridRowNumber kertoo kuinka monennella n�ytt�ruudukossa n�kyv�ll� kartta rivill� 
...
void NFmiStationViewHandler::Draw(NFmiToolBox * theGTB)
{
	...
	{
		if(!viewCache)
		{ // T�t� ruutua ei l�ydy cachesta, pit�� piirt��.
			try
			{
...
void NFmiStationViewHandler::DrawCPCropArea(void)
{
	if(itsCtrlViewDocumentInterface->IsCPGridCropInAction())
	{
		...
		NFmiRect relativeRect = CalcCPCropAreasRelativeRect(itsCtrlViewDocumentInterface->CPGridCropLatlonArea());
			...
			{ // CP croppaus on k�yt�ss�, mutta ei mahdollista, piirr� varoittava punainen rasti kartalle
...
void NFmiStationViewHandler::DrawSelectedMTAModeSoundingPlaces(void)
{
	...
	if(temps.size() > 0)
	{
		...
		for( ; it != temps.end(); ++it)
		{
            // jos luotaus-aikalukitus p��ll� ja aika oli sama kuin aktiivisen karttan�yt�n aika, TAI jos ollaan normaali moodissa, oliko luotaus olion aika ja kartan aika samat
...
bool NFmiStationViewHandler::IsSoundingMarkersDrawnOnThisMap(bool fDrawThisOnEveryRow)
{
    ...
    if(mtaSystem.TempViewOn() && mtaSystem.ShowMapMarkers() && itsCtrlViewDocumentInterface->ShowSoundingMarkersOnMap(itsMapViewDescTopIndex))
    {
        ...
        // Riippuen karttan�yt�ss� k�ytetyst� aikamoodista (mapViewDisplayMode):
        // 0. Normaali moodi -> piirret��n vain alimmalle n�ytt�ruudukon riville (jokaisessa sarakkeessa sama aika)
        // 1. One-time moodi -> piirret��n vain alimman n�ytt�ruudukon rivin oikeaan kulmaan
        // 2. Running-time moodi -> piirret��n kaikille n�ytt�ruudukon ruuduille (kaikilla eri aika)
...
// ja halutaan n�ytt�� kyseisi� luotauksia luotaus n�yt�ss� (ruksi p��ll�).
void NFmiStationViewHandler::DrawSoundingPlaces(void)
{
	...
	for(size_t i = 0; i<extraSoundingProds.size(); i++)
	{
        ...
        if(prod.GetIdent() == kFmiMAST)
    		...
    		DrawSoundingSymbols(soundingInfo, 3, 0.9); // 3=piirr� neli�
	}
...
void NFmiStationViewHandler::DrawSoundingSymbols(boost::shared_ptr<NFmiFastQueryInfo> &theSoundingInfo, int theUsedSymbol, double theSymbolSizeInMM)
{
	...
	{ // jos luotaus info l�ytyi ja l�ytyi viel� aikakin, niin aletaan asemien piirto
		NFmiDrawingEnvironment envi;
		// piirret��n oranssi kolmia osoittamaan asemien paikkaa
		envi.EnableFill();
		envi.SetFillColor(NFmiColor(1.f, 0.35f, 0.f));
		...
		for(theSoundingInfo->ResetLocation() ; theSoundingInfo->NextLocation(); )
		{
			...
			if(NFmiSoundingData::HasRealSoundingData(theSoundingInfo))
				markerPolyLine.GetEnvironment()->SetFillColor(NFmiColor(0.551f, 0.9f, 0.21f)); // l�ytyy dataa, vihre� kolmio
			else
...
void NFmiStationViewHandler::DrawSilamStationMarkers(void)
{
	...
	try
	{
		...
		{
			// piirret��n vain alimm�lle n�ytt�riville
...
namespace
{
	...
	AnimationButtonImageHolder gAnimationButtonImageHolder; // yksi instannsi luodaan nimett�m��n namespaceen
}
#endif // UNIX
...
void NFmiStationViewHandler::DrawKaHaMessageIcon(const HakeMessage::HakeMsg &theWarningMessage, const NFmiPoint &latlon)
{
    ...
    case 9:
    { // S��n aiheuttama tieliikenneh�iri�
...
void NFmiStationViewHandler::DrawWmsLegends(NFmiToolBox* theGTB)
{
	...
	if(drawParamList)
	{
		for(const auto& registered : registeredLayers)
		{
			...
			{
				// Viel� jos piirto-optioissa ei ole legendan piirto p��ll�, poistetaan rekister�idyist� (en ymm�rr� logiikkaa, miten sen saa taas p��lle)
...
static std::string MakeUsedTimeboxTimeFormatString(const std::string &theOriginalTimeFormatString, const NFmiBetaProduct *theUsedBetaProduct)
{
    if(theUsedBetaProduct && !theUsedBetaProduct->UseUtcTimesInTimeBox())
    {
        ...
        boost::algorithm::ireplace_first(finalFormatString, "utc", ""); // Poistetaan vain 1. l�ytyv�, jos utc on useita kertoja niin ei voi mit��n...
...
// piirt�� reunat sopivalla v�rill� ja mahdollisesti levitysty�kalun jutut
// 1. normaali ei aktiivinen v�ri musta, ohut reuna
// 2. Aktiivinen n�ytt�, rivi ja aika punaisella paksulla reunalla
// 3. aktiivinen n�ytt� ja rivi ohuella haaleammalla punaisella
...
void NFmiStationViewHandler::Update(void)
{
	if(itsViewList)
	{
		...
		if(drawParamList && (doGeneralViewUpdates || drawParamList->IsDirty()))
		{
			...
			itsViewList->Time(itsTime); // pit�� asettaa my�s aika oikein, koska joskus n�ytt�jen synkkaus menee pieleen ja luultavasti currentti aika joka on j��nyt t�ss� updatessa, joutuu mouseMove:ssa taas luotauksen ajaksi jos k�ytt�j� v��nt�� hiiri pohjassa karttan�yt�ss�
			SetViewListArea();
		}
	}
}
...
bool NFmiStationViewHandler::LeftButtonDown(const NFmiPoint& thePlace, unsigned long theKey)
{
	...
	if(itsViewList && GetFrame().IsInside(thePlace))
	{
		...
		if(IsMouseCursorOverParameterBox(thePlace)) // param-n�yt�n on napattava ensimm�iseksi hiiren toiminnot!!!!!!!!
		{
...
void NFmiStationViewHandler::DoTotalLocationSelection(const NFmiPoint & thePlace, const NFmiPoint &theLatlon, unsigned long theKey, bool fDrawSelectionOnThisView)
{
	try
	{
		if(itsViewList->NumberOfItems() > 0) // jos yksikin n�ytt� listassa, hoidetaan hiiren klikkaus siell�
		{
...
// Jotta hiirell� voidaan manipuloida poikkileikkausn�yt�n pisteit�, pit�� seuraavat ehdot pit�� paikkaansa:
// 1. Poikkileikkausn�yt�n pit�� olla p��ll�.
// 2. Siin� ei saa olla trajektori -moodi p��ll�, koska silloin ei voi s��t�� aloituspisteit�.
// 3. Karttan�yt�n pit�� olla moodissa miss� pisteet n�ytet��n kartalla.
// 4. T�M� EI EN�� K�YT�SS�: Karttaruudun pit�� olla vasemmalla ylh��ll� (1. ruutu), koska vain siihen piirret��n kyseiset pisteet.
...
void NFmiStationViewHandler::LeftButtonUpControlPointModeActions(const NFmiPoint& thePlace, unsigned long theKey)
{
    ...
    // P�ivityst� vaatii vain t�m� karttan�ytt�, p��karttan�ytt� ja aikasarjaikkuna
...
void NFmiStationViewHandler::LeftButtonUpBrushToolActions()
{
    ...
    if(drawParam)
    {
        ...
        // P�ivitet��n pensseli vedon lopuksi vain karttan�ytt�j� (voisi p�ivitt�� viel� aikasarja ja taulukkon�yt�n, mutta optimoidaan)
	...
	if(IsCrossSectionViewStuffShownOnThisMapView())
	{
		...
		if(crossSectionSystem->CrossSectionMode() == NFmiCrossSectionSystem::k3Point)
		{
			...
			if(crossMode == NFmiCrossSectionSystem::kNormal || crossMode == NFmiCrossSectionSystem::kRoute)
				return true; // t�ll�in keski hiiren nappi pit�� j�tt�� poikkileikkausn�yt�n k�ytt��n!!
		}
	}
	return false;
}
...
bool NFmiStationViewHandler::MiddleButtonUp(const NFmiPoint & thePlace, unsigned long /* theKey */ )
{
	...
	{
        itsCtrlViewDocumentInterface->MapMouseDragPanMode(false);
        if(AllowCrossSectionPointManipulations() && IsCrossSectionSystemDisableingNormalMiddleMouseButtonUse()) // eli k�ytet��n sitten keski nappia poikkileikkausn�yt�lle
        {
...
bool NFmiStationViewHandler::HandleTimeBoxMouseWheel(const NFmiPoint& thePlace, unsigned long theKey, short theDelta)
{
	...
	if(mapViewDescTop)
	{
		...
		{
			// CTRL + SHIFT pohjassa tehd��n taustav�rin alpha kanavan s��t��
			auto origAlpha = mapViewDescTop->GetTimeBoxFillColorAlpha();
			auto newAlphaValue = origAlpha + (theDelta > 0 ? 0.05f : -0.05f);
			// Arvon asetus pit�� tehd� CombinedMapHandler:in kautta, jotta muutos menisi my�s WinRegistry:n ja sitten n�ytt�makroihin
		...
		if((theKey & kCtrlKey))
		{
			// CTRL pohjassa tehd��n kokokertoimen s��t��
...
bool NFmiStationViewHandler::MouseWheel(const NFmiPoint &thePlace, unsigned long theKey, short theDelta)
{
	if(itsViewList && GetFrame().IsInside(thePlace))
	{
		...
		else if(theKey & kShiftKey) // jos shift-nappi pohjassa muutetaan aktiivisen piirto-layerin hybrid-datojen leveli� yl�s/alas tai satel datan kanavaa
		{
			bool status = false;
			for(itsViewList->Reset(); itsViewList->Next(); ) // hybrid data muutos ensin
			{
...
// Katsoo onko kyseisen karttan�yt�n data hybrid ja nyt my�s painepinta dataa.
				...
				if(info->SizeLevels() > 1)
				{
					...
					if(levType != kFmiSoundingLevel)
					{
						if(theDelta < 0)
						{
							...
							else // py�r�ytet��n levelit ymp�ri jos ollaan tultu loppuun
							{
								info->FirstLevel();
								drawParam->Level(*info->Level());
								return true;
						...
						else
						{
							...
							else // py�r�ytet��n levelit ymp�ri jos ollaan tultu alkuun
							{
								info->LastLevel();
								drawParam->Level(*info->Level());
								return true;
					...
					{ // luotaus datalla on omat level s��d�t
...
bool NFmiStationViewHandler::MouseMove(const NFmiPoint &thePlace, unsigned long theKey)
{
    ...
    try
    {
        ...
        if(itsCtrlViewDocumentInterface->MiddleMouseButtonDown() && itsCtrlViewDocumentInterface->MouseCaptured())
            return MouseDragZooming(thePlace); // tehd��n zoomi laatikon piirtoa suoraan karttan�yt�lle
		else if(IsRangeMeterModeOn(false))
		{
			if(!(theKey & kCtrlKey))
			{
...
void NFmiStationViewHandler::DrawSelectedLocations(void)
{
    ...
    {   // Jos yksikin parametri valittuna n�yt�lle, piirret��n sen avulla valitut pisteet
...
NFmiStationView * NFmiStationViewHandler::CreateStationView(boost::shared_ptr<NFmiDrawParam> &theDrawParam)
{
	...
	if(theDrawParam)
	{
        ...
        { // Jos hila-piirto on symboli piirto ja asema-piirtona on valittu sateenolomuoto, piirret��n data sateen olomuoto n�yt�ll�, vaikka kyse olisi hiladatasta.
		...
		{ // eli tarvittaessa my�s asema data voidaan haluta piirt�� isoviivoina tai contoureina
			stationView = new NFmiIsoLineView(itsMapViewDescTopIndex, itsMapArea
											 ,itsToolBox
											 ,theDrawParam
											 ,param
...
void NFmiStationViewHandler::DrawMasksOnMap(NFmiToolBox* theGTB)
{
    if(itsCtrlViewDocumentInterface->Registry_ShowMasksOnMap(itsMapViewDescTopIndex))
	{
		...
		envi.EnableFill();
		envi.DisableFrame();
        EditedInfoMaskHandler editedInfoMaskHandler(info, NFmiMetEditorTypes::kFmiNoMask); // muutin activaatiomaskin k�yt�st� kaikkien piirtoon, koska muuten harvennetun datan piirto ei toimi oikein
...
void NFmiStationViewHandler::DrawMapViewRangeMeterData()
{
	...
	if(mapViewRangeMeter.ModeOn())
	{
		...
		if(toolTipRealRowIndex == CalcRealRowIndex() && toolTipColumnIndex == itsViewGridColumnNumber && toolTipMapViewDescTopIndex == itsMapViewDescTopIndex)
		{
			...
			// Nopeus laskuissa k�ytet��n mitatun et�isyyden kanssa karttan�yt�ss� olevaa aika-askelta matkaan k�ytett�v�n� aikana
...
NFmiRect NFmiStationViewHandler::SearchNameBoxLocation(const NFmiPoint &theRelativePoint, const NFmiRect &theRelativeBoundingBox, std::vector<NFmiRect> &theAutoCompletionRects, double theOneLineBoxHeight, FmiDirection &theMarkerConnectingPlace)
{
	...
	// Jos ei l�ytynyt paikkaa, laitetaan se siteen vain 1. sijoitus paikkaan
...
void NFmiStationViewHandler::DrawAutocompleteLocation(Gdiplus::Graphics *theGdiPlusGraphics, const NFmiACLocationInfo &theLocInfo, NFmiRect &theMarkerCircleBase)
{
	if(theLocInfo.itsLatlon != NFmiACLocationInfo::gMissingLatlon)
	{
		...
		if(markerConnectingPlace == kCenter || markerConnectingPlace == kTopCenter)
			fillColor.Alpha(fillColor.Alpha()*0.8f); // jos boxille ei l�ytynyt paikkaa, joka ei ole p��llekk�in, laitetaan enemm�n l�pin�kyvyytt�
...
// mutta tarkistetaan my�s tehd��nk� overlay-layerille teksti, jos se piirret��n heti kartan p��lle.
// 2. Jos beforeDataCase == false, ei tehd� ollenkaan background tapausta, vaan katsotaan ainoastaan tehd��nk� 
