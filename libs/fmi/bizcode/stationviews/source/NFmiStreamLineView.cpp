...
// Menev�tk� viivat 1 ja 2 ristiin? 
...
// Leikkaus koodit voivat j�tt�� my�s toiseen suuntaan menev�n polun 
...
bool NFmiStreamlineData::IsShorty(double theLengthLimitInKM) const
{
    ...
    else
    {
        ...
        {   // jos molemmat olivat yhden viivan pituisia
            ...
            {
                // lasketaan jos toinen viivoista on metreiss� todella lyhyt ja luokitellaan se t�ll�in lyhyeksi
...
// T�ll� estet��n matalapaine py�rteit� tai muuten tulemista itse��n l�helle.
...
// Palauttaa true, jos ehdokas oli 't�ysosuma', eli startIndex oli 0 tai 1. T�ll�in my�sk��n ei saa resetoida, 
// koska kandidaatti palautetaan suoraan k�ytt��n.
...
// Pisteiden pit�� olla sopivalla v�ljyydell� toisistaan, ett� streamlineista tulee visuallisesti miellytt�v�n n�k�isi�.
// Pisteiden v�ljyys riippuu sek� datan hilan tarkkuudesta ja karttan�yt�n (mahdollisen ali ruudun) 
...
// T�ll�in kasvatetaan aika-steppi� sit� enemm�n, mit� enemm�n WS l�hestyy 0:aa.
...
//        if((!theCalcParams.fUseRelativeParticleJumps && WS <= 1) || index > maxAllowedIndex)
        if(WS <= 0.1f || index > maxAllowedIndex)
            break; // alle 0.1 m/s tuulella on turha jatkaa tai jos ollaan jo junnattu polun kanssa �lytt�m�ll� m��r�ll� pisteit�
    }
...
static double CalcProximityLimit(const NFmiDataMatrix<NFmiPoint> &theStartingPoints)
{
    if(theStartingPoints.NX() > 2 && theStartingPoints.NY() > 2)
    {
        ...
        return 1; // en jaksa tehd� pienille matriiseille erikoistapausta, menk��t pieleen
}
...
// Jos zoomattu laatikko on tarpeeksi paljon pienempi kuin koko matriisi, palauta true (t�ll�in kutsuja laskee uuden pienemm�n matriisin), muuten false.
    ...
    { // Laske uusi pienempi matriisi, jossa mukana vain zoomatun alueen sis�osio
        ...
        for(size_t j=0; j < zoomedEliminationMatrix.NY(); j++)
        {
            for(size_t i=0; i < zoomedEliminationMatrix.NX(); i++)
            {
                ...
                zoomedEliminationMatrix[i][j].itsStartingPointIndex = NFmiPoint(static_cast<double>(i), static_cast<double>(j)); // indeksit pit�� my�s p�ivitt�� kohdalleen
            }
        }
...
void NFmiStreamLineView::DoSingleThreadCalculations(const StreamlineCalculationParameters &theCalcParams, boost::shared_ptr<NFmiFastQueryInfo> &theInfo, NFmiDataMatrix<NFmiStartPointEliminationData> &theStartingPointEliminatioMatrix, int timeStepInMinutes, std::vector<NFmiStreamlineData> &theLatlonPathsOut, double theProximityLimit, double theLengthLimitInKM)
{
    ...
    // Sitten loput keski pysty rivist� (alas ja yl�s p�in erikseen)
...
static float CalcWindArrowSizeFactor(float WS, float noScaleLimit)
{
    ...
    if(wsLimit2 == 0)
        return 1; // drawParamin oletus arvo t�lle s��d�lle on 0, jos t�m� siis 0, annetaan kaikille arvoilla kerroin 1
    float wsLimit1 = 0;
