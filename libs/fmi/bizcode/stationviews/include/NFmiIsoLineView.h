...
bool ColorTableInitialized(void); // laitoin esittelyn globaalista funktiosta t�h�n
...
struct LabelingInfo
{
	...
	int itsStrategy; // 0 = ei labeleita, 1 = simppeli, 2 = simppeli, muutta ei p��llekk�isi� labeleita (jos ei l�ydy paikkaa, ei labelia)
	float itsFontSize;
	NFmiColor itsFontColor;
	float itsFontOpacity; // arvo 0-1, 0 ei n�y ollenkaan ja 1 t�ysin l�pin�kym�t�n
	NFmiColor itsBoxFillColor;
	float itsBoxFillOpacity; // arvo 0-1, 0 ei n�y ollenkaan ja 1 t�ysin l�pin�kym�t�n
	NFmiColor itsBoxStrokeColor;
	float itsBoxStrokeOpacity; // arvo 0-1, 0 ei n�y ollenkaan ja 1 t�ysin l�pin�kym�t�n
...
protected:
	bool fDataOk; // false, jos pit�isi piirt�� tasainen kentt� (min = max)
	NFmiDataMatrix<float> itsIsolineValues; // t�h�n ker�t��n isoviivoja varten dataa, t�m�n avulla tehd��n my�s hatch-laskut ToolMaster-piirrossa

#ifndef UNIX
	CBitmap *itsTransparencyDrawBitmap;
	CBitmap *itsTransparencyDrawOldBitmap;
	CDC *itsTransparencyDrawDC;
	CDC *itsTransparencyDrawBackupDC; // t�h�n talletetaan v�liaikaisesti oikea DC, kun piirto suoritetaan v�liaikaisesti tranparentti DC:lle
	...
	// label dataa
/*/EL*/	std::vector<LabelBox> itsExistingLabels; // laitetaan kaikki k�ytetyt label paikat talteen tarkistuksia varten (taas globaali pikaviritys)
