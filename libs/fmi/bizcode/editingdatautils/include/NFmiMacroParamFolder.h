...
public:
	...
	checkedVector<std::string> GetDialogListStrings(bool getQ3Macros); // palauttaa macroparam-dialogin listaa vatern stringi-listan, jossa n‰kyy kunkin macroparamin nimi
	...
	const checkedVector<boost::shared_ptr<NFmiMacroParam> >& MacroParams(void) const {return itsMacroParams;}
private:
	bool RemoveFiles(const std::string &theName);
	checkedVector<std::string> GetNames(const std::string &thePath); // palauttaa listan macroparameja halutusta hakemistosta
	...
	checkedVector<boost::shared_ptr<NFmiMacroParam> > itsMacroParams;
