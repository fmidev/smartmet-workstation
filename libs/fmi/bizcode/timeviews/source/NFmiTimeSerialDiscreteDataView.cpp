...
bool NFmiTimeSerialDiscreteDataView::LeftButtonUp (const NFmiPoint & thePlace, unsigned long theKey)
{
	if(IsIn(thePlace))
	{
		...
		{
			// kuinka l�helt� pit�� aikaakselia klikata ennenkuin ohjelma suostuu 'l�yt�m��n' klikkauksen paikan
			...
			if(FindTimeIndex(thePlace.X(), proximityFactor, index))
			{
				...
				return ModifyFactorPointsSetValue(boost::math::iround(value), index) == true;	// Vain yksi tapa k�sitell� aikasarjaa. M.K.
			}
		}
	}
	return false;
...
{ // t�m� pit�� m��ritt�� uusien hiiri ominaisuuksien takia t�nne paaluttamaan falsea
	return false;
}
...
void NFmiTimeSerialDiscreteDataView::CreateModifyFactorScaleView(bool /* fSetScalesDirectlyWithLimits */, double /* theValue */)
{
	...
	NFmiStepScale scale(lowerLimit,upperLimit, 1./*,testScale.StepValue()*/);	// Askelv�li 1. M.K. 10.5.99
...
// HUOM! POIKKEAVATKO N�M� VIRTUAALI FUNKTIOT TOISISTAAN
{
