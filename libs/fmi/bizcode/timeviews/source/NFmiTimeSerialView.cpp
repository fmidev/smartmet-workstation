...
static boost::shared_ptr<NFmiFastQueryInfo> GetWantedData(CtrlViewDocumentInterface *theCtrlViewDocumentInterface, boost::shared_ptr<NFmiDrawParam> &theViewedDrawParam, const NFmiProducer &theWantedProducer, NFmiInfoData::Type theWantedType, const NFmiPoint* possibleLatlonPtr)
{
    ...
    if(!wantedInfo)
    {
		if(usedType == NFmiInfoData::kViewable) // jos ei l�ytynyt kViewable-tyypill�, kokeillaan viel� kModelHelpData
...
void NFmiTimeSerialView::Draw(NFmiToolBox* theToolBox)
{
	...
	// en keksinyt t�lle parempaa paikkaa, mutta nyt akseli ja maksimi muutos vastaavat toisiaan
		...
		DrawParamName();
		if(editedDataDrawed)
			DrawModifyingUnit();	// Piirt�� aikasarjaikkunaan muokkausakselin yksik�n.
...
{
    // Smartmet on kaatunut joskus mystisesti HESSAA symbolin kanssa, joten ei piirret�, kun on erikoisn�yt�st� kyse
...
void NFmiTimeSerialView::DrawModelDataLegend(const std::vector<NFmiColor> &theUsedColors, const std::vector<std::string> &theFoundProducerNames)
{
	...
	if(itsCtrlViewDocumentInterface->MetEditorOptionsData().ControlPointMode() == false) // ei piirret� CP legendaa ja model legendaa yht� aikaa
	{
		...
		if(info)
		{
			...
			// laitetaan tarvittaessa my�s help editor data legend� n�kyviin
			if(itsCtrlViewDocumentInterface->HelpEditorSystem().Use())
			{
                ...
                do
                {
                    if(theCheckedTimes.IsInside(theInfo->Time()))
				    {
					    if(theInfo->FloatValue() != kFloatMissing)
						    return theNearestLocations[i]; // heti kun jostain l�ytyy jotain muuta kuin puuttuvaa, palautetaan kyseinen paikka
				    }
                    else
                        break;
...
bool NFmiTimeSerialView::SetObsDataToNearestLocationWhereIsData(boost::shared_ptr<NFmiFastQueryInfo> &theObsInfo, const NFmiPoint &theLatlon, std::pair<int, double> &theLocationWithDataOut)
{
	...
	bool drawJustLegend = theLocationWithDataOut.first == -1; // jos ei l�ytynyt asemaa, jolle on dataa, otetaan vain l�hin asema ja piirret��n sen nimi
	if(drawJustLegend)
	{
	...
	if(obsInfo)
	{
		...
		if(::DataHasNeededParameters(obsInfo, itsDrawParam->Param().GetParamIdent(), itsCtrlViewDocumentInterface)) // parametrikin pit�� asettaa
		{
			...
			if(SetObsDataToNearestLocationWhereIsData(obsInfo, theLatlon, locationWithData))
			{
				...
				// displayCount+1 pit�� laittaa kertoimeksi, koska my�s havainto asema on piirretty
	...
	if(Info()->Param().GetParamIdent() == kFmiTemperature) // fraktiileja l�ytyy vain l�mp�tilalle!!!
	{
		...
		if(fraktiiliInfo)
		{
			...
			if(timesInView.FirstTime().GetYear() == timesInView.LastTime().GetYear()) // pit�� mietti se erikseen, miten hoidetaan kun aikaikkunan alku ja loppu ovat eri vuosilla (esim. vuoden vaihteessa)
			{
				...
				int fraktiiliDataYear = fraktiiliTimes.FirstTime().GetYear(); // fraktiili data on jollekin vuodelle tehty p�tk�, otetaan vuosi talteen, ett� voidaan rakentaa sopiva timebagi datan l�pik�ymiseen
				...
				if(status)
				{
					...
					if(fraktiiliInfo->NearestPoint(theLatlon)) // asetetaan kepadata l�himp��n pisteeseen (t�ss� kaupunkiin) piirtoa varten
					{
						...
						if(distance < gMaxDistanceToFractileStation) // fraktiili tiedot vain jos tarpeeksi l�helt� l�ytyi asema
						{
							NFmiPoint place(CalcParamTextPosition());
							NFmiPoint fontSize(CalcFontSize());
							fontSize *= NFmiPoint(0.9, 0.9); // pienennet��n fonttia hieman
							...
							// displayCount+2 pit�� laittaa kertoimeksi, koska my�s havainto asema on piirretty
...
// Jos kyseess� on Sea-level parametri (id = 60), katsotaan l�ytyyk� siihen liittyv��
...
// Jos k�yt�ss� jokin Hansenin meri fraktiili datan ProbLimit1-4 parametreista, piirret��n ne kaikki 
// parveen samoilla v�reill� kuin DrawSeaLevelProbLines. Huom! kyse ei ole samojen vakia viivojen piirrosta, 
// vaan eri limittien todenn�k�isyys arvoista.
...
// 2. Katso l�ytyyko kyseinen asema g_SeaLevelProbabilityStationData listasta.
// Jos l�ytyy, piirr� n�yt�n yli vaakasuoraan eri prob viivat halutuilla v�reill�.
...
void NFmiTimeSerialView::DrawSimpleDataInTimeSerial(const NFmiTimeBag &theDrawedTimes, boost::shared_ptr<NFmiFastQueryInfo> &theInfo, boost::shared_ptr<NFmiDrawParam>& theDrawParam, NFmiDrawingEnvironment &theEnvi, const NFmiPoint &theLatLonPoint, const NFmiPoint& theSinglePointSize, bool drawConnectingLines, int theTimeOffsetWhenDrawedInMinutes)
{
	...
	if(timeLimits.isValid())
	{
		...
		if(interpolateValues == false && (theInfo->NearestLocation(wantedLocation, gMaxDistanceToFractileStation) == false))
			return; // jos asema dataa ei l�ydy 500 km sis�lt� haluttua pistett�, ei k�ytet� sit�
...
{
    // Piirret��n se ainoa datasta l�ytynyt aika kuitenkin aikasarjaan
...
// DrawDataLine
//--------------------------------------------------------
// ottaa huomioon my�s, jos viiva on leikattava kun toinen arvoista menee akselin yli
		...
		else
		{
			...
			if(info->NextLocation())
			{
				...
				if(IsAnalyzeRelatedToolUsed())
					DrawAnalyzeToolEndTimeLine(); // piirret��n vain ensimm�isell� kerralla pystyviiva, joka kuvaa analyysity�kalun lopetusajan kohdan
			}
...
// Jos ollaan CP-moodissa ja ollaan my�s Obs-blender moodissa ja kyse on editoidusta datasta.
...
bool NFmiTimeSerialView::IsThisFirstEditedParamRow()
{
    ...
    if(drawParamList)
    {
        for(drawParamList->Reset(); drawParamList->Next(); )
        {
            ...
            // Katsotaan mist� kohtaa l�ytyy 1. editoitava parametri
            if(drawParam->DataType() == NFmiInfoData::kEditable)
            {
                ...
                break; // Jos l�ydetty t�m�n olion paikka listasta, mutta se ei ollut editable, lopetetaan
            }
        }
    }
    return false;
...
void NFmiTimeSerialView::DrawCPReferenceLines_DrawCpLocation(boost::shared_ptr<NFmiEditorControlPointManager> &cpManager, CpDrawingOptions &cpDrawingOptions, bool drawModificationLines)
{
    ...
    if(isActiveCp)
        DrawHelperDataLocationInTime(cpManager->LatLon()); // piirret��n aktiivisen CP-pisteen apu datat my�s ruudulle (=kepa, obs ja clim datat)
											   ...
											   ,unsigned long theKey)
{
	...
	if(IsIn(thePlace))
	{
        ...
        // kun monta alin�ytt�� yht�aikaa, pit�� ensimm�isell� klikkauksella asettaa kyseinen n�ytt� 'editointitilaan'
...

        {
            ...
            {
	            // kuinka l�helt� pit�� aika-akselia klikata ennenkuin ohjelma suostuu 'l�yt�m��n'
			    ...
			    else
			    {
				    ...
				    if(FindTimeIndex(thePlace.X(), proximityFactor, index))
				    {
					    ...
					    if(itsCtrlViewDocumentInterface->MetEditorOptionsData().ControlPointMode())
					    { 
							// 13.11.2002/Marko Muutos CP-ty�kalun k�yt�kseen siten, ett� piirret��n lopullista arvok�yr�� haluttuun pisteeseen.
...
static NFmiTimeBag GetScannedTimes(const NFmiTimeBag &theViewTimes)
{
	...
	float extensionLengthInMinutes = lengthInMinutes/2.f; // lis�t��n molempiin p�ihin ajallisesti puolet aikan�yt�n pituudesta
...
bool NFmiTimeSerialView::AutoAdjustValueScale(void)
{
	...
	// 4. jos min ja max ovat ei-puuttuvia, laske sopiva py�ristys alas min-arvosta ja sopiva py�ristys yl�s max-arvosta
...
bool NFmiTimeSerialView::MouseMove(const NFmiPoint &thePlace, unsigned long theKey)
{
	...
	if(IsIn(thePlace))
	{
		...
		if(itsDrawParam && itsDrawParam->IsParamEdited())
		{
            ...
            {
                // kuinka l�helt� pit�� aikaakselia klikata ennenkuin ohjelma suostuu 'l�yt�m��n'
	            // klikkauksen paikan (nyt lineaariselle laitetaan isommat 'reunat' hakua varten)
			    double proximityFactor = CalcMouseClickProximityFactor();
			    if(IsAnalyzeRelatedToolUsed())
			    {
...
{
	// LAITA T�M� UUSIKSI WCTR:n TAKIA!!!!!!!!
...
// Jos on, avataan timeserialView-popup, jolla voidaan manipuloida suoraan kyseist� n�ytt��.
										  ...
										  ,unsigned long theKey)
{
	if(IsIn(thePlace))
	{
		...
		return true; // jos n�yt�n sis�ll�, palauttaa true (ainakin v�liaikaisesti, muuten is�nt� avaa v��r�n popupin)
	}
	return false;
}
...
// piirt�� y�h�n 00-06 utc varjostuksen
// piirt�� my�s 12 utc:n kohdalle viivan
void NFmiTimeSerialView::DrawNightShades(void)
{
	...
	if(Info())
	{
		...
		startTime.PreviousMetTime(); // laitetaan aloitus varmasti ennen n�yt�n alkua
		
		NFmiMetTime endTime = timesInView.LastTime();
		endTime.SetTimeStep(usedTimeStep);
		endTime.NextMetTime(); // laitetaan loppu aika varmasti ohi n�yt�n lopun
...
double NFmiTimeSerialView::Value2AxisPosition(float theValue)
{
	if(itsValueAxis)
	{
		...
		double finalValue = rect.Bottom() - value * rect.Height(); // HUOMM!! toimii vain jos value on y-akselilla ja alkaa alhaalta yl�s!!!
		return finalValue;
	}
...
void NFmiTimeSerialView::ChangeTimeSeriesValues(void)
{
	if(itsCtrlViewDocumentInterface->MetEditorOptionsData().ControlPointMode() || IsAnalyzeRelatedToolUsed() || IsModifyFactorValuesNonZero())
	{
		...
		if(itsCtrlViewDocumentInterface->MetEditorOptionsData().ControlPointMode()) // kontrollipiste ty�kalulla muutokset tehd��n aina kaikkiin pisteisiin, eli = nomask
...
// Py�rist�� luvun l�himp��n 1, 2 tai 5 arvoon, tarkoittaen ett� theStep:in suuruus luokasta riippuen
// luku py�ristet��n esim. 1:een, 2:een tai 5:een (jos originaali theStep oli n. 0.8 - 8). 
// Py�ristet��n 0.1:een, 0.2:een tai 0.5:een (jos originaali theStep oli n. 0.08 - 0.8). 
// Py�ristet��n 10:een, 20:een tai 50:een (jos originaali theStep oli n. 8 - 80).  jne.
...
void NFmiTimeSerialView::DrawAnalyzeToolRelatedChangeLineFinal(bool useObservationData, float usedAnalyzeValue, const NFmiPoint &theLatLonPoint, NFmiDrawingEnvironment &envi, boost::shared_ptr<NFmiFastQueryInfo> &editedInfo, boost::shared_ptr<NFmiFastQueryInfo> &usedToolInfo, const NFmiMetTime &startTime, std::vector<std::string> &messages)
{
    ...
    {
        // Jos hila-analyysidatan aikaa ei l�ydy editoitavasta datasta, ei kannata jatkaa
...
// HUOM!!! Piirt�� my�s analyysidatan ruutuun!!!!!
...
// Piirt�� tietyt analyysity�kaluun liittyv�t lyhyet sanomat keskelle (ja keskitetysti) n�ytt�rivi� otsikko-osioon.
...
static std::vector<boost::shared_ptr<NFmiFastQueryInfo>> GetInfosWithWantedParam(std::vector<boost::shared_ptr<NFmiFastQueryInfo>> &infoVectorIn, FmiParameterName wantedParamId)
{
    ...
    {
        // Datassa ei saa olla my�sk��n ship/buoy dataa (= moving station data)
...
// funktio piirt�� annetun arvojoukon annettuina aikoina n�ytt��n halutuilla piirtooptioilla
...
bool NFmiTimeSerialView::MouseWheel(const NFmiPoint &thePlace, unsigned long theKey, short theDelta)
{
	...
	if(IsIn(thePlace))
	{
		...
		else if(theKey & kShiftKey) // jos shift-nappi pohjassa muutetaan hybrid/pressure -datojen leveli� yl�s/alas
		{
...
std::string NFmiTimeSerialView::GetSideParametersToolTipText(const NFmiPoint& theLatlon, const NFmiMetTime& theTime, bool addNewlineAtStart)
{
	...
	if(viewRowSideParameters && viewRowSideParameters->NumberOfItems() > 0)
	{
		...
		for(viewRowSideParameters->Reset(); viewRowSideParameters->Next(); sideParameterColorIndex++)
		{
			// Joskus esim. side-parameterin lis�ys ei laukaise aikasarjaikkunan piirtoa, ja t�ll�in jos
			// tooltip piirto tulee ensin, ei listasta l�ydy kaikkia parametri nimi�
...
std::string NFmiTimeSerialView::GetSeaLevelPlumeDataToolTipText(boost::shared_ptr<NFmiFastQueryInfo>& theViewedInfo, const NFmiPoint& theLatlon, const NFmiMetTime& theTime, const NFmiColor& theColor)
{
	...
	if(seaLevelPlumeData)
	{
		...
		{
			const auto& fractileParams = seaLevelPlumeData->fractileParams();
			// Jos l�ytyy 1. parametreista ja l�hin asema piste on g_SeaLevelProbabilityMaxSearchRangeInMetres rajan sis�ll�
...
std::string NFmiTimeSerialView::ComposeToolTipText(const NFmiPoint& theRelativePoint)
{
	...
	if(editedInfo && viewedInfo)
	{
        ...
        // T�m� varmistaa ett� my�s erikoistapaus eli synop-data menee oikein
...
std::string NFmiTimeSerialView::MakeTimeSerialCsvString()
{
	...
	// 1. Tarkista ett� n�ytt�rivilt� tulee sama m��r� aikalistoja, parametrilistoja ja parametrien nimi�
...
static std::string MakeCsvParameterNameSectionString(boost::shared_ptr<NFmiFastQueryInfo>& theInfo, unsigned long wantedParamId)
{
	...
	{
		// jos haluttu parametri l�ytyi datasta, palautetaan sen nimi
