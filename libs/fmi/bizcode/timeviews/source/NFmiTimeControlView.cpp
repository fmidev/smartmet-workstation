...
void NFmiTimeControlView::Initialize(bool theShowSelectedTimes, bool theUseActiveMapTime) 
{
	...
	if(itsCtrlViewDocumentInterface->DefaultEditedDrawParam())
	{
		...
		NFmiDrawingEnvironment theEnvironment; // t�st� envist� en oikein tied�, pit�isik� k�ytt�� yleist� vai ei??
	...
	try
	{
		...
		if(fHasData)
		{
			...
			if(fDrawTimeFilterTimesInNormalDraw)
				DrawTimeFilterTimes(); // t�m� piirret��n DrawOverBitmapThings-metodsissa kaiken p��lle esim. karttan�yt�n yhteydess�
...
// Aikas��timen full-range -nappula piirret��n vain karttan�ytt�jen aikakontrolli-ikkunoihin, mink� AllowAnimationBoxShown -metodi kertoo.
...
void NFmiTimeControlView::ChangeResolution(bool fLeftClicked, bool ctrlKeyDown)
{
    ...
    {
        // P��karttan�yt�n aika-stepin vaihto vaikuttaa my�s warning ja sea-icing ikkunoiden tiloihin
...
void NFmiTimeControlView::DrawTimeFilterTimes()
{
	if(IsTimeFiltersDrawn())
	{
		...
		// itsGdiPlusGraphics-instanssin k�ytt� kaatoi omituisesti ohjelman. T�m� on varoittava esimerkki
		// siit�, ett� ehk� kannattaa k�ytt�� lokaaleja Graphics-olioita. En tied� miten raskas operaatio
		// on luoda aina joka k�ytt��n oma instanssi piirto hommia varten.
		// Luulen ett� ongelma on siin� ett� SmartMet luo jatkuvasti n�ytt�luokkia uudelleen ja uudelleen
		// t�st� ehk� seuraa se ett� joku piirto metodi saattaa aiheuttaa luokan uudelleen luonnin kesken 
		// kaiken, jolloin my�s GDI+ otus pit�isi luoda uudestaan. Korjaaminen voisi ehk� tapahtua siten 
		// ett� luokkien luonti ja muu toiminta pit�� laittaa uusiksi (= iso ja vaivaloinen ty�).
...
bool NFmiTimeControlView::AnimationButtonReleased(const NFmiPoint & thePlace,unsigned long theKey)
{
	if(IsAnimationButtonPressed() || IsInsideAnyAnimationButton(thePlace))
	{
		...
		if(fAnimationCloseButtonPressed && CalcAnimationCloseButtonRect().IsInside(thePlace))
		{
			...
			animData.AnimationOn(false); // suljetaan my�s animaatio samalla (ett� jos animointi ikkuna avataan my�hemmin, ei animaatio mene heti p��lle)
		}
		...
		else if(fAnimationLockModeButtonPressed || CalcAnimationLockModeButtonRect().IsInside(thePlace))
		{
			...
			if(animData.LockMode() != NFmiAnimationData::kNoLock)
				itsCtrlViewDocumentInterface->CheckAnimationLockedModeTimeBags(itsMapViewDescTopIndex, false); // t�m� p�ivitt�� timebagit kohdalleen ja likaa mahd. n�ytt� ruutuja cachesta
		}
...
bool NFmiTimeControlView::LeftButtonUp(const NFmiPoint & thePlace,unsigned long theKey)
{
	...
	else
	{
		...
		if(fMouseMovedWhenCaptured)
		{
			...
			{ // lopetetaan aikakontrollin raahaus ja s��det��n sen alku- ja loppup��t sopiviksi
				...
				// jos raahaus on ollut mit�t�n, siirret��n valittua aikaa, eik� tehd� raahausta
			...
			else if(fMouseCapturedAnimationBox)
			{
				...
				{
					NFmiAnimationData &animData = itsCtrlViewDocumentInterface->AnimationData(itsMapViewDescTopIndex);
					animData.ShowTimebag(animData.Times()); // asetetaan lopuksi oikeat animaatio ajat my�s show-times ajoiksi (t�m� siis 'loksauttaa' animaatio ajat timestepin mukaisesti)
				}
				status = true;
			}
        }
    ...
    if(status)
    {
        // Jos status on laitettu true:ksi, varmistetaan ett� tehd��n ainakin kyseisen karttan�yt�n p�ivitys. 
        // Optimointia, ettei kaikkia n�ytt�j� p�ivitet�. Jos halutaan ett� tehd��n laajempia n�ytt�jen p�ivityksi�, pit�� eri kohdissa laittaa enemm�n n�ytt�jen p�ivityslippuja p��lle
        ApplicationInterface::GetApplicationInterfaceImplementation()->ApplyUpdatedViewsFlag(GetWantedMapViewIdFlag(itsMapViewDescTopIndex));
    }
...
{
	// eli jos ollaan aika-kontrollissa, mutta ei osuttu alku/loppu aika s��t� kahvoihin tai resoluutio boxiin
...
bool NFmiTimeControlView::LeftButtonDown (const NFmiPoint & thePlace, unsigned long theKey)
{
	...
	if(IsIn(thePlace))
	{
		...
		else
		{
			...
			{ // aletaan raahaamaan p�� karttan�yt�n ensimm�isen kartan aika-laatikkoa (se vihre� laatikko aikakontrolli ikkunassa)
...
{ // HUOM!! ei pid� tutkia onko hiiri ikkunan sis�ll� vaan onko hiiri kaapattu!
    ...
    if(itsCtrlViewDocumentInterface->MouseCaptured())
    {
        ...
        { // t�m� blokki on editoinntiin liittyvien alku/loppu aikajanan s��t��
