...
void NFmiTrajectoryView::ChangePAxisValue(FmiDirection theMouseButton, bool upperPart)
{
	...
	{
		// **** MSVC 7.1 BUGI ************
		// Ilman tuota stringi virityst� ei Trajektori-n�yt�n paineasteikon
		// alap��n s��t� vasemmalla hiiren klikkauksella ei toimi release-versiona.
...
{
	double step = 25;
	if(theKey & kCtrlKey) // tehd��n yhden mb:n tarkkuudella py�ristys
		step = 1;
...
NFmiRect NFmiTrajectoryView::CalcPressureScaleRect(void)
{
	...
	double emptySpaceTop = itsToolBox->SY(3); // v�h�n tilaa j�tet��n yl�s
	double emptySpaceBottom = itsToolBox->SY(54); // pit�� olla tilaa aika ikkunalle
...
// Pit�� laskea vasta kun itsDataRect on laskettu!!
NFmiRect NFmiTrajectoryView::CalcTimeControlViewRect(void)
{
	...
	double spaceX = itsToolBox->SX(usedStretchFactor); // ikkunaa pit�� levent�� hieman datarectiin n�hden, koska itse aika-akseli ei men reunoille asti
	double spaceY = itsToolBox->SY(4); // pieni v�li datalaatikon ja aikaikkunan v�lill�
...
void NFmiTrajectoryView::DrawPressureScale(void)
{
	...
	for (auto it = pValues.begin(); it != endIt;  ++it)
	{
		...
		if(pressure <= itsMaxPressure && pressure >= itsMinPressure)
		{
			...
			NFmiPoint p2(pressureOnBorder ? itsDataRect.Left() : itsDataRect.Right(), y); // jos viiva on datalaatikon ala/yl� rajalla, piirret��n vain tick mark, muuten koko ruudun ylitt�v� apuviiva
			NFmiLine line1(p1, p2, 0, &envi);
			itsToolBox->Convert(&line1);
