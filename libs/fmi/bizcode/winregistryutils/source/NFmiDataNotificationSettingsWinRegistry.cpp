...
#ifdef _MSC_VER
#pragma warning( disable : 4800 ) // t�m� est�� pitk�n varoituksen joka tulee kun k�ytet��n CachedRegBool -> registry_int<bool> -tyyppist� dataa, siell� operator T -metodissa DWORD muuttuu bool:iksi
#endif
...
static void FixVersionNumber(boost::shared_ptr<CachedRegString> &cachedValue, const std::string &fullAppVer)
{
    try
    {
        ...
        boost::regex re("[\\d+\\.]+[\\d+]{1}"); // versio numero, jossa on v�h. x.x, miss� x voi sis�lt�� useita numeroita esim. 2.12
...
bool NFmiDataNotificationSettingsWinRegistry::Init(const std::string &baseRegistryPath, const std::string &fullAppVer)
{
    ...
    // Kahdelle seuraavalle arvolle pit�� tehd� viel� versio numeron korvaus:
    // Jos arvosta l�ytyy versio numero joka on muotoa x.x.x.x tai "${SmartMetVersion}", korvaa se nykyisell� versionumerolla
