...
// T�m� asettaa kaikki talletettavat asiat Windows rekistereihin.
...
void NFmiCrossSectionSystem::CalcMinorPoints(const boost::shared_ptr<NFmiArea> &theArea)
{
    ...
    itsCtrlViewDocumentInterface->UpdateCrossSectionMacroParamDataSize(); // Pit�� p�ivitt�� macroParam laskuja varten poikkileikkaus hilakoot
}

// Jos annettu data (eli t�m� on juuri ladattu metkun editoriin) on jonkinlaista level-dataa
// niin p�ivityst� tarvitaan. Mutta tyypin pit�� olla editable, viewable tai hybrid, koska mm.
// havainnoissa on level-dataa ja p�ivityst� ei haluta
...
// aktivoi l�himm�n ali-pisteen, mutta jos kyseinen piste jo aktivoitu, deaktivoi se
void NFmiCrossSectionSystem::ActivateNearestMinorPoint(const NFmiPoint &thePlace)
{
	...
	if(minDist < 1000 * 200) // minimi et�isyyden pit�� olla alle halutun rajan (nyt 200km), ett� jokin piste aktivoidaan, muuten tehd��n deaktivointi
	{
...
// Paluuarvo kertoo tarvitaanko n�yt�n p�ivityst�
...
// paluu arvo kertoo tarvitaanko n�yt�n p�ivityst�
bool NFmiCrossSectionSystem::RowCount(int newValue)
{
	...
	// jos rivien m��r� vaikuttaa nyt alku rivin indeksiin, se tarkistetaan t�ss�
	if(itsStartRowIndex > itsMaxViewRowSize - itsRowCount + 1)
		itsStartRowIndex = itsMaxViewRowSize - itsRowCount + 1;
...
// kun poikkileikkaus n�yt�ss� on nyt 3 eri moodia, pit�� se p��tell� jotenkin kahden booleanin avulla
...
// v�h�n mousewheel k�sittely� t�nne, koska t�t� kutsutaan parista eri view-luokasta
bool NFmiCrossSectionSystem::MouseWheel(const NFmiPoint & /* thePlace */ , unsigned long theKey, short theDelta)
{
	...
	{ // vertikaali pisteiden s��t�
		if(theDelta > 0)
			VerticalPointCount(VerticalPointCount()+1); // lis�t��n yksi vert. piste
		else
			VerticalPointCount(VerticalPointCount()-1);  // v�hennet��n yksi vert. piste
	}
	else if(theKey & CtrlViewUtils::kShiftKey)
	{ // vaakapisteiden m��r�n s��t�
		// 2-piste moodissa yksi piste lis�� ja 3-piste moodissa 2 lis�� (tai v�hemm�n)
...
static void SetCrossSectionViewPressureLevels(NFmiCrossSectionSystem &theCrossSectionSystem, const std::string &thePressureLevelsStr)
{
	...
	int maxSize = static_cast<int>(pressureLevels.size()/2); // n�in moneen riviin limit l�ytyy maksimissaan
...
void NFmiCrossSectionSystem::Write(std::ostream& os) const
{
	...
	// *** 1. uusi tallennus itsCrossSectionTimeBag talletetaan aikasiirtoina ja siit� tehd��n yksi aika-stringi ***
	...
	// *** 2. uusi tallennus jokaisen rivin paineasteikon alku ja loppu p��t talletetaan ja siit� tehd��n yksi stringi ***
...
{ // toivottavasti olet poistanut kommentit luettavasta streamista!!
	...
	NFmiMetTime usedViewMacroTime = NFmiDataStoringHelpers::GetUsedViewMacroTime();
	NFmiMetTime dummyTime; // erilliset itsRouteStartTime ja itsRouteEndTime on poistettu k�yt�st�, luetaan vain dummy arvot pois
...
void NFmiCrossSectionSystem::Init(const NFmiCrossSectionSystem &theData, bool disableWindowManipulations)
{
	...
	if(theData.itsCrossSectionTimeControlTimeBag.GetSize() > 1) // vanhoissa makroissa timebagin koko voi olla 1, sit� ei sijoiteta k�ytt��n
...
// Ottaa 1. n�kyv�n rivin akseli arvot ja laittaa ne default-akseli arvoiksi.
// Talettaa samalla my�s settings arvot ja tekee NFmiSettings -save:n levylle.
void NFmiCrossSectionSystem::SaveCrossSectionDefaultAxisValues(void)
{
	// itsStartRowIndex - 1, koska itsStartRowIndex alkaa 1:st� ja vektorin indeksit 0:sta
	itsAxisValuesDefaultWinReg = itsExtraRowInfos[itsStartRowIndex - 1];
}

// Ottaa 1. n�kyv�n rivin akseli arvot ja laittaa ne special-akseli arvoiksi.
// Talettaa samalla my�s settings arvot ja tekee NFmiSettings -save:n levylle.
