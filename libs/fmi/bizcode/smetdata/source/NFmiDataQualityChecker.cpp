...
void NFmiDataQualityChecker::CheckData(NFmiThreadCallBacks *theThreadCallBacks)
{
	...
	if(itsDataPtr.get())
	{
		...
		{
            // Ensin tarkistetaan erikois tilanne eli T>=Td tarkistus, datassa pit�� t�ll�in my�s olla T ja Td parametrit
			...
			{ // pit�� laittaa t�m�n parametrin checkeihin joka aikaan ett� chekkaus-data on k�yty l�pi, mutta parametria ei vain l�ytynyt (se on jo oletus arvona)
				::CheckIfStopped(theThreadCallBacks);
				for(size_t j=0; j<itsValueCheckMatrix.NY(); j++) // ajat l�pi
				{
					::DoStepIt(theThreadCallBacks);
