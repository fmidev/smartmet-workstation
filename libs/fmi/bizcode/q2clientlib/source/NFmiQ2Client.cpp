...
static std::istream& operator>>(std::istream& s, NFmiDataMatrix<float>& m)
{
	...
	if(columns == 0 || rows == 0) // luultavasti istreamiss� olevassa stringiss� oli virheilmoitus tai jotain roskaa ja nyt heitet��n poikkeus
	...
	for (sz_type j = 0; j < rows; j++)
	{
		for (sz_type i = 0; i < columns; i++)
		{
			if(s.peek() == ',')
			{
				m[i][j] = kFloatMissing; // t�m� on mahd. turha sijoitus, mutta varmuuden vuoksi
				s >> tmp3; // q3-server palauttaa puuttuvan arvon tyhj�n�, eli seuraava pilkku on jo kohdalla ja luetaan vain se
			}
			else
				s >> m[i][j] >> tmp3;
		}
...
static bool MakeHttpCommand(const std::string& theServerStr,
	const std::string& thePortStr,
	const std::string& theParamStr,
	std::string& theHeaderStr,
	std::string& theResponseStr,
	std::string& theErrorStr,
	bool fDoGet,
	bool useTls /* <- new flag; or infer from port */)
{
	theHeaderStr.clear();
	theResponseStr.clear();
	theErrorStr.clear();
	...
	try
	{
		...
		// ----- Resolve host:port -----
		std::string service = thePortStr.empty()
			? (useTls ? "443" : "80")
			: thePortStr;

		tcp::resolver resolver(ioc);
		auto results = resolver.resolve(theServerStr, service);

		// ----- Build the HTTP request buffer (same for HTTP/HTTPS) -----
		asio::streambuf request;
		std::ostream request_stream(&request);

		request_stream << (fDoGet ? "GET " : "POST ") << theParamStr << " HTTP/1.1\r\n";
		request_stream << "Host: " << theServerStr;
		if(!thePortStr.empty()) request_stream << ":" << thePortStr;
		request_stream << "\r\n";
		request_stream << "Accept: */*\r\n";
		request_stream << "User-Agent: asio-demo/1.0\r\n";
		request_stream << "Connection: close\r\n\r\n";

		// Common response buffer/vars
		...
		if(!useTls)
		{
			...
			for(auto it = results.begin(); ec && it != results.end(); ++it)
			{
				socket.close();
			...
			unsigned status_code = 0; response_stream >> status_code;
			...
			// Body (already-read + until EOF)
			std::stringstream sstream;
			if(response.size() > 0) sstream << &response;
			...
			theResponseStr = sstream.str();
			if(status_code != 200) { theErrorStr.swap(theResponseStr); return false; }
		}
		else
		{
			// ================== HTTPS (TLS) ==================
...
//			ctx.set_default_verify_paths(); // MARKO if certificate verification used later, take this line out of comment!!!!
			...
			// SNI (many servers require this)
			if(!SSL_set_tlsext_host_name(stream.native_handle(), theServerStr.c_str()))
			...
			for(auto it = results.begin(); ec && it != results.end(); ++it)
			{
				...
				stream.lowest_layer().connect(*it, ec);
			}
			if(ec) throw boost::system::system_error(ec);
...
//			stream.set_verify_mode(ssl::verify_peer); // MARKO if certificate verification used later, take this line out of comment!!!!
			...
			// Graceful TLS shutdown (ignore errors; servers often just close)
	...
	{
		theErrorStr = std::string("Error in MakeHttpCommand:\n") + e.what();
		return false;
	}
...
// Halutaan etsi� annetusta stringista annetusta l�ht�pisteest� maksimissaan tarkastaen halutun m��r�n merrkej� ja etsien haluttua merkki�.
...
static void GetMatrixFromString3(const std::string &theResponseStr, NFmiDataMatrix<float> &theDataOut, std::string &theExtraInfoStr)
{
	...
	if(pos1 == std::string::npos) // ei l�ytynyt ;-merkki�, pakko olla joku virhe
		throw std::runtime_error(std::string("Error message from server:\n") + theResponseStr);
	else
	{ // tutkitaan l�ytyyk� 1. ;-merkin j�lkeen numero,numero; yhdistelm�, jos l�ytyy, on alussa extra infoa. Jos ei l�ydy, ei kannata
		// k�yd� koko j�ljell� olevaa stringi� l�pi etsim�ss� toista mahd. ;-merkki�
		// K�yt�nn�ss� etsin seuraavat max 40 merkki�, l�ytyyk� toista;-merkki�
		std::string::size_type pos2 = ::FindSubStringWithInGivenChars(theResponseStr, pos1 + 1, ';', 40);
		
		if(pos2 != std::string::npos)
		{ // nyt l�ytyi toinenkin ;-merkki, mik� merkitsee sit� ett� on olemassa extrainfo-osuus.
			// Oletus: ei saa olla kuin kaksi ;-merkki� vastauksessa!!!! Haluan vain est�� ett� ei tarvitse k�yd� koko stringi� l�pi aina, koska stringi voi olla pitk�
			theExtraInfoStr = std::string(theResponseStr.begin(), theResponseStr.begin()+pos1);
		}
		else
			pos1 = static_cast<std::string::size_type>(-1); // pit�� s��t�� pos1:n arvo kun ei ollut extrainfo osiota
	}
...
static void GetMatrixFromBinaryData3(const std::string &theBinaryResponseStr, NFmiDataMatrix<float> &theDataOut, std::string &theExtraInfoStr)
{
	...
	if(intExtraSize)
	{ // l�ytyi tavaraa, luetaan ne sstringiin talteen
		...
		extraInfoChars += 4; // siirret��n ensin osoitin oikeaan paikkaan
		theExtraInfoStr = std::string(extraInfoChars, extraInfoChars+intExtraSize);
	}

	char* dataChars = const_cast<char*>(theBinaryResponseStr.c_str() + 4 + intExtraSize); // matriisi data l�ytyy integer luvun ja extrainfon j�lkeen
...
*/
		// t�ss� tarkistetaan, l�ytyyk� error sanaa stringin alusta, jolloin tiedet��n ett� 
		// paluuna tuli virheviesti.
		if(theResponseStr.size() > 5)
		{
			std::string firstLetters;
			firstLetters.resize(5); // tehd��n tilaa mahdolliselle "error" sanalle
			...
			// q3-serveri asettaa http-header flagin p��lle virheen merkiksi.
...
static bool SplitUrlStr(const std::string &theUrlStrIn, std::string &theServerAddressStrOut, std::string &theParamStrOut, std::string &thePortStrOut, std::string scheme = std::string())
{
	...
	std::string::size_type currentPos = tmpUrlStr.find("//"); // etsit��n, l�ytyyk� tupla kenoa
	if(currentPos != std::string::npos) // jos l�ytyi, poistetaan alku osa koodista 
		tmpUrlStr = std::string(tmpUrlStr.begin()+currentPos+2, tmpUrlStr.end());

	std::string::size_type pos1 = tmpUrlStr.find("/"); // etsit��n, 1. keno
	std::string::size_type pos2 = tmpUrlStr.find(":"); // etsit��n, 1. :-merkki
...
{
	// alustetaan viel� parametri stringi� annetuilla optioilla
...
*/
	...
	if(logDataRequest)
	{
		...
		logMsg += NFmiStringTools::UrlDecode(finalUrlStr); // Puretaan lokitukseen url encodaus, jotta haku n�ytt�� ihmism�iselt�
	...
	std::string portStr;
	std::string scheme;
	if(::SplitUrlStr(finalUrlStr, serverAddressStr, paramsStr, portStr, scheme))
	{
		bool useTls = (scheme == "https");
		std::string headerStr;
		std::string responseStr;
		std::string errorStr;
		if(::MakeHttpCommand(serverAddressStr, portStr, paramsStr, headerStr, responseStr, errorStr, true, useTls))
...
void NFmiQ2Client::MakeHTTPRequest(const std::string &theUrlStr, std::string &theResponseStrOut, bool fDoGet)
{
	...
	if(::SplitUrlStr(theUrlStr, serverAddressStr, paramsStr, portStr, scheme))
	{
		...
		std::string headerStr;
		std::string errorStr;
		::MakeHttpCommand(serverAddressStr, portStr, paramsStr, headerStr, theResponseStrOut, errorStr, fDoGet, useTls);
	}
	else
