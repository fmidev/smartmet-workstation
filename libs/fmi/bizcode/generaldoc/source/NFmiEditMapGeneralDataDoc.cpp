...
#include "psapi.h" // ei l�ytynyt toshibasta, pit�� korjata tilanne!!!
#endif
...
{
	theOwnerInfo = std::shared_ptr<NFmiFastQueryInfo>();
}
...
// Tieto havainto tuottajista, mik� on viimeisin ajanhetki, milt� l�ytyy dataa.
...
// tein pikaviritys funktion ett� saisin t�m�n timebagin kaivettua dokumentin sy�vereist�
const NFmiTimeBag& MaximalCoverageTimeBag(void)
{
	return GetUsedDataLoadingInfo().MaximalCoverageTimeBag();
}

// tein pikaviritys funktion ett� saisin t�m�n editormodedatan kaivettua dokumentin sy�vereist�
...
void InitMacroParamDataCache()
{
    ...
    try
    {
        // Alustetaan macroParam cache kaikkien karttan�ytt�jen indekseill� (0-2) ja viel� poikkileikkausn�yt�n indeksill�
...
void InitMultiProcessPoolOptions(void)
{
	...
	try
	{
        ...
        // alustetaan t�ss� my�s itsMultiProcessClientData:n MultiProcessPoolOptions -osio
...
void InitApplicationWinRegistry(std::map<std::string, std::string> &mapViewsPositionMap, std::map<std::string, std::string> &otherViewsPositionPosMap)
{
	...
	try
	{
        ...
        // 3 = 3 eri karttan�ytt��, ei viel�k��n miss��n asetusta t�lle, koska p��karttan�ytt� poikkeaa kahdesta apukarttan�yt�st�
...
void InitCombinedMapHandler()
{
	...
	{
		// Jos wms systeemi� ei oteta k�ytt��n, pit�� lopettaa timer joka odottaa 1. data p�ivityst� joka 3. sekunti
...
// Esim. 1 Normaali tilanne, nimess� ei ole interpolaatiotietoa, "L�mp�tila" -> "L�mp�tila",kLinearly  (pari)
// Esim. 2 Nimess� on interpolaatiotietoa (lopussa kaarisuluissa luku, joka my�s j�tet��n nimest� pois), "L�mp�tila{2}" -> "L�mp�tila",kNearestPoint  (pari)
std::pair<std::string, FmiInterpolationMethod> GetEditedParamFinalNameAndInterpolationMethod(const std::string &theParamName)
{
    ...
    {
        if(theParamName[theParamName.size() - 1] != '}') // nimen pit�� my�s loppua kaarisulkuun, ennen kuin siin� olevat osiot hyv�ksyt��n interpolaatio menetelm�ksi
            return std::make_pair(theParamName, kLinearly);
        else
        {
...
NFmiDataIdent MakeEditedParam(FmiParameterName theParId, const std::string &theParamName, const NFmiProducer &theDefaultProducer)
{
	...
	else
	{
		...
		std::shared_ptr<NFmiDrawParam> drawParam = itsSmartInfoOrganizer->CreateDrawParam(tmpDataIdent, 0, NFmiInfoData::kEditable);
		if(drawParam)
		{
...
// rakennetaan ladattaessa dataa. Jos molempia ei l�ydy, ei kyseist� hila m��rityst� oteta
// k�ytt��n ja t�ll�in hilam��ritykset otetaan prim��ri datasta (mik� on prioriteetti 1
...
void InitDrawDifferenceDrawParam(void)
{
	...
	if(itsSmartInfoOrganizer)
	{
		...
		// Laitoin alustamaan my�s valittujen hilapisteiden uuden visualisointi piirron
...
//			itsSelectedGridPointDrawParam->DrawIsoLineHatchWithBorders1(true); // hatch with borders ei my�sk��n onnistu jostain mystisest� syyst�
...
// Laskee halutun tiedoston nimen sekunneissa. Jos paluu arvo on -99, ei tiedostoa l�ytynyt.
...
// Metkuneditori pit�� tietoa tilastaan ty�hakemistossa olevalla tiedostolla, johon
...
// Tarkistaa samalla kuitenkin my�s, onko samasta editorista useita versioita (prosesseja) k�ynniss�.
// 'Sama' tarkoittaa, ett� onko olemassa muuta samannimist� prosessia, joka on k�ynnistetty
// samasta paikasta (polusta). Jos l�ytyy muita samanaikaisia 'samoja' prosesseja, ei tee h�lytyst�,
// jos k�ynnistyess� l�ytyy tarkistus tiedostosta "running" sana.
void CheckRunningStatusAtStartup(const NFmiString& theCheckedDirectory)
{
	...
	if(ifile)
	{
		...
		if(testStr == string("running")) // status v��r� (onko ohjelma kaatunut edellisen kerran)
		{
			...
			// T�H�N TULEE SITTEN H�LYTYS VIESTIN L�HETYS!!!!!!
...
bool StoreOptionsData(void)
{
	try
	{
    	...
    	itsHelpDataInfoSystem.StoreToSettings(); // n�it� asetuksia muutetaan my�s options-dialogissa (HUOM! pit�� tallettaa normaalit datat, ei CaseStudy:yn liittyvi� juttuja)
		SaveSettingsToConfFile(); // n�it� asetuksia muutetaan my�s options-dialogissa
...
{
	CombinedMapHandlerInterface::doVerboseFunctionStartingLogReporting(__FUNCTION__);
	std::shared_ptr<NFmiAreaMaskList> paramMaskList(new NFmiAreaMaskList());
...
// HUOM! t�t� k�ytet��n my�s s��t�m��n  aikasarjaikkunan aikabagia, koska t�m� on siis tarkoitus kutsua vain kerran AddData-metodista!!!!
...
void NormalizeGridDataArea(NFmiQueryData* theData)
{
	...
	// t�m� s��t� ja t�ll�in infoOrganizeriin j�� talteen vanhat datat
	...
	{ // t�ss� varmistetaan ett� hiladatassa on arean XYWorld-recti 0,0 - 1,1 maailmassa, ett� editori osaa piirt�� datan oikein
		...
		{ // tehd��n kuitenkin s��d�t vain jos leveys/korkeus poikkeaa 1:st� (varmuuden vuoksi)
...
int GetModelRunTimeGap(std::shared_ptr<NFmiDrawParam> &theDrawParam)
{
	std::shared_ptr<NFmiDrawParam> tmpDrawParam(new NFmiDrawParam(*theDrawParam));
	tmpDrawParam->ModelRunIndex(0);
	std::shared_ptr<NFmiFastQueryInfo> aInfo = InfoOrganizer()->Info(tmpDrawParam, false, false); // kokeillaa ensin ei poikkileikmkaus dataa
	if(aInfo == 0)
		aInfo = InfoOrganizer()->Info(tmpDrawParam, true, false); // jos ei l�ytynyt, kokeillaan poikkileikkausdataa
	if(aInfo)
...
// maski-otuksen k�yt�ss�, pit�� vain sellainen maski p�ivitt��.
void UpdateParamMaskList(NFmiInfoData::Type theType, const std::string& theDataFilePattern, NFmiQueryData* theData)
{
	std::shared_ptr<NFmiFastQueryInfo> fastInfo = GetMatchingFastInfo(theType, theDataFilePattern, theData);
	if(fastInfo)
	{
		std::shared_ptr<NFmiAreaMaskList> paramMaskList = ParamMaskListMT();
		if(paramMaskList)
		{
			for(paramMaskList->Reset(); paramMaskList->Next(); )
			{
				std::shared_ptr<NFmiFastQueryInfo> maskInfo = paramMaskList->Current()->Info();
				if(maskInfo) // kaikilla maskeilla ei ole infoa
				{
					if(fastInfo->DataFilePattern() == maskInfo->DataFilePattern())
					{
						std::shared_ptr<NFmiFastQueryInfo> fastInfoCopy = NFmiInfoOrganizer::DoDynamicShallowCopy(fastInfo); // pit�� tehd� dynaaminen kopio, ett� saadaan NFmiOwnerInfo-tasoinen matala kopio maskin UpdateInfo-metodille.
...
std::shared_ptr<NFmiFastQueryInfo> GetMatchingFastInfo(NFmiInfoData::Type theType, const std::string& theDataFilePattern, NFmiQueryData* theData)
{
	std::shared_ptr<NFmiFastQueryInfo> foundFastInfo;
	...
	{
		std::vector<std::shared_ptr<NFmiFastQueryInfo> > fastInfoVector = itsSmartInfoOrganizer->GetInfos(theDataFilePattern);
		if(fastInfoVector.empty() == false)
			foundFastInfo = fastInfoVector[0]; // otetaan vain ensimm�inen vektorista, koska oikeasti siin� ei voi olla kuin yksi l�ytynyt fastInfo
	}
...
// Yritet��n est�� liiallisen muistin k�ytt�� kun varataan tilaa undo-redo datalle editoinnissa.
// Jos undo-redo vaatii liikaa muistia, ei oteta sit� k�ytt��n ollenkaan.
...
// dataan teht�v� fiksaus ja t�ss� asetetaan interpolaatio halutuksia, jos datasta l�ytyy total-wind.
...
void DoPossibleTEMPCodeSoundingDataSetup(NFmiQueryData* theData, NFmiInfoData::Type theType, const std::string& theDataFileName)
{
	...
	// eik� ole olemassa editoitua dataa, laitetaan t�m�n tyyppinen data my�s editoitavaksi dataksi,
...
void DoNewEditedDataLoadedSetups(NFmiInfoData::Type theType, bool loadFromFileState, int undoredoDepth, bool setCurrentTimeToNearestHour)
{
	...
	if(theType == NFmiInfoData::kEditable) // 1999.08.30/Marko
	{
		...
		std::shared_ptr<NFmiFastQueryInfo> editedInfo = EditedInfo();
		if(editedInfo)
		{
			...
			editedInfo->First(); // t�m� asetetaan siksi, ett� jos edellisess� datassa oli leveleit� ja nykyisess� ei ole ja oli levelparameja n�yt�ll�, levelit menee pieleen jos leveli� ei aseteta t�ss�
...
bool ReportPossibleRejectedLoadedData(bool fDataWasDeletedOut, const std::string &theDataFileName)
{
	if(fDataWasDeletedOut)
	{
		...
		// 2. Jos ollaan Virtual-time moodissa, oletetaan ett� uutta dataa ei lueta sen vuoksi ett� valittu VT on latadun datan edell�, t�ll�in
...
void DoEditedInfoTimeSetup(std::shared_ptr<NFmiFastQueryInfo>& editedInfo, bool loadFromFileState, bool setCurrentTimeToNearestHour)
{
	if(editedInfo)
	{
...
{
	// Jos ei ole editoitavaa dataa ja on jonkinn�k�inen case-study lataus/sulkeminen -tilanne
...
// Etsi viimeinen aika, milt� qDatasta l�ytyy mit��n dataa (mik� tahansa par, lev, loc).
...
// T�ll�in sallittuja aikoja ovat mm. klo 7:00, klo 8:00 jne. (p�iv�ys huomioiden tietenkin).
// Jos uudempaa sallittua aikaa ei l�ydy, palautetaan missing-aika, muuten palautetaan l�ydetty oikea aika.
NFmiMetTime FindLatestTimeWithAnyData(NFmiQueryData *thedata, const NFmiMetTime &theLatestTime, int theReolutionInMinutes)
{
	...
	if(thedata && theReolutionInMinutes > 0)
	{
		...
		for( ; ; dataTime.PreviousMetTime())
		{
			...
			{
				// etsi l�ytyyk� t�lt� ajan hetkelt� mit��n dataa
...
void DoNewDataNotifications(NFmiQueryData *theData, const std::string &theNotificationStr, const std::string &theDataFilePattern)
{
	...
	else // Jos kyseiselle datalle ei tehd� tavallista uusi data notifikaatiota, tarkastetaan pit�� tehd� ns. uutta dataa uudelle tunnille ilmoitus
	{
		...
		if(helpDataInfo && helpDataInfo->ReportNewDataTimeStepInMinutes() != 0)
		{
			...
			{
			// 6. Jos l�ytyi dataa ajalta, mill� on merkityst�, 
...
void DoLastEditedDataSendHasComeBackChecks(NFmiInfoData::Type theType)
{
	if(theType == NFmiInfoData::kKepaData)
	{
		...
		if(flagChanged && ApplicationWinRegistry().ConfigurationRelatedWinRegistry().ShowLastSendTimeOnMapView())
			// 0 = vain p��karttan�ytt� liataan, cacheja ei tarvitse tyhjent��, koska t�h�n liittyv�t jutut piirret��n vain karttan�yt�n p��lle, ei karttaruudukkoihin
...
std::shared_ptr<NFmiFastQueryInfo> EditedInfo(void)
{
	if(itsSmartInfoOrganizer)
	{
		std::shared_ptr<NFmiFastQueryInfo> info = itsSmartInfoOrganizer->FindInfo(NFmiInfoData::kEditable);
		// 27.12.2001/Marko Jouduin lis��m��n t�h�n levelin s��t� virityksen, koska joskus
	...
	return std::shared_ptr<NFmiFastQueryInfo>(); // jos muu ei auta, palautetaan tyhj� shared_ptr
}
...
std::shared_ptr<NFmiAreaMaskList> ParamMaskListMT(void)
{
	WriteLock lock(gParamMaskListMutex);
	if(itsParamMaskListMT)
		return std::shared_ptr<NFmiAreaMaskList>(new NFmiAreaMaskList(*itsParamMaskListMT));
	else
		return itsParamMaskListMT;
}

// T�m� tekee kopion annetusta paramMaskListasta, ja kopiointi on suojassa threadi lukolla
void ParamMaskListMT(const std::shared_ptr<NFmiAreaMaskList> &theParamMaskList)
{
	WriteLock lock(gParamMaskListMutex);
	if(theParamMaskList)
		itsParamMaskListMT = std::shared_ptr<NFmiAreaMaskList>(new NFmiAreaMaskList(*theParamMaskList));
	else
...
void SnapShotData(std::shared_ptr<NFmiFastQueryInfo> &theInfo, const NFmiDataIdent &theDataIdent, const string &theModificationText
...
std::shared_ptr<NFmiDrawParam> DefaultEditedDrawParam(void)
{
...
public:
	...
	// tietyt parametri saa lis�t� vain karttan�ytt��n
	bool fDoMapMenu = false;
	// n�it� ei sallita toistaiseksi kuin karttan�yt�lle ja poikkileikkaukseen
	...
	// T�t� k�ytet��n kartta- poikkileikkausn�yt�ille
...
void AddCustomFolderToMenuItemList(const MenuCreationSettings &theMenuSettings, NFmiMenuItemList *theCustomMenuList, const std::string &theCustomMenuName)
{
	...
	{
		std::vector<std::shared_ptr<NFmiFastQueryInfo> > infoList = itsSmartInfoOrganizer->GetInfos(helpDataList[j].UsedFileNameFilter(*HelpDataInfoSystem()));
...
// ********* muuta n�ytt�rivin kaikkien datojen tuottajat halutuiksi *********
...
{
	// Laitetaan alkuun editoitu data ja virallinen data tuottajat listaan, jos ei kyse poikkileikkausn�yt�st�
	if(crossSectionPopup == false)
	{
		...
		std::shared_ptr<NFmiFastQueryInfo> operativeInfo = itsSmartInfoOrganizer->FindInfo(NFmiInfoData::kKepaData);
		...
		std::shared_ptr<NFmiFastQueryInfo> helpDataInfo = itsSmartInfoOrganizer->FindInfo(NFmiInfoData::kEditingHelpData);
...
void AddChangeAllProducersFromVectorPart(unsigned int theDescTopIndex, std::vector<NFmiProducerInfo>& producerInfos, NFmiMenuItemList* changeProducersMenuList, FmiMenuCommandType theMenuCommandType)
{
		for(auto &producerInfo : producerInfos)
		{
			...
			if(!infoVector.empty())
			{
				// Jos l�ytyy jotain dataa, laitetaan feikki parametri ja tuottaja listasta
...
void AddMacroParamPartToPopUpMenu(const MenuCreationSettings &theMenuSettings, NFmiMenuItemList *theMenuList, const std::vector<NFmiMacroParamItem> &theMacroParamItemList, NFmiInfoData::Type theDataType)
{
	...
	for(int i=0 ; i<mpSize; i++)
	{
		...
		else
		{
			...
			// HUOM! T�SS� pit�� k�ytt�� drawParamin initfile-namea, koska muuten ei l�ydy macroparamia kuin juuri tasolta
...
void AddHelpDataToParamSelectionPopup(const MenuCreationSettings &theMenuSettings, NFmiMenuItem *theMenuItem)
{
	...
	if(theMenuSettings.fDoMapMenu && ConceptualModelData().Use()) // jos k�siteanalyysi systeemi k�yt�ss�, lis�t��n sen lis��mismahdollisuus popup-valikkoon
	{
	...
	// Mix popup-osiota ei tehd� operatiivisessa moodissa, koska se hidastaa k�ytt��
...
{
	std::vector<std::shared_ptr<NFmiFastQueryInfo> > infos(itsSmartInfoOrganizer->GetInfos(theDataType));
...
// ********** lis�t��n synop-plot data osa *************************
		...
		{ // lis�t��n t�m� siis vain jos editoitavassa datassa on synop-dataa, tai l�ytyy yleens� synop dataa
...
// ********** lis�t��n metar-plot data osa *************************
		...
		{ // lis�t��n t�m� siis vain jos editoitavassa datassa on metar-dataa, tai l�ytyy yleens� metar dataa
...
{
	std::vector<std::shared_ptr<NFmiFastQueryInfo> > infos(itsSmartInfoOrganizer->GetInfos(theProducerId));
	int size = static_cast<int>(infos.size());
	if(size > 0)
	{
		for(int i=0; i<size; i++)
									...
									,NFmiInfoData::Type usedDataType6 = NFmiInfoData::kNoDataType)
{
	if(ProducerSystem().ExistProducer(theModelIndex))
	{
		...
		if(doCrossSectionMenu == false || (doCrossSectionMenu == true && ProducerSystem().Producer(theModelIndex).HasRealVerticalData()))
		{
			...
			std::vector<std::shared_ptr<NFmiFastQueryInfo> > infos(itsSmartInfoOrganizer->GetInfos(prodInfo.ProducerId()));
			int size = static_cast<int>(infos.size());
			int foundSize = 0;
			if(size > 0)
			{
...
// my�s tiedoston nimi ett� kyseisen datan datan (tiedoston) parametrien valinta onnistuisi
// tarvittaessa. Nyt homma menee sekaisin kun jos tiputetaan esim. pari saman tuottajan saman
// tyyppist� dataa editoriin ja t�ll�in vain 1. datoista oikeasti voidaan valita n�yt�lle, koska
...
bool AddMixDataToParamSelectionPopup(const MenuCreationSettings &theMenuSettings, NFmiMenuItem *theMenuItem)
{
	...
	{
		std::vector<std::shared_ptr<NFmiFastQueryInfo> > infos(itsSmartInfoOrganizer->GetInfos(dataTypes[typeCounter]));
...
void AddSmartInfoToMenuItem(const MenuCreationSettings &theMenuSettings, std::shared_ptr<NFmiFastQueryInfo> &theSmartInfo, NFmiMenuItem *theMenuItem, NFmiInfoData::Type theDataType, NFmiParamBag *theWantedParamBag = 0)
{
	if(theSmartInfo == 0 && theMenuItem == 0)
		return ;
	else if(theSmartInfo && theMenuItem)
	{
		// tarkistetaan, l�ytyyk� kyseiselle datalle custom-valikkoa
...
// Jos lis�tt�v� data on salama tyyppist�, sallitaan sen lis�ys vain karttan�yt�ille
bool DoLightningDataTypePopupCheck(const MenuCreationSettings& theMenuSettings, std::shared_ptr<NFmiFastQueryInfo>& info)
{
...
void AddSmartInfoToMenuList(const MenuCreationSettings &theMenuSettings, std::shared_ptr<NFmiFastQueryInfo> &theSmartInfo, NFmiMenuItemList *theMenuItemList, NFmiInfoData::Type theDataType, const std::string &theDictionaryStr = "", NFmiParamBag *theWantedParamBag = 0)
{
...
{
	std::shared_ptr<NFmiFastQueryInfo> info = itsSmartInfoOrganizer->FindInfo(theDataType);
	if(info)
	{
...
void AddMultiModelRunToMenu(std::shared_ptr<NFmiDrawParam> &theDrawParam, NFmiMenuItemList &thePopupMenu, unsigned int theDescTopIndex)
{
	if(theDrawParam)
	{
...
// T�t� popupia k�ytet��n kun on klikattu olemassa olevaa n�ytt��.
...
// T�t� popupia k�ytet��n kun lis�t��n aikasarjaikkunaan uusia n�ytt�j�.
...
void AddFixedDrawParamsToMenu(const NFmiFixedDrawParamFolder &theFixedDrawParamFolder, const std::string &theSubFolderName, NFmiMenuItemList &thePopupMenu, unsigned int theDescTopIndex, int theRowIndex, int index, const NFmiDataIdent &theParam, const NFmiLevel* theLevel, NFmiInfoData::Type theDataType, std::shared_ptr<NFmiDrawParam> &theDrawParam)
{
    ...
    { // theFixedDrawParamFolder oli tyhj�, varoitetaan k�ytt�j�� ett� ei ole l�ytynyt yht��n tehdasasetuksia
...
void AddChangeParamSectionIntoPopupMenu(NFmiMenuItemList* thePopupMenu, unsigned int theDescTopIndex, int layerIndex, std::shared_ptr<NFmiDrawParam>& drawParam)
{
...
bool CreateViewParamsPopup(unsigned int theDescTopIndex, int theRowIndex, int layerIndex, double layerIndexRealValue)
{
	...
	if(itsSmartInfoOrganizer && rowDrawParamList)
	{
		if(rowDrawParamList->Index(layerIndex))
		{
			...
			std::shared_ptr<NFmiDrawParam> drawParam = rowDrawParamList->Current();
			...
			if(crossSectionPopup == false)
			{ // poikkileikkaus-n�ytt� ei tue tekstimuotoista piirtoa
			...
			menuItem->ExtraParam(0); // kun parametria aktivoidaan (tai tulevaisuudessa tehd��n mit� vain), k�ytet��n extraParamia kertomaan miss� parametri on, 0=karttan�ytt�, 1=poikkileikkaus ja 2=aikasarja
			itsPopupMenu->Add(std::move(menuItem));
...
{
	if(theDescTopIndex > CtrlViewUtils::kFmiMaxMapDescTopIndex)
		return; // Jos kyse muusta kuin karttan�yt�st�, ei tehd� mit��n...
	...
	if(drawParamList)
	{
		...
		// Todellinen k�ytt�j�n haluama layer-indeksi (mihin uusi layer lis�t��n tai siirret��n) on py�ristys layerIndexRealValue:sta.
		int wantedLayerIndex = boost::math::iround(layerIndexRealValue);
...
bool CreateMaskParamsPopup(int theRowIndex, int index)
{
	itsCurrentViewRowIndex = theRowIndex;
	fOpenPopup = false;
	std::shared_ptr<NFmiAreaMaskList> tempParamMaskList = ParamMaskListMT();
...
// n�ytt�komentoja, joita voi antaa dokumentille.
// K�ytet��n aluksi ainakin erilaisten tuplaklikkausten toteutuksia varten.
// Tulevaisuudessa n�ytt� makrot voisi toteuttaa t�t� kautta?
...
// viewType voi kertoa esim. onko kyseeess� kartta tai aikasarja n�ytt� (tulevaisuudessa lis�� esim. luotaus jne.).
...
NFmiAreaMask* CreateMask(const NFmiMenuItem& theMenuItem)
{
	...
	{
		std::shared_ptr<NFmiFastQueryInfo> info = itsSmartInfoOrganizer->Info(theMenuItem.DataIdent(), theMenuItem.Level(), theMenuItem.DataType());
		if(info)
		{
			// T�ss� tehd�� dynaaminen pinta kopio infosta, jotta ainakin NFmiOwnerInfo-osio tulee mukaan.
			std::shared_ptr<NFmiFastQueryInfo> infoCopy = NFmiInfoOrganizer::DoDynamicShallowCopy(info);
			if(infoCopy)
			{
...
{
	GetCombinedMapHandler()->timeSerialViewDirty(true);
	std::shared_ptr<NFmiAreaMask> mask(CreateMask(theMenuItem));
	if(mask)
	{
		...
		if(dlg.DoModal() == IDOK)
		{
			...
			if(paramMaskList)
			{
				...
				ParamMaskListMT(paramMaskList); // maskilistaan tuli lis�ys/poisto, t�ll�in lista pit�� asettaa dokumenttiin k�ytt��n, muutokset yksitt�isiin maskeihin menev�t automaattisesti k�ytt��n
			}
		}
	}
}
...
void DisableMask(const NFmiMenuItem& theMenuItem)
{
	...
	if(paramMaskList)
	{
		if(paramMaskList->Index(theMenuItem.IndexInViewRow()))
		{
            ...
            ParamMaskListMT(paramMaskList); // maskilistaan tuli muutos, t�ll�in lista pit�� asettaa dokumenttiin k�ytt��n, jolloin my�s n�ytt�jen likaukset tehd��n oikein
        }
	}
}
...
void ModifyMask(const NFmiMenuItem& theMenuItem)
{
	...
	{
		std::shared_ptr<NFmiAreaMask> mask = paramMaskList->Current();
		if(mask)
		{
			CFmiMaskOperationDlg dlg;
			dlg.ParamMask(mask.get());
...
void SetCrossSectionTrajectoryParams()
{
	...
	for(int i = 1; i <= ssize; i++)
	{
		...
		if(aList)
		{
			...
			{
				std::shared_ptr<NFmiDrawParam> aDParam = aList->Current();
				...
				{ // pinta dataa, joka ei n�y kuitenkaan poikkileikkaus n�yt�ss�
					aDParam->DataType(NFmiInfoData::kHybridData);
...
void SetCrossSectionTrajectoryTimes(int theRowIndex)
{
	...
	{ // muutin systeemin asettamaan ajat my�s aika-moodissa. Ottaa ensimm�isen drawParamia
	  // vastaavan infon ja s��t�� alku ja loppu ajat sen mukaan
		...
		if(aList)
		{
			...
			{
				std::shared_ptr<NFmiDrawParam> dParam = aList->Current();
				std::shared_ptr<NFmiFastQueryInfo> info = InfoOrganizer()->Info(dParam, true, false);
				if(info)
				{
...
// laskee n�ytt�ruudukon yhden ruudun koon pikseleiss�
...
bool MakeGridFileForMacroParam(unsigned long usedMapViewIndex, std::shared_ptr<NFmiDrawParam> &drawParam, const NFmiString& theFileName)
{
...
// tallettaa aktiivisen n�ytt�rivin aktiivisen parametrin currentin ajan gridin tiedostoon,
// jonka nimi annetaan parametrina, mutta se talletetaan ty�hakemistoon
bool MakeGridFile(const NFmiString& theFileName)
{
	// tehd��n vain p��karttaikkunasta n�it� talletuksia
	auto usedMapViewIndex = 0ul;
	std::shared_ptr<NFmiDrawParam> drawParam = GetCombinedMapHandler()->activeDrawParamFromActiveRow(usedMapViewIndex);
	bool status = false;
	if(drawParam)
	{
...
void SetTimeFilterStartTime(const NFmiMetTime& theTime)
{
	...
	std::shared_ptr<NFmiFastQueryInfo> editedInfo = EditedInfo(); // HUOM! EditedInfo pit�� ottaa omaan muuttujaan, ja k�ytt�� siit�. Jos pyyt�� EditedInfo-funktion kautta kokoajan uutta, voi palautua eri iteraattori ja s��d�t menev�t pieleen.
...
bool IsMacroParamAndDrawWithSymbols(std::shared_ptr<NFmiDrawParam> &drawParam)
{
...
// Jos karttan�yt�n koko muutetaan, muuttuu t�ll�in piirrett�v�n karttaalueen koko pikseleiss�.
// T�ll�in jos n�yt�ll� on macroParametreja, jotka piirret��n symboleilla, pit�� niiden macroParamCachet 
...
bool DoMapViewOnSize(int mapViewDescTopIndex, const NFmiPoint& clientPixelSize, CDC* pDC)
{
	...
	if(mapViewDesctop)
	{
		...
		return true; // Jos kyse oli karttan�yt�n asetuksesta, palautetaan true
	}
	else if(mapViewDescTopIndex == CtrlViewUtils::kFmiCrossSectionView)
	{
		...
		return true; // Jos kyse oli poikkileikkausn�yt�n asetuksesta, palautetaan true
	}
	return false; // Jos kyse oli jostain muusta kuin karttan�yt�st�, palautetaan false sen merkiksi ett� mit��n ei tehty
}
#endif
...
void DoAutoSaveData(void)
{
    ...
    try
    {
	    ...
	    {
		    if(MetEditorOptionsData().ControlPointMode()) // talletetaan CP-dataa vain kun k�ytet��n kyseist� ty�kalua, eik� joka ikinen minuutti
            {
		    ...
		    std::shared_ptr<NFmiFastQueryInfo> smart = EditedInfo();
		    if(smart)
		    {
...
// Ns. working data talletetaan vain jos on k�yt�ss� undo/redo toiminto (editointia ei voi tehd� ilman sit�: memorymapping read-only jutun takia)
...
bool StoreWorkingData(std::shared_ptr<NFmiFastQueryInfo> &smart, bool askForSave)
{
...
{
	std::shared_ptr<NFmiFastQueryInfo> info = EditedInfo();
	if(info)
	{
		if(newFile)
		{
...
std::string GetHelperForecasterId()
{
    ...
    {
        std::shared_ptr<NFmiFastQueryInfo> helperData = InfoOrganizer()->FindInfo(NFmiInfoData::kEditingHelpData);
        if(helperData && helperData->FindFirstKey(FmiModifyEditdData::ForecasterIDKey)) // L�ytyik� helperData ja l�ytyyk� siit� ForecasterID:t�?
...
bool StoreData(const NFmiString& theFileName, std::shared_ptr<NFmiFastQueryInfo> &theSmartInfo, bool askForSave)
{
	...
	if(theSmartInfo && (theFileName != NFmiString())) // MIT� JOS EI OLE NIME�!!!!!!!
	{
        ...
        {
            // Laitetaan normaali talletus my�s ensin lokaali tmp-tiedostoon josta kopiointi win32 funktiolla serverille, miss� rename lopulliseksi.
            // T�m� siksi ett� nyt my�s normaali talletus on alkanut t�kkim��n ja se saattaa kest�� kymmeni� minuutteja.
...
void PutProducerIdListInDataHeader(std::shared_ptr<NFmiFastQueryInfo> &theSmartInfo, std::vector<NFmiFastQueryInfo*> &theSourceInfos, std::vector<int> &theModelIndexVector)
{
...
{
    // Haetaan k�yt�ss� oleva CP-manager
    ...
    if(controlPointManager)
    {
        ...
        // Vain aikasarja pit�� laittaa t�ss� likaiseksi, sielt� mist� t�t� kutsutaan (MakePopUpCommandUsingRowIndex) laitetaan karttan�yt�t likaisiksi.
		GetCombinedMapHandler()->timeSerialViewDirty(true);
    }
}
...
{
	// armeija / military / CD rom editor versiossa on try - catch blokki k�yt�ss�!!
	...
	try
	{
...
#ifdef UNIX
		::chdir(itsBasicConfigurations.WorkingDirectory().c_str()); // palautetaan alkuper�inen ty�hakemisto voimaan talletuksia varten
#else
		::_chdir(itsBasicConfigurations.WorkingDirectory().c_str()); // palautetaan alkuper�inen ty�hakemisto voimaan talletuksia varten
#endif
		itsMTATempSystem.StoreSettings();
        // N�m� sounding dialogin asetukset pit�� ottaa MTATempSystem:ist� takaisin Win-registeriin, koska niit� on mahdollisesti latailtu n�ytt�makroista
#ifndef UNIX
		...
		StoreOptionsData(); // t�m� tekee my�s asetuksien talletuksen konfiguraatio tiedostoihin! T�m� pit�� siis kutsua viimeisen�.
...
bool InitCPManagerSet(void)
{
	...
	bool InitSmartToolInfo(void)
	{
		...
		SetCurrentSmartToolMacro(itsSmartToolInfo.CurrentScript()); // laitetaan currentti skripti my�s dociin

        // Alustetaan my�s yksi smartTool kieleen liittyv�t callback funktiot
        NFmiInfoAreaMask::SetMultiSourceDataGetterCallback(
			[this](std::vector<std::shared_ptr<NFmiFastQueryInfo> > &theInfoVector, 
				const NFmiDataIdent& dataIdent,
				const NFmiLevel &level,
				NFmiInfoData::Type dataType,
				const std::shared_ptr<NFmiArea> &theArea)
			{
				std::shared_ptr<NFmiDrawParam> drawParam(new NFmiDrawParam(dataIdent, level, 0, dataType));
...
#undef max
	...
	{ // t�yt� makro editorin n�ytt� asetuksilla
	...
	void AdjustDrawParam(std::shared_ptr<NFmiDrawParam> &theDrawParam, const NFmiViewSettingMacro::Param &theParam, bool fTreatAsViewMacro)
	{
		if(theDrawParam)
		{
	...
	void AdjustDrawParam(unsigned int theDescTopIndex, const NFmiViewSettingMacro::Param &theParam, int theRowIndex, int paramIndexInRow, bool fTreatAsViewMacro)
	{
		...
		if(dList)
		{
			std::shared_ptr<NFmiDrawParam> dPar;
    ...
    void SetCrossSectionView(NFmiViewSettingMacro &theMacro, bool fTreatAsViewMacro)
	{
        ...
        // Pit�� tyhjent�� aluksi poikkileikkausn�yt�n kaikkien rivien parametrit, koska sen t�ytt� looppia (joka tyhjent�� kulloisenkin rivin ensin) ei v�ltt�m�tt� edes ajeta
    ...
    // fTreatAsViewMacro -parametria tarvitaan, koska ns. backupViewMacro:jen latauksien yhteydess� halutaan ladata parametreja n�yt�ille
	...
	{
        // Pit�� tyhjent�� aluksi karttan�yt�n kaikkien rivien parametrit, koska sen t�ytt� looppia (joka tyhjent�� kulloisenkin rivin ensin) ei v�ltt�m�tt� edes ajeta
		...
		for(size_t rowVectorCounter = 0; rowVectorCounter < mapRowsSize; rowVectorCounter++)
		{
			...
			for(size_t viewMacroLayerIndex = 0 ; viewMacroLayerIndex < viewMacroRowParams.size(); viewMacroLayerIndex++)
			{
                ...
                if(usedDataType >= NFmiInfoData::kSoundingParameterData)
                {
                    ...
                    // on NFmiInfoData::kModelHelpData. T�m� siis vain pit�� yll� taaksep�in yhteen sopivuutta n�ytt�makroissa.
...
//		theViewMacro.SetAllExtraRowParams(descTop.ExtraDrawParamListVector(), itsMacroParamSystem);
	...
	void SetMasksParams(NFmiViewSettingMacro &theMacro)
	{
		...
		for( ;counter < ssize ; counter++)
		{
			...
			std::shared_ptr<NFmiAreaMask> areaMask(CreateMask(menuItem));
			if(areaMask)
			{
		...
		ParamMaskListMT(paramMaskList); // asetetaan lopuksi uudet maskit k�ytt��n
	}
	...
	{
		// ota k�ytt��n kaikki makron asetukset ja tee n�yt�ist� 'likaisia'
        ...
        // Lopuksi (jos poikkeuksia ei ole lent�nyt) laitetaan ladatun macron nimi talteen p��karttan�yt�n title teksti� varten
        SetLastLoadedViewMacroName(theMacro, fTreatAsViewMacro, undoRedoAction);
        ApplicationInterface::GetApplicationInterfaceImplementation()->UpdateMainFrameTitle(); // t�m� sitten p�ivitt�� p��karttan�yt�n otsikko tekstiin viimeisimm�n viewMacron nimen
    }
#endif
    ...
    // Jos n�ytt�makro oli korruptoitunut, palautetaan false, muuten true
	bool LoadViewMacro(const std::string &theName)
	{
		...
		try
		{
			...
			// HUOM! t�ss� vaiheessa pit�� n�ytt�makro lukea uudestaan, ett� ajat p�ivittyv�t nykyhetkeen.
            ...
            if(ReadViewMacro(*CurrentViewMacro(), initFileName))
            {
                ...
                // lokitetaan, mik� makro on ladattu k�ytt��n...
...
void UndoViewMacro(void)
{
	...
	if(IsUndoableViewMacro())
	{
	    ...
	    // 2. Lataa k�ytt��n undoredoListasta edellinen makro k�ytt��n
...
void RedoViewMacro(void)
{
	...
	{
    	// 2. Lataa k�ytt��n undoredoListasta seuraava makro k�ytt��n
		itsUndoRedoViewMacroListPointer++;
...
bool IsRedoableViewMacro(void)
{
	...
	void RefreshViewMacroList(void)
	{
		...
		for( ; it != itEnd; ++it)
		{
			...
			itsViewMacroDescriptionList.push_back(viewMacroDescription); // laitetaan my�s virheellinen listaan, ett� se voidaan poistaa ViewMacro-dialogin avulla
		}
	}
...
std::shared_ptr<NFmiArea> MakeCPCropArea(std::shared_ptr<NFmiFastQueryInfo> &theInfo, float theShrinkFactor, unsigned long x1, unsigned long y1, unsigned long x2, unsigned long y2, unsigned long shrinkX, unsigned long shrinkY)
{
	...
	return std::shared_ptr<NFmiArea>(new NFmiLatLonArea(bottomLeftLatlon, topRightLatlon));
}

void SetCPCropGridSettings(const std::shared_ptr<NFmiArea> &theArea, unsigned int theDescTopIndex)
{
    if(theDescTopIndex != 0)
        return; // Ei tehd� CP-crop juttuja kuin p��karttan�yt�n kanssa

	itsCPGridCropMargin = NFmiPoint();
	itsCPGridCropRect = NFmiRect(); // asetetaan tyhj� crop-recti aluksi
	itsCPGridCropLatlonArea = std::shared_ptr<NFmiArea>();
	itsCPGridCropInnerLatlonArea = std::shared_ptr<NFmiArea>();
	std::shared_ptr<NFmiFastQueryInfo> editedInfo = EditedInfo();
	if(theArea && editedInfo && editedInfo->Grid())
	{
        std::shared_ptr<NFmiArea> editedArea(editedInfo->Area()->Clone());
		if(NFmiQueryDataUtil::AreAreasSameKind(theArea.get(), editedArea.get()))
		{ // crop rect voidaan laskea vain kun editoidun datan area ja zoomattu area ovat samaa tyyppi�, t�ll�in zoomi alue on neliskanttinen editoidulla datalla
			...
			if((bottomLeftCornerLocationIndex != gMissingIndex) && (topRightCornerLocationIndex != gMissingIndex) && (bottomLeftCornerLocationIndex != topRightCornerLocationIndex))
			{
				...
				{ // CP crop ei ole j�rkev��, on zoomattu datassa liian l�helle ja k�yt�ss� on liian v�h�n hilapisteit�.
					// Nyt lasketaan vain ulompi laatiiko, jonka sis��n voidaan karttan�yt�ss� piirt�� punainen rasti.
					itsCPGridCropLatlonArea = MakeCPCropArea(editedInfo, 1, x1, y1, x2, y2, shrinkX, shrinkY);
				}
			}
		}
	...
	// lis�� halutun nimisen macroParamin halutun karttan�yt�n riville (1-5)
	...
	// poistaa halutun macroparamin dokumentista, tiedostoista ja n�yt�ilt�
	void RemoveMacroParam(const std::string &theName)
	{
		...
		else
		{
			...
			if(currentFolder)
				currentFolder->Remove(theName); // poista macroparam mpsysteemist� (joka tuhoaa my�s tiedostot)
...
// ********* muuta n�ytt�rivin kaikkien datojen tuottajat halutuiksi *********
		...
		// ********** lis�t��n mahdollinen aika-moodin aikas��t� komento osa *************************
...
void AddTrajectoryOptionsToCrossSectionPopupMenu(NFmiMenuItemList *thePopupMenu)
{
	if(TrajectorySystem()->ShowTrajectoriesInCrossSectionView())
	{
        auto menuItem = std::make_unique<NFmiMenuItem>(-1, "Trajectories", NFmiDataIdent(), kFmiCrossSectionSetTrajectoryTimes, g_DefaultParamView, nullptr, NFmiInfoData::kViewable); // NFmiInfoData::kViewable on vain merkitykset�n default arvo
		NFmiMenuItemList *menuList = new NFmiMenuItemList;
        auto item = std::make_unique<NFmiMenuItem>(-1, ::GetDictionaryString("CrossSectionViewSelectionPopUpSetTimes")
											,NFmiDataIdent()
											,kFmiCrossSectionSetTrajectoryTimes
...
void AddToCrossSectionPopupMenu(NFmiMenuItemList *thePopupMenu, NFmiDrawParamList *theDrawParamList, const std::string &theMenuTitle, FmiMenuCommandType theMenuCommand)
{
	...
	{
        auto menuItem = std::make_unique<NFmiMenuItem>(-1, theMenuTitle, NFmiDataIdent(), theMenuCommand, g_DefaultParamView, nullptr, NFmiInfoData::kViewable); // NFmiInfoData::kViewable on vain merkitykset�n default arvo
		NFmiMenuItemList *menuList = new NFmiMenuItemList;
		int aIndex = 1;
		for(theDrawParamList->Reset(); theDrawParamList->Next();)
		{
			...
			{ // muuten laitetaan parametrin nimeen my�s mallitietoa, jos data l�ytyy
			...
			item->ExtraParam(1); // kun parametria aktivoidaan (tai tulevaisuudessa tehd��n mit� vain), k�ytet��n extraParamia kertomaan miss� parametri on, 0=karttan�ytt�, 1=poikkileikkaus ja 2=aikasarja
			menuList->Add(std::move(item));
			aIndex++;
		}
		menuItem->AddSubMenu(menuList);
	...
	// omat-hakemistoa, ettei sinne j�� �lytt�mi� m��ri� tiedostoja roikkumaan
	bool CleanFilesFromDirectory(const NFmiString& theWorkingPath, int theTimeLimitInHours)
	{
		...
		if(NFmiFileSystem::DirectoryExists(wantedPath))
		{
			...
			for( ; it != fileList.end(); ++it)
			{
				...
				if(tim < 0)
					continue; // NFmiTime:n konstruktori kaatuu, jos tim on negatiivinen, ja luku on negatiivinen jos tiedopstoa ei kaikesta huolimatta l�ydyk��n.
				NFmiTime aTime(tim);
	...
	// Sama koodi l�ytyy NFmiStationViewHandler-luokasta
	...
	// Melkein vastaava koodi l�ytyy NFmiStationViewHandler::ComposeSilamLocationsToolTipText -metodista
	...
	void SelectLocations(unsigned int theDescTopIndex, std::shared_ptr<NFmiFastQueryInfo> &theInfo, const std::shared_ptr<NFmiArea> &theMapArea, const NFmiPoint& theLatLon
						...
						,bool fDoOnlyMTAModeAdd)
	{
		...
		// jos ollaan luotaus n�yt�n MTA-moodissa, lis�t��n my�s valittu luotaus info dokumenttiin
		if(doMTAModeAdd)
		{
...
//				MakeEditorDirty(theDescTopIndex, true, true, true);
			...
			{
				// Jos vasen klikkaus karttan�yt�ll� ja ollaan MTA-moodissa, nollataan luotaukset ja lis�t��n yksi
		...
		if(doTrajectoryModeAdd)
		{
            ...
            // Trajektorien muutos vaikuttaa kaikkiin karttan�yt�n ruutuihin joka rivill� (siksi 3. parametri on true)
	...
	void DoTEMPDataUpdate(const std::string &theTEMPCodeTextStr, std::string &theTEMPCodeCheckReportStr, bool fJustCheckTEMPCode)
	{
		...
		{
			// otetaan TEMP koodi data k�ytt��n jos l�ytyi ja  ei ollut pelkk� tarkistus operaatio
			bool dataWasDeleted = false;
	...
	void ClearTEMPData(void)
	{
		...
		// Karttan�ytt� on piirrett�v� uudestaan, koska siell� saattaa n�ky�  uusia luotaus merkkej�.
	...
	void SetMacroParamDataGridSize(int xSize, int ySize)
	{
        ...
        // Pit�� tyhjent�� kaikkien ikkunoiden kaikkien rivien macroParam datat (voisi optimoida, jos tiedett�isiin, miss� on k�yt�ss� "RESOLUTION = xxx", niit� ei tarvitsisi tyhjent��)
...
#else
			int qDataConstructors = 0;
			int qDatadestructors = 0;
#endif
	...
	std::shared_ptr<NFmiFastQueryInfo> GetNearestSynopStationInfo(const NFmiLocation &theLocation, const NFmiMetTime &theTime, bool ignoreTime, std::vector<std::shared_ptr<NFmiFastQueryInfo> > *thePossibleInfoVector, double maxDistanceInMeters = 1000. * kFloatMissing)
	{
		std::shared_ptr<NFmiFastQueryInfo> info;
		std::vector<std::shared_ptr<NFmiFastQueryInfo> > infoVector = (thePossibleInfoVector == 0) ? GetSortedSynopInfoVector(kFmiSYNOP, kFmiTestBed, kFmiSHIP, kFmiBUOY) : *thePossibleInfoVector;
	...
	// T�m� funktio takaa ett� haluttaessa synop-data tulevat k�ytt��n aina samassa j�rjestyksess�.
	...
	std::vector<std::shared_ptr<NFmiFastQueryInfo> > GetSortedSynopInfoVector(int theProducerId, int theProducerId2 = -1, int theProducerId3 = -1, int theProducerId4 = -1)
	{
		std::vector<std::shared_ptr<NFmiFastQueryInfo> > infoVector = InfoOrganizer()->GetInfos(theProducerId, theProducerId2, theProducerId3, theProducerId4);

		if(itsSynopDataFilePatternSortOrderVector.size())
		{ // jos on m��r�tty sorttaus j�rjestys, tehd��n sorttaus
			std::vector<std::shared_ptr<NFmiFastQueryInfo> > copyOfInfoVector(infoVector); // t�st� listasta otetaan sorttauksessa l�ydetyt infot pois,
			...
			std::vector<std::shared_ptr<NFmiFastQueryInfo> > sortedInfoVector;
			for(size_t i = 0; i < itsSynopDataFilePatternSortOrderVector.size(); i++)
			{
				for(size_t j = 0; j < infoVector.size(); j++)
				{
					...
					if(itsSynopDataFilePatternSortOrderVector[i] == noPathFilePattern)
					{
						...
						copyOfInfoVector[j] = std::shared_ptr<NFmiFastQueryInfo>(); // laitetaan 0-pointteri t�h�n merkiksi, ett� on otettu jo k�ytt��n
						break;
					}
				}
			}
			for(size_t k = 0; k < copyOfInfoVector.size(); k++)
			{
				if(copyOfInfoVector[k] != 0)
					sortedInfoVector.push_back(copyOfInfoVector[k]); // t�t� ei l�ytynyt sorttaus listasta, eli laitetaan se per��n
			}
    ...
    // Tapaus 1: Kun animaatiossa on havaintoja seuraava moodi p��ll� ja on isompi ruudukko k�yt�ss� (esim. 3x2 eli 3 aikaa ja 2 rivi�), 
    // t�ll�in on tarkoitus j�tt�� animaatio looppi vajaaksi, niin ett� animaatio loppuu silloin kun viimeisten havaintojen aika on juuri
	...
	int DoAllAnimations(void)
	{
		...
		bool mainViewMustBeUpdated = false; // t�ss� yritet��n optimoida p�ivitys rutiinia. Jos p�� ikkunaa pit�� p�ivitt��,
										// pit�� p�ivitt�� smartmetin kaikkia n�ytt�j�. Jos animaatiot py�riv�t vain apukarttan�yt�iss�
		...
		if(maxStatus)
		{
			...
			{
				// jos ei ollut v�lit�nt� animaatio tarvetta, lasketaan t�ss� pieni nukkumis aika, 
				// ett� SmartMet ei jauha OnIdle-looppia t�ydell� h�yryll� kokoaikaa. Aika on joko 
...
#undef min // MSVC:n m��rittelem� min-makro pit�� disabloida
	...
	// tarkasta CView-n�ytt�luokissa, onko mahdollisesti animaatiota k�ynniss�. Jos on, �l� laita odota-cursoria n�kyviin, koska se vilkuttaa ik�v�sti
	...
	void ToggleTimeControlAnimationView(unsigned int theDescTopIndex)
	{
		...
		if(descTop)
		{
			...
			GetCombinedMapHandler()->mapViewDirty(theDescTopIndex, false, false, true, false, false, false); // t�m�n pit�isi asettaa n�ytt� p�ivitys tilaan, mutta cachea ei tarvitse en�� erikseen tyhjent��
    ...
    NFmiPoint GetPrintedMapAreaOnScreenSizeInPixels(unsigned int theDescTopIndex)
	{
		...
		{ // pit�� laskea uusi printti alue ilman k�yt�ss� olevaa aikakortrolli-ikkuna osiota 
	...
	NFmiMetTime GetNewerOriginTimeFromInfos(std::vector<std::shared_ptr<NFmiFastQueryInfo> > &theInfos, const NFmiMetTime &theCurrentLatestTime)
	{
	...
	NFmiMetTime GetLatestOriginTimeFromModel(std::shared_ptr<NFmiDrawParam> &theDrawParam)
	{
		...
		if(dataType == NFmiInfoData::kViewable || dataType == NFmiInfoData::kHybridData || dataType == NFmiInfoData::kModelHelpData) // jos kyseess� tavallinen mallidata, tarkastellaan kolmea eri dataa ja etsit��n niist� uusin origin aika
		{
			...
			std::vector<std::shared_ptr<NFmiFastQueryInfo> > groundInfos = InfoOrganizer()->GetInfos(NFmiInfoData::kViewable, true, prodId);
			latestTime = GetNewerOriginTimeFromInfos(groundInfos, latestTime);
			std::vector<std::shared_ptr<NFmiFastQueryInfo> > pressureInfos = InfoOrganizer()->GetInfos(NFmiInfoData::kViewable, false, prodId);
			latestTime = GetNewerOriginTimeFromInfos(pressureInfos, latestTime);
			std::vector<std::shared_ptr<NFmiFastQueryInfo> > hybridInfos = InfoOrganizer()->GetInfos(NFmiInfoData::kHybridData, false, prodId);
		...
		else
		{
			...
			std::shared_ptr<NFmiFastQueryInfo> aInfo = InfoOrganizer()->Info(tmpDrawParam, false, false);
			if(aInfo)
	...
	bool SetNearestBeforeModelOrigTimeRunoff(std::shared_ptr<NFmiDrawParam> &theDrawParam ,const NFmiMetTime &theTime, unsigned int theDescTopIndex, int theViewRowIndex)
	{
		...
		// 3.2 jos ei aikaa, tee nyky ajan hetkest� aika py�ristettyn� l�himp��n edelliseen mahdolliseen malliaikaan
		if(latestTime != NFmiMetTime::gMissingTime)
		{
	...
	void SetModelRunOffset(std::shared_ptr<NFmiDrawParam> &theDrawParam, int theMoveByValue, unsigned int theDescTopIndex, int theViewRowIndex)
	{
	...
	NFmiMetTime GetModelOrigTime(std::shared_ptr<NFmiDrawParam> &theDrawParam, int theIndex = 0)
	{
	...
	std::string GetModelOrigTimeString(std::shared_ptr<NFmiDrawParam> &theDrawParam, int theIndex = 0)
	{
		std::string str;
    ...
    // Legacy syist� polkua yritet��n lukea my�s vanhasta dataPath4 -asetuksesta.
	...
	NFmiParamBag& AllStaticParams(void)
	{
		...
		std::shared_ptr<NFmiFastQueryInfo> statInfo = itsSmartInfoOrganizer->FindInfo(NFmiInfoData::kStationary);
		if(statInfo)
	...
	bool LoadCaseStudyData(const std::string &theCaseStudyMetaFile)
	{
		...
		std::shared_ptr<NFmiHelpDataInfoSystem> oldCaseStudyHelpDataInfoSystem = itsCaseStudyHelpDataInfoSystem;
		...
		try
		{
			...
			// 5. Laita kaikkien n�ytt�jen kello CaseStudy-aikaan
			...
			// 6. Heit� kaikki dynaaminen data roskiin
                    ...
                    // 7. Laita HelpDataInfoSystem-tarvittaviin paikkoihin k�ytt��n (HelpDataInfoSystem-funktio, datanlataus-threadeihin jne.) JA aloita dynaamisen datan uudelleen lataus
			// CSmartMetDoc:in CaseStudyLoadingActions my�s likaa cachet ja p�ivitt�� lopuksi kaikki n�yt�t!!, joten se pit�� tehd� viimeisen�...
	...
	bool UseMultithreaddingWithModifyingFunctions(void)
	{
		...
		if(itsMachineThreadCount <= 1)
			return false; // jos oli k�yt�ss� vain yksi core, turha multi-threadata, koska se vain hidastaa
	...
	const std::shared_ptr<NFmiArea> CPGridCropLatlonArea(void) 
	{
		return itsCPGridCropLatlonArea;
	}

	std::shared_ptr<NFmiArea> CPGridCropInnerLatlonArea(void)
	{
    ...
    void ZoomMapWithRelativeRect(int theMapViewDescTopIndex, std::shared_ptr<NFmiArea> &theMapArea, const NFmiRect &theZoomedRect)
    {
 	    std::shared_ptr<NFmiArea> newZoomedArea(theMapArea->CreateNewArea(theZoomedRect));
	    if(newZoomedArea)
	    {
            ...
            // Laitetaan t�h�n "Ohitetaan normi p�ivit� kaikki n�yt�t ja p�ivit� vain muuttunutta karttan�ytt��" -toiminto optimoinnin takia.
            // HUOM! Optimoinnin voi laittaa t�nne, koska esim. viewMacrojen latauksissa ei k�ytet� t�t� metodia (jolloin tarvitsee p�ivitt�� kaikkia n�ytt�j�)
            ...
            {
                // Jos kyse p��karttan�yt�n zoomista, pit�� my�s p�ivitt�� asemadatataulukkon�ytt��, koska siihen laitetaan
    ...
    void PanMap(int theMapViewDescTopIndex, std::shared_ptr<NFmiArea> &theMapArea, const NFmiPoint &theMousePoint, const NFmiPoint &theZoomDragUpPoint)
    {
#ifndef UNIX
    ...
    NFmiRect ScaleZoomedAreaRect(std::shared_ptr<NFmiArea> &theMapArea, const NFmiRect &theMaxRect, const NFmiPoint &theMousePoint, double theScaleFactor)
    {
        ...
        newRect.Center(currentXYRect.Center()); // pit�� asettaa my�s keskipiste vanhan rect:in keskipisteen kohdalle, ett� keskitys laskut onnistuvat

        std::shared_ptr<NFmiArea> tmpArea(theMapArea->CreateNewArea(newRect));
        if(tmpArea)
	    ...
	    // Pieni hienos��t� tarvitaan jossain tapauksissa viel�, ett� ei varmasti menn� max alueen yli (t�m� on t�rke�� ainakin koko maailman karttojen kanssa)
	    if(newRect.Left() < theMaxRect.Left())
    ...
    void ZoomMapInOrOut(int theMapViewDescTopIndex, std::shared_ptr<NFmiArea> &theMapArea, const NFmiPoint &theMousePoint, double theZoomFactor)
    {
#ifndef UNIX
        std::shared_ptr<NFmiArea> totalMapArea = GetCombinedMapHandler()->getMapViewDescTop(theMapViewDescTopIndex)->MapHandler()->TotalArea();
...
//        boost::thread tt(boost::bind(&GeneralDocImpl::MakeSureToolMasterPoolIsRunning2, this));
    ...
    void StoreBackUpViewMacro(bool fUseNormalBackup)
    {
        try
        {
            ...
            if(fUseNormalBackup)
                NFmiFileSystem::SafeFileSave(MakeBackUpViewMacroFileName(false), out.str()); // Jos normaali lopetus, talletetaan t�ss� samalla my�s crashBackupViewMacro
        }
        catch(std::exception &e)
        {
    ...
    {
        // Tutkitaan l�ytyyk� karttan�yt�st� mit��n theImageCacheUpdateData -listan jutuista.
        // Jos l�ytyy, liataan sen karttan�ytt�cache ja palautetaan true.
    ...
    // Jos on, tee tarvittavat n�ytt�jen likaukset, p�ivit� ne jos oli tarvetta.
    // Jos n�ytt�j� p�ivitettiin palauta 1 jos ei, palauta 0.
    // HUOM! Vaikka olisi joku p�ivitys tarve listalla, ei se tarkoita sit� ett� 
    // kyseinen kuva olisi mill��n n�yt�ll� valittuna. Eli voi tulla ilmoitus ett�
    // on tullut uusi meteosat9 HRV kanavan kuva, mutta sit� ei v�ltt�m�tt� ole mill��n
    // karttan�yt�n rivill� valittuna.
	...
	// T�m�n pit�� tehd� kaksoistoiminto, koska sek� Beta-tuotteet, ett�
	// MacroParam-datat k�ytt�v�t querydata trigger�intej� ja t��ll�
	// GetDataTriggerListOwnership -metodi ottaa sen hetkiset triggerit pois,
	// joten molemmat ty�t pit�� k�sitell� samalla kertaa
    ...
    std::shared_ptr<NFmiFastQueryInfo> GetModelClimatologyData(const NFmiLevel &theLevel)
    {
            ...
            if(NFmiFastInfoUtils::IsModelClimatologyData(info))
            {
				if(theLevel.GetIdent() != 0)
				{
					...
					if(info->Level(theLevel))
						...
						info->FirstLevel(); // Jos kyse oli pintadatasta, josta ei l�ydy haluttua leveli�, pit�� info laittaa osoittamaan taas pinta-leveliin
				}
				else
				{
    ...
    std::shared_ptr<NFmiFastQueryInfo> GetMosTemperatureMinAndMaxData()
    {
    ...
    std::vector<std::shared_ptr<NFmiFastQueryInfo>> FilterOnlyGridSurfaceNonYearLongData(std::vector<std::shared_ptr<NFmiFastQueryInfo>> &origDataVector)
    {
        std::vector<std::shared_ptr<NFmiFastQueryInfo>> filteredData;
    ...
    std::shared_ptr<NFmiFastQueryInfo> SeekMostFavoriteProducerData(std::vector<std::shared_ptr<NFmiFastQueryInfo>> &dataVector, const std::vector<unsigned long> &favoriteProducers, const NFmiLevel* actualLevel, const NFmiProducer &actualProducer)
    {
        if(dataVector.size())
        {
			std::shared_ptr<NFmiFastQueryInfo> backupData = nullptr;
			size_t backupDataIndex = 999;
			// Katsotaan ensin l�ytyyk� ihan t�ysosuma dataa
    ...
    std::shared_ptr<NFmiFastQueryInfo> GetBestSuitableModelFractileData(std::shared_ptr<NFmiFastQueryInfo> &usedOriginalInfo)
    {
        auto infoVector = itsSmartInfoOrganizer->GetInfos(NFmiInfoData::kClimatologyData);
		...
		// Jos ei ole annettu usedOriginalInfo:a, niin silloin haetaan p��s��nt�isesti Ec:n fraktiilidataa
	...
	// FixDataElapsedTimeInSeconds funktio korjaa yhden mahdollisen querydatoihin tehdyn aikav��renn�ksen.
	// QueryDatoihin talletetaan timeri joka laskee sen latauksesta k�ytt��n kuluneen ajan.
	// Kyseisen ajan avulla voidaan merkit� k�ytt�liittym�ss� alle 5 minuuuttia sitten ladatatut datat uusiksi
	...
	{
		// Katsotaan onko miss��n queryDatassa konffattuna ns. my�h�stymisaikaraja.
	...
	void SetupInfoOrganizerVirtualTime(const std::string &logMessage)
	{
		...
		{
			// P�ivitet��n normi editoidun datan muutoksiin liittyv�t n�yt�t (kartat, aikasarja, stationdata-taulukko, wind-taulukko)
			auto updatedViews = GetUpdatedViewIdMaskForEditingData();
			// Lis�t��n siihen viel� poikkileikkaus ja luotaus n�yt�t
	...
	{
		{
			// MacroParamSystem:in k�yt�n synkronointi p��lle
			std::lock_guard<std::mutex> macroParamSystemLock(itsMacroParamSystemMutex);
			// P�ivtet��n uuteen MacroParamSystem:iin ty�datan k�yt�ss� olevia asetuksia
	...
	bool IsMacroParamOk(std::shared_ptr<NFmiDrawParam>& theUsedDrawParam)
	{
		...
		std::shared_ptr<NFmiArea> area = NFmiAreaFactory::Create("latlon:-20, 20, -30, 30");
		int descTopIndex = 0;
	...
	// T�h�n talletetaan havaitut korruptoituneet n�ytt�makro nimet, ett� niiden avulla voidaan varoittaa k�ytt�ji� niist� viewMacro-dialogissa 
	...
	// Onko viimeisen editoidun datan tietokantaan l�hetyksen j�lkeen tullut operatiivista dataa k�ytt��n?
	...
	// T�m� viewMacro saadaan ladattua k�ytt��n SmartMetissa pikan�pp�imell� CTRL + F12. 
	...
	// flagi ett� k�ytet��nk� croppia vai ei. T�m� menee p��lle aikasarjaikkunasta.
	bool fUseCPGridCrop; 
	// T�ss� on tallessa latlon kulmapisteet croppi laatikosta (kartalle piirtoa varten)
	std::shared_ptr<NFmiArea> itsCPGridCropLatlonArea; 
	// T�ss� on tallessa latlon kulmapisteet sisemm�st� croppi laatikosta (kartalle piirtoa varten)
	std::shared_ptr<NFmiArea> itsCPGridCropInnerLatlonArea; 
	...
	// k�ytt�j� voi asettaa erikseen sallitaanko multi-threaddaavan koodin k�ytt� editoitaessa
	...
	// K�yt�ss� olevan CaseStudyn eri apudatojen tiedot ovat t�ss�
	std::shared_ptr<NFmiHelpDataInfoSystem> itsCaseStudyHelpDataInfoSystem; 
	...
	// T�h�n ladataan olemassa oleva CaseStudy-data k�ytt��n
	...
	// Sen j�lkeen pit�� MainFramessa timerill� k�yd� v�liajoin katsomassa onko tullut sopivaa dataa ladattavaksi, kunnes sit� l�ytyy.
	...
	// Erilaisissa paikoissa on aiemmin k�ytetty luotausn�yt�n v�risetti� hyv�kseen kuvaamaan eri k�yri� (aikasarja, trajektori, jne)
	// Nyt teen oman v�ri setin t�ll�iseen k�ytt��n, koska esim. LENiss� on v�ritetty kaikki luotaukset mustaksi
	...
	// n�kee, ett� pit��k� data tarkistaa viel� kun editoidaan tai dataa l�hetet��n tietokantaan.
	...
	// t�m� pit�� asettaa aina kun joku n�ytt� piirt�� isoviiva/contoureja. T�m�n avulla piirret��n
	...
	// luotauksia piirret��n karttan�yt�lle vain tiettyihin vakio painepinnoille
	...
	// joita on tarkoitus katsella luotaus-n�yt�ll�
	...
	// tietyss� tilanteessa CDokumentti voidaan pakottaa piirt�m��n aikasarjan�ytt�
	bool fMustDrawTimeSerialView; 
	// tietyss� tilanteessa CDokumentti voidaan pakottaa piirt�m��n poikkileikkausn�ytt�n�ytt�
	bool fMustDrawCrossSectionView; 
	// tietyss� tilanteessa CDokumentti voidaan pakottaa piirt�m��n luotausn�ytt�
	...
	// kaikilla konfiguraatioilla ei voi tallettaa ty�tiedostoa, eik� siit� tarvitse varoittaa, t�m� luetaan settingseist�
	bool fWarnIfCantSaveWorkingFile; 
	// poikkileikkausn�ytt� ikkunan clientti osan koko pikseleiss�
	...
	bool fShowMouseHelpCursorsOnMap; // kartta ruudukolla n�ytett�vien apukursorien tila, halutaan s��dett�v�ksi, koska mm. kontrollipiste ty�kalu vilkkuu viel� liikaa
	int itsLastSelectedSynopWmoId; // mik� on synop-taulukkon�ytt� ikkunassa viimeksi valitun aseman wmoid
	bool fMouseOnMapView; // onko hiiren kursori kartan p��ll� vai ei
	int itsSynopHighlightStationWmoId;
	bool fShowSynopHighlightStationMarker; // n�ytet��nk� merkki kartta n�yt�ss�, kun jokin asema on mahdollisesti valittu
	NFmiPoint itsSynopHighlightLatlon; // t�ss� latlon pisteess� oli viimeksi merkattu synop asema synop-teksti-taulukko n�yt�ss�
	bool fSynopDataGridViewOn; // onko synop teksti muodossa katselu ikkuna n�kyviss� vai ei
	bool fTimeSerialDataViewOn; // onko aikasarja n�ytt� p��ll� vai ei
	NFmiMTATempSystem itsMTATempSystem; // tieto luotaus moodista ja jos MTA-moodi p��ll�, tietoa luotausn�yt�ss� n�ytett�vist� luotauksista
	NFmiObsComparisonInfo itsObsComparisonInfo;
	bool fMapMouseDragPanMode; // ollaanko karttan�yt�ll� tekem�ss� pan:i� hiiren keski nappia vet�m�ll�
	...
	string itsLatestMacroParamErrorText; // kun macroParam piirret��n karttan�yt�lle, ja tulee virhe, talletetaan t�h�n aina viimeisin virhe ilmoitus
	std::shared_ptr<NFmiMacroParamSystem> itsMacroParamSystemPtr; // t�m� hanskaa macroParamit, joilla voi piirt�� smarttoolmacroja karttan�yt�lle
	...
	string itsCurrentMacroText; // t�ss� on tallessa viimeisin smarttool-dialogin teksti
								// k�ytet��n macroparam-n�yt�n testivaiheessa

	NFmiString itsSavedDirectory; // talletettu ty�hakemisto, mik� voidaan ladata k�ytt��n my�hemmin
    ...
    NFmiViewSettingMacro itsHelperViewMacro; // t�m� on apuna n�ytt�-makrojen latauksessa ja talletuksessa. Todelliset makrot ovat makrolistassa
	...
	std::shared_ptr<NFmiDrawParam> GetDrawDifferenceDrawParam(void){return itsDrawDifferenceDrawParam;}
	std::shared_ptr<NFmiDrawParam> itsDrawDifferenceDrawParam; // t�m�n avulla piirret�� toolmasterilla erotus kent�t eri parametreille
	std::shared_ptr<NFmiDrawParam> GetSelectedGridPointDrawParam(void){return itsSelectedGridPointDrawParam;}
	std::shared_ptr<NFmiDrawParam> itsSelectedGridPointDrawParam; // t�m�n avulla piirret��n toolmasterilla editoinnissa valittujen pisteiden joukko
	...
	bool fActivateParamSelectionDlgAfterLeftDoubleClick; // kikka 6, jotta zeditmap2view tiet��, pit��k� dialogi aktivoida

    // N�ytet��nk� erilaisia apudatoja aikasarjaikkunoissa?
    // 0 = ei apudatoja k�yt�ss�,
	...
	bool fCPDataBackupUsed; // Tarkistus bitti sille onko jostain syyst� pit�nyt lukea k�nnistyksen yhteydess� CP-datan backup. Tiedon avulla ei tarvitse lukea uudestaan Init vaiheessa CP-pisteit� (ja tuhota muutos k�yri�).
	...
	int itsToolTipRealRowIndex; // hiiren kursorin lepopaikan n�ytt�rivi tooltippi� varten
	int itsToolTipColumnIndex; // hiiren kursorin lepopaikan n�ytt�rivin sarake numero tooltippi� varten
	...
	int itsToolTipMapViewDescTopIndex; // Mist� karttan�yt�lt� tooltip otettiin
	std::shared_ptr<NFmiDrawParam> itsDefaultEditedDrawParam; // t�m�n jouduin ottamaan k�ytt��n kun upudatat
											  // alkoivat tuottaa ongelmia t�ll�inen luodaan aina
											  // uudesta datasta (esim. 1. parametri). Jos k�ytt�j�
											  // on laittanut esim apuparametrin n�ytt��n, ei ole
	...
	std::shared_ptr<NFmiAreaMaskList> itsParamMaskListMT; // universaali maskilista, olen tehnyt siit� nyt multi-threaddausta tukevan version. T�st� pit�� aina pyyt�� kopio ParamMaskList -metodilla
	int itsCurrentViewRowIndex; // jokaisella n�ytt�rivill� (ruudukossa) on indeksi ja
								// kun tehd��n valintoja popup-menun kautta, rivin indeksi
								// talletetaan t�h�n muuttujaan, ett� n�ytt�parametrien
								// muutokset saadaan tehty� oikeaan kohtaan drawparamlist-listiss�
	int itsCurrentCrossSectionRowIndex;
	int itsFilterFunction; // testi k�yt�ss� toistaiseksi 0 = avg, 1 = max ja 2 = min filtterit
...
//                                                  talletetaan keskimm�iseen hilaan
	...
	NFmiPoint itsTimeFilterLimits; // aikafiltter�inniss� olevien rajojen minimi ja maksimi arvot minuuteissa
	NFmiPoint itsTimeFilterRangeStart;	// aikasarjafiltter�innin aloitus rajat (x=alkuaikasiirtym� ja y=loppuaikasiirtym�) (muokkausdialogin 1. kohta)
	NFmiPoint itsTimeFilterRangeEnd; // (muokkausdialogin 2. kohta)
	NFmiMetTime itsActiveViewTime; // kun n�ytt�ruudukkoa klikataan hiirell�, yksi ruuduista muuttuu aktiiviseksi, sen ruudun aika talletetaan t�h�n
	int itsFilteringParameterUsageState; // onko valittuna 0=aktiivinen parametri, 1=kaikkiparametrit vai 2=valitutParametrit
	NFmiParamBag itsFilteringParamBag; // t�t� bagia aktivoidaan kun halutaan valita tietyt parametrit, jotka filtter�id��n
	...
	NFmiRect itsUpdateRect; // piirr� t�m� alue n�yt�st� uudelleen (ei ehk� tarvita)
	NFmiPoint itsBrushSize; // prosenteissa n�ytt�ruudun koosta
...
std::shared_ptr<NFmiDrawParam> NFmiEditMapGeneralDataDoc::DefaultEditedDrawParam(void)
{
...
{
// valitsee kaikki editoitavan parametrin paikat (PIT�ISI VALITA VAIN NE, JOTKA OVAT AKTIIVISIA!!)
	std::shared_ptr<NFmiFastQueryInfo> editedInfo = EditedSmartInfo();
	if(editedInfo)
	{
...
std::shared_ptr<NFmiFastQueryInfo> NFmiEditMapGeneralDataDoc::EditedSmartInfo(void)
...
std::shared_ptr<NFmiAreaMaskList> NFmiEditMapGeneralDataDoc::ParamMaskListMT(void)
{
...
std::shared_ptr<NFmiDrawParam> NFmiEditMapGeneralDataDoc::GetDrawDifferenceDrawParam(void)
{
	return pimpl->GetDrawDifferenceDrawParam();
}

std::shared_ptr<NFmiDrawParam> NFmiEditMapGeneralDataDoc::GetSelectedGridPointDrawParam(void)
{
...
void NFmiEditMapGeneralDataDoc::SnapShotData(std::shared_ptr<NFmiFastQueryInfo> &theInfo, const NFmiDataIdent &theDataIdent, const string &theModificationText
				 , const NFmiMetTime &theStartTime, const NFmiMetTime &theEndTime)
{
...
void NFmiEditMapGeneralDataDoc::AddMacroParamToView(unsigned int theDescTopIndex, int theViewRow, const std::string &theName) // lis�� halutun nimisen macroParamin halutun karttan�yt�n riville (1-5)
{
...
void NFmiEditMapGeneralDataDoc::SelectLocations(unsigned int theDescTopIndex, std::shared_ptr<NFmiFastQueryInfo> &theInfo, const std::shared_ptr<NFmiArea> &theMapArea, const NFmiPoint& theLatLon
						,const NFmiMetTime &theTime, int theSelectionCombineFunction, unsigned long theMask
...
std::shared_ptr<NFmiFastQueryInfo> NFmiEditMapGeneralDataDoc::GetNearestSynopStationInfo(const NFmiLocation &theLocation, const NFmiMetTime &theTime, bool ignoreTime, std::vector<std::shared_ptr<NFmiFastQueryInfo> > *thePossibleInfoVector, double maxDistanceInMeters)
{
...
std::vector<std::shared_ptr<NFmiFastQueryInfo> > NFmiEditMapGeneralDataDoc::GetSortedSynopInfoVector(int theProducerId, int theProducerId2, int theProducerId3, int theProducerId4)
{
...
std::string NFmiEditMapGeneralDataDoc::GetModelOrigTimeString(std::shared_ptr<NFmiDrawParam> &theDrawParam, int theIndex)
{
	return pimpl->GetModelOrigTimeString(theDrawParam, theIndex);
}

NFmiMetTime NFmiEditMapGeneralDataDoc::GetModelOrigTime(std::shared_ptr<NFmiDrawParam> &theDrawParam, int theIndex)
{
...
bool NFmiEditMapGeneralDataDoc::SetNearestBeforeModelOrigTimeRunoff(std::shared_ptr<NFmiDrawParam> &theDrawParam ,const NFmiMetTime &theTime, unsigned int theDescTopIndex, int theViewRowIndex)
{
	return pimpl->SetNearestBeforeModelOrigTimeRunoff(theDrawParam ,theTime, theDescTopIndex, theViewRowIndex);
}

void NFmiEditMapGeneralDataDoc::SetModelRunOffset(std::shared_ptr<NFmiDrawParam> &theDrawParam, int theMoveByValue, unsigned int theDescTopIndex, int theViewRowIndex)
{
...
bool NFmiEditMapGeneralDataDoc::StoreData(const std::string& theFileName, std::shared_ptr<NFmiFastQueryInfo> &theSmartInfo, bool askForSave)
{
...
const std::shared_ptr<NFmiArea> NFmiEditMapGeneralDataDoc::CPGridCropLatlonArea(void) 
{
	return pimpl->CPGridCropLatlonArea();
}

std::shared_ptr<NFmiArea> NFmiEditMapGeneralDataDoc::CPGridCropInnerLatlonArea(void)
{
...
void NFmiEditMapGeneralDataDoc::ZoomMapWithRelativeRect(int theMapViewDescTopIndex, std::shared_ptr<NFmiArea> &theMapArea, const NFmiRect &theZoomedRect)
{
    pimpl->ZoomMapWithRelativeRect(theMapViewDescTopIndex, theMapArea, theZoomedRect);
}

void NFmiEditMapGeneralDataDoc::PanMap(int theMapViewDescTopIndex, std::shared_ptr<NFmiArea> &theMapArea, const NFmiPoint &theMousePoint, const NFmiPoint &theZoomDragUpPoint)
{
    pimpl->PanMap(theMapViewDescTopIndex, theMapArea, theMousePoint, theZoomDragUpPoint);
}

void NFmiEditMapGeneralDataDoc::ZoomMapInOrOut(int theMapViewDescTopIndex, std::shared_ptr<NFmiArea> &theMapArea, const NFmiPoint &theMousePoint, double theZoomFactor)
{
...
std::shared_ptr<NFmiFastQueryInfo> NFmiEditMapGeneralDataDoc::GetModelClimatologyData(const NFmiLevel& theLevel)
{
    return pimpl->GetModelClimatologyData(theLevel);
}

std::shared_ptr<NFmiFastQueryInfo> NFmiEditMapGeneralDataDoc::GetBestSuitableModelFractileData(std::shared_ptr<NFmiFastQueryInfo>& usedOriginalInfo)
{
...
std::shared_ptr<NFmiFastQueryInfo> NFmiEditMapGeneralDataDoc::GetMosTemperatureMinAndMaxData()
{
...
void NFmiEditMapGeneralDataDoc::SetCPCropGridSettings(const std::shared_ptr<NFmiArea>& newArea, unsigned int mapViewDescTopIndex)
{
...
bool NFmiEditMapGeneralDataDoc::IsMacroParamOk(std::shared_ptr<NFmiDrawParam>& theUsedDrawParam) const
{ 
