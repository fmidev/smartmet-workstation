	...
	bool LoadViewMacro(const std::string &theName)
	{
		try
		{
            ...
            if(ReadViewMacro(*CurrentViewMacro(), initFileName))
            {
				...
				// T‰m‰ kutsu varmistaa ett‰ eri karttan‰ytˆiss‰ on oikeat kartan koko arvot tallessa.
				// Jos n‰yttˆmakro lataa saman koon jollekin kartalle, ei oikeita p‰ivitys juttuja kutsuta ollenkaan,
				// koska kartan pikseli koko ei ole muuttunut. T‰m‰ on todellinen ongelma, koska usein n‰yttˆjen koot
				// j‰‰v‰t edellisesta saman n‰yttˆmakron latauksesta. Jos t‰t‰ p‰ivityst‰ ei tehd‰, vaikuttaa se
				// joidenkin piirtoobjektien koon laskuihin (mm. v‰rilegendat ja erilaiset symbolit).
