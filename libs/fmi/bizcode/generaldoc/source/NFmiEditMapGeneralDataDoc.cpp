...
{
	boost::shared_ptr<NFmiDrawParam> drawParam = GetCombinedMapHandler()->activeDrawParam(0, GetCombinedMapHandler()->activeViewRow(0)); // tehd‰‰n vain p‰‰karttaikkunasta n‰it‰ talletuksia
	bool status = false;
	if(drawParam)
	{
...
#undef min // MSVC:n m‰‰rittelem‰ min-makro pit‰‰ disabloida
	...
	int DoAllProfilingAnimations()
	{
		...
		animationData.CurrentTime(mainMapViewDescTop.CurrentTime()); // currentti aika pit‰‰ ottaa desctopista ja antaa animaattorille
