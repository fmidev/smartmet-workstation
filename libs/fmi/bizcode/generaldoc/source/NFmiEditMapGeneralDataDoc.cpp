...
void MakeNeededDirtyOperationsWhenDataAdded(NFmiQueryData *theData, NFmiInfoData::Type theType, const NFmiTimeDescriptor &theRemovedDatasTimes, const std::string &theFileName)
{
    ...
    if(theType == NFmiInfoData::kKepaData)
    {
        ...
        if(flagChanged && ApplicationWinRegistry().ConfigurationRelatedWinRegistry().ShowLastSendTimeOnMapView())
            MapViewDirty(0, true, false, false, false, true); // 0 = vain p‰‰karttan‰yttˆ liataan, cacheja ei tarvitse tyhjent‰‰, koska t‰h‰n liittyv‰t jutut piirret‰‰n vain karttan‰ytˆn p‰‰lle, ei karttaruudukkoihin
...
void MakeNeededDirtyOperationsWhenDataAdded(unsigned int theDescTopIndex, NFmiFastQueryInfo &theInfo, NFmiInfoData::Type theType, const NFmiTimeBag &theDirtyViewTimes, const std::string &theFileName)
{
	...
	MapViewDirty(theDescTopIndex, true, false, false, false, false); // ei laiteta cachea likaiseksi
...
// Tutkii vertikaali macroParam funktioiden kanssa ett‰ jos kyse on pressure datasta, ett‰ lˆytyykˆ samalta tuottajalta myˆs hybrid dataa, miss‰ on haluttu parametri.
// Kaikissa muissa tapauksissa palauttaa true (eli tehd‰‰n rivin cache likaus), paitsi jos data oressure dataa ja lˆytyy vastaava hybrid data infoOrganizerista.
...
void MakeMapViewRowDirty(int theMapViewDescTopIndex, int theViewRowIndex)
{
    ...
    MapViewDirty(theMapViewDescTopIndex, true, false, false, false, false);
}
...
void SetModelRunDifference(const NFmiMenuItem &theMenuItem, int theViewRowIndex)
{
	...
	if(modifiedDrawParam)
	{
		...
		MapViewDirty(theMenuItem.MapViewDescTopIndex(), true, false, false, false, false);
	}
}
...
void ReloadAllDynamicHelpData()
{
	...
	// T‰m‰n j‰lkeen pit‰‰ laittaa datan luku threadi heti p‰‰lle
	// ylemm‰ll‰ tasolla eli CSmartMetDoc-luokassa, mist‰ t‰t‰ metodia on kutsuttukkin.
}
...
void ForceStationViewUpdate(unsigned int theDescTopIndex)
{
    if(theDescTopIndex <= CtrlViewUtils::kFmiMaxMapDescTopIndex)
    {
        // Liataan maphandler, jotta k‰ytˆss‰ karttan‰ytˆss‰ ollut stationView vaihtuu tarvittaessa jos esim. isoline piirto vaihtuu teksti esitykseen.
        MapViewDescTop(theDescTopIndex)->MapHandler()->MapDirty(true);
    }
}
...
void ReloadDrawParamSettings(const NFmiMenuItem& theMenuItem, int theRowIndex)
{
    ...
    ForceStationViewUpdate(theMenuItem.MapViewDescTopIndex());
}
    ...
    if(list)
    {
        if(list->Index(theParamIndex))
        {
            ...
            if(drawParam)
            {
                ...
                bool macroParamCase = drawParam->IsMacroParamCase(true);
                MapViewDirty(theDescTopIndex, true, true, false, macroParamCase, false);
                return true;
            }
        }
    }
...
void AddMask(const NFmiMenuItem& theMenuItem, int /* theRowIndex */ , bool fClearListFirst)
{
	...
	if(mask)
	{
		...
		if(dlg.DoModal() == IDOK)
		{
			...
			if(paramMaskList)
			{
				string logStr("Otetaan maski k‰yttˆˆn ");
...
void HideView(const NFmiMenuItem& theMenuItem, int theRowIndex)
{
	...
	if(drawParamList->Index(theMenuItem.IndexInViewRow()))
	{
		...
		drawParamList->Dirty(true);
		CheckAnimationLockedModeTimeBags(theMenuItem.MapViewDescTopIndex(), false); // kun parametrin n‰kyvyytt‰ vaihdetaan, pit‰‰ tehd‰ mahdollisesti animaatio moodin datan tarkistus
		MapViewDirty(theMenuItem.MapViewDescTopIndex(), true, true, false, false, false);
	}
	...
	else
	{
		...
		if(modifiedDrawParam)
		{
			...
			if(dlg.DoModal() == IDOK)
			{
				...
				return dlg.RefreshPressed(); // myˆs false:lla halutaan ruudun p‰ivitys, koska jos painettu p‰ivit‰-nappia ja sitten cancelia, pit‰‰ ruutu p‰ivitt‰‰
		}
	}
	return false;
}
...
void ChangeWmsMapType(unsigned int theDescTopIndex, bool fForward)
{
    ...
    MapViewDirty(theDescTopIndex, true, true, false, false, false);
#endif // DISABLE_CPPRESTSDK
}
...
bool ScrollViewRow(unsigned int theDescTopIndex, int theCount)
{
	...
	if(MapViewDescTop(theDescTopIndex)->ScrollViewRow(theCount, activeViewRow))
	{
		...
		MapViewDirty(theDescTopIndex, true, false, false, false, false);
		return true;
	}
	else
		return false;
...
void TimeControlTimeStep(unsigned int theDescTopIndex, float newValue)
{
	if(MapViewDescTop(theDescTopIndex)->TimeControlTimeStep() != newValue)
	{
        ...
        // pit‰‰ mahdollisesti piirt‰‰ uusiksi aikaerotus juttuja ja salama dataa (ja ehk‰ jotain muuta?), 
        // joten varmuuden vuoksi laitan aina v‰limuistin likaiseksi
		MapViewDirty(theDescTopIndex, false, true, false, false, false);
	}
}
...
//		theViewMacro.SetAllExtraRowParams(descTop.ExtraDrawParamListVector(), itsMacroParamSystem);
	...
	{ // ota k‰yttˆˆn kaikki makron asetukset ja tee n‰ytˆist‰ 'likaisia'
		...
		MapViewDirty(CtrlViewUtils::kDoAllMapViewDescTopIndex, true, true, true, false, false);
		unsigned int ssize = static_cast<unsigned int>(itsMapViewDescTopList.size());
		for(unsigned int i = 0; i<ssize; i++)
		{
...
void ClearMacroParamDataCache(unsigned int theDescTopIndex, bool clearMacroParamDataCache, bool clearEditedDataDependentMacroParamDataCache)
{
    if(clearMacroParamDataCache)
        MacroParamDataCache().clearView(theDescTopIndex);
    if(clearEditedDataDependentMacroParamDataCache)
...
void MapViewDirty(unsigned int theDescTopIndex, bool mapDirty, bool clearCache, bool areaViewDirty, bool clearMacroParamDataCache, bool clearEditedDataDependentMacroParamDataCache)
{
    ...
    if(theDescTopIndex == CtrlViewUtils::kDoAllMapViewDescTopIndex)
        MapViewDirtyForAllDescTops(mapDirty, clearCache, areaViewDirty, clearMacroParamDataCache, clearEditedDataDependentMacroParamDataCache);
    else
    {
        MapViewDescTop(theDescTopIndex)->MapViewDirty(mapDirty, clearCache, areaViewDirty);
        ClearMacroParamDataCache(theDescTopIndex, clearMacroParamDataCache, clearEditedDataDependentMacroParamDataCache);
    }
}

void MapViewDirtyForAllDescTops(bool mapDirty, bool clearCache, bool areaViewDirty, bool clearMacroParamDataCache, bool clearEditedDataDependentMacroParamDataCache)
{
    unsigned int ssize = static_cast<unsigned int>(itsMapViewDescTopList.size());
    for(unsigned int viewIndex = 0; viewIndex < ssize; viewIndex++)
    {
        MapViewDescTop(viewIndex)->MapViewDirty(mapDirty, clearCache, areaViewDirty);
        ClearMacroParamDataCache(viewIndex, clearMacroParamDataCache, clearEditedDataDependentMacroParamDataCache);
    }
}
...
void SetSelectedMapHandler(unsigned int theDescTopIndex, unsigned int newMapIndex)
{
	...
	MapViewDirty(theDescTopIndex, true, true, true, false, false);
}
...
void SetMapArea(unsigned int theDescTopIndex, const boost::shared_ptr<NFmiArea> &newArea)
{
	if(newArea)
	{
		...
		MapViewDirty(theDescTopIndex, true, true, true, true, false); 
...
void SwapArea(unsigned int theDescTopIndex)
{
	...
	mapDescTop->BorderDrawDirty(true);
	MapViewDirty(theDescTopIndex, true, true, true, true, false); // laitetaan viela kaikki ajat likaisiksi cachesta
	mapDescTop->GridPointCache().Clear();
}
	...
	void RemoveMacroParam(const std::string &theName)
	{
		...
		else
		{
			...
			MapViewDirty(CtrlViewUtils::kDoAllMapViewDescTopIndex, false, true, false, false, false); // laitetaan viela kaikki ajat likaisiksi cachesta
		}
	}
...
//				MakeEditorDirty(theDescTopIndex, true, true, true);
			...
			else
			{
				...
				GetMTATempSystem().AddTemp(tempInfo);
				MapViewDirty(theDescTopIndex, true, true, false, false, false);
				theRedrawMapAfterMTATempClear = true; // h‰t‰ paska viritys optimoinnin takia
			}
		}
	...
	{
		InfoOrganizer()->ClearData(NFmiInfoData::kTEMPCodeSoundingData); // tyhjennet‰‰n TEMP-data
		// Karttan‰yttˆ on piirrett‰v‰ uudestaan, koska siell‰ saattaa n‰ky‰
		// uusia luotaus merkkej‰.
		MapViewDirty(CtrlViewUtils::kDoAllMapViewDescTopIndex, true, true, false, false, false); // laitetaan viela kaikki ajat likaisiksi cachesta
	}
	...
	void BorrowParams(unsigned int theDescTopIndex, int row)
	{
		...
		if(drawParamListFrom && drawParamListTo && (drawParamListFrom != drawParamListTo))
		{
			...
			MapViewDirty(theDescTopIndex, true, true, false, false, false); // laitetaan viela kaikki ajat likaisiksi cachesta
	...
	void OnShowGridPoints(unsigned int theDescTopIndex)
	{
		...
		MapViewDirty(theDescTopIndex, true, true, false, false, false); // t‰m‰ laittaa cachen likaiseksi
	...
	void OnHideParamWindow(unsigned int theDescTopIndex)
	{
		...
		MapViewDirty(theDescTopIndex, true, false, false, false, false); // laitetaan kartta likaiseksi
	...
	void OnButtonRefresh(void)
	{
		...
		MapViewDirty(CtrlViewUtils::kDoAllMapViewDescTopIndex, true, true, true, false, false); // laitetaan kartta likaiseksi
        MacroParamDataCache().clearAllLayers();
	...
	void OnShowProjectionLines(void)
	{
		...
		if(projInfo)
		{
			...
			MapViewDirty(CtrlViewUtils::kDoAllMapViewDescTopIndex, true, true, false, false, false);
	...
	void OnAcceleratorToggleKeepMapRatio(void)
	{
		...
		for(size_t i = 0; i<itsMapViewDescTopList.size(); i++)
		{
			...
			MapViewDirty(static_cast<unsigned int>(i), true, true, true, newKeepAspectRatioState, false);
...
#undef min // MSVC:n m‰‰rittelem‰ min-makro pit‰‰ disabloida
				...
				if(theDescTopIndex == CtrlViewUtils::kDoAllMapViewDescTopIndex || i == theDescTopIndex)
				{
					...
					if(lockMode == NFmiAnimationData::kFollowLastObservation || lockMode == NFmiAnimationData::kFollowEarliestLastObservation)
					{
						...
						{ // lˆytyi jotain ennusteita, jossa uusi lastTime (=newLastTime), joten voidaan p‰ivitt‰‰ animaation timebagi.
							...
							MapViewDirty(static_cast<unsigned long>(i), true, false, false, false, false); // t‰m‰n pit‰isi asettaa n‰yttˆ p‰ivitys tilaan, mutta cachea ei tarvitse en‰‰ erikseen tyhjent‰‰
						}
					}
				}
			}
	...
	void ToggleTimeControlAnimationView(unsigned int theDescTopIndex)
	{
		...
		if(descTop)
		{
			...
			MapViewDirty(theDescTopIndex, true, false, false, false, false); // t‰m‰n pit‰isi asettaa n‰yttˆ p‰ivitys tilaan, mutta cachea ei tarvitse en‰‰ erikseen tyhjent‰‰
			...
			if(aList)
			{
				for(aList->Reset(); aList->Next(); )
				{
                    ...
                    if(!drawParam->IsParamHidden())
                    {
                        ...
                        {
                            MapViewDirty(theDescTopIndex, true, false, false, false, false); // liataan mapView, mutta ei viel‰ t‰ll‰ rivill‰ cachea (menisi kaikki rivit kerralla)
                            descTop->MapViewCache().MakeRowDirty(cacheRowNumber);// clean cache row
...
*/
	...
	void OnButtonRedo(void) 
	{
		...
		if(smartInfo && smartInfo->Redo())
		{
			if(RedoData())
			{
				MapViewDirty(CtrlViewUtils::kDoAllMapViewDescTopIndex, false, true, false, false, true);
    ...
    void ZoomMapWithRelativeRect(int theMapViewDescTopIndex, boost::shared_ptr<NFmiArea> &theMapArea, const NFmiRect &theZoomedRect)
    {
	    ...
	    if(newZoomedArea)
	    {
		    ...
		    MapViewDirty(theMapViewDescTopIndex, true, true, true, true, false);
		    MapViewDescTop(theMapViewDescTopIndex)->BorderDrawDirty(true);
		    SetMapArea(theMapViewDescTopIndex, newZoomedArea);
	    }
	    MapViewDirty(theMapViewDescTopIndex, true, false, true, false, false); // t‰m‰ laittaa karttan‰ytˆn cachen sile‰ksi
   }
...
void NFmiEditMapGeneralDataDoc::MapViewDirty(unsigned int theDescTopIndex, bool mapDirty, bool clearCache, bool areaViewDirty, bool clearMacroParamDataCache, bool clearEditedDataDependentMacroParamDataCache)
{
    pimpl->MapViewDirty(theDescTopIndex, mapDirty, clearCache, areaViewDirty, clearMacroParamDataCache, clearEditedDataDependentMacroParamDataCache);
}
