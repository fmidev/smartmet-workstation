...
namespace
{
	...
	// 2) Mutex lukon turvissa vaihdetaan swap:illa wmsSupport_ ja tmpWmsSupport_ olioiden sisältä
...
// Jos palautettavan timebagin resoluutio on 0, ei timebagia käytetä jatkossa.
	...
	// HUOM! tämä ei laita päivityksiä päälle Parameter-selection dialogille, se hoidetaan toista kautta
	...
	void setMacroParamDrawParamSettings(const NFmiMenuItem& menuItem, boost::shared_ptr<NFmiDrawParam>& drawParam)
	{
		...
		{
			drawParam->ParameterAbbreviation(menuItem.MenuText()); // macroParamin tapauksessa pitää nimi asettaa tässä (tätä nimilyhennettä käytetään tunnisteena myöhemmin!!)
...
void NFmiCombinedMapHandler::doCutBorderDrawInitialization()
{
	{ 
		...
		auto &mapHandlerList = mapViewDescTops_[0]->GdiPlusImageMapHandlerList(); // otetaan pääkarttanöäytön mapHandlerList
			...
			if(mapViewDescTopIndex == CtrlViewUtils::kDoAllMapViewDescTopIndex || checkedDescTopIndex == mapViewDescTopIndex)
			{
				...
				if(lockMode == NFmiAnimationData::kFollowLastObservation || lockMode == NFmiAnimationData::kFollowEarliestLastObservation)
				{
					...
					{ // pitää fiksata viimeistä aikaa, koska se ei oikeastaan voi olla tulevaisuudessa, tai joku havainto on tulevaisuudesta
						...
						lastTime.ChangeByHours(-2); // lähdetään etsimään viimeistä havaintoa 3 tuntia menneisyydestä
					}
...
// Tutkii kaikki näkyvät havainto datat ja etsii sen ajan, mikä on aikaisin eri datojen viimeisistä ajoista.
...
void NFmiCombinedMapHandler::doAutoZoom(unsigned int mapViewDescTopIndex)
{
	...
	{
		auto* mapViewDescTop = getMapViewDescTop(mapViewDescTopIndex);
		auto* mapHandler = mapViewDescTop->MapHandler();
		boost::shared_ptr<NFmiArea> oldArea = mapHandler->Area();
		if(oldArea)
		{
			...
			{ // pitää muuttaa zoomattua areaa niin, että sen aspectratio vastaa ikkunan aspectratiota
				...
				if(newArea)
				{
					// Aspect ratio correction is automatic/internal — preserve itsSwapMode so that
					// SwapArea's mode=1 is not clobbered before the user's second SPACE press.
					int savedSwapMode = mapHandler->SwapMode();
					setMapArea(mapViewDescTopIndex, newArea);
					mapHandler->SwapMode(savedSwapMode);
				}
			}
		}
	}
...
// Liataan vain 1. näkyvät karttarivit niistä karttanäytöistä, missä näyttö-maski on päällä
...
void NFmiCombinedMapHandler::makeSwapBaseArea(unsigned int mapViewDescTopIndex)
{
	...
	// täällä ei tarvitse liata mitään eikä päivittää mitään
}
...
void NFmiCombinedMapHandler::swapArea(unsigned int mapViewDescTopIndex)
{
	...
	// sitten vielä tarvittavat likaukset ja päivitykset
	mapDescTop->SetBorderDrawDirtyState(CountryBorderDrawDirtyState::Geometry);
...
// Kun ollaan hiiren kanssa Näytö-ikkunan päällä (karttanäytössä) ja rullataan
