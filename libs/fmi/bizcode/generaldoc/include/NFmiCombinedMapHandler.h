...
class NFmiCombinedMapHandler : public CombinedMapHandlerInterface
{
    ...
    // **************** Uudet country border datat, joita työtetään Gdal:in avulla **********************
    std::string countryBorderShapeFile_;
    // Tässä on 'raaka' wgs84 datumissa oleva latlon koordinaatistossa oleva rajaviiva shape data
    std::unique_ptr<OGRGeometryCollection> globalBaseCountryBorderGeometry_;
    // Tähän lasketaan jokaiseen eri karttapohjaan (kartat 1-4) leikatut rajaviivat. Näillä voidaan optimoida zoomaukseen käytettyjä rajaviiva laskuja.
