...
class NFmiLevel;
class NFmiArea;
class NFmiEditMapDataListHandler; //laura lis�si 30081999
#ifndef UNIX
class CDC; // toivottavasti vain v�liaikaisesti/Marko
class CBitmap;
#endif // UNIX
...
{
public:
	bool IsMacroParamOk(std::shared_ptr<NFmiDrawParam>& theUsedDrawParam) const;
	...
	bool StoreData(const std::string& theFileName, std::shared_ptr<NFmiFastQueryInfo> &theSmartInfo, bool askForSave);
	...
	void DrawOverBitmapThings(NFmiToolBox * theGTB); // t�m� on kirjastojen pilkkomiseen vaadittuja funktioita
	...
	bool ShowWaitCursorWhileDrawingView(void); // tarkasta CView-n�ytt�luokissa, onko mahdollisesti animaatiota k�ynniss�. Jos on, �l� laita odota-cursoria n�kyviin, koska se vilkuttaa ik�v�sti
	...
	void RemoveMacroParam(const std::string &theName); // poistaa halutun macroparamin dokumentista, tiedostoista ja n�yt�ilt�
	void AddMacroParamToView(unsigned int theDescTopIndex, int theViewRow, const std::string &theName); // lis�� halutun nimisen macroParamin halutun karttan�yt�n riville (1-5)
	void AddMacroParamToCrossSectionView(int theViewRow, const std::string &theName); // lis�� halutun nimisen macroParamin halutun karttan�yt�n riville (1-5)
	std::shared_ptr<NFmiMacroParamSystem> MacroParamSystem(void);
	// N�m� makro tekstin get ja set metodit on makroparam-n�ytt�� varten tehtyj� virityksi�
	void SetCurrentSmartToolMacro(const std::string& theMacroText);
	const std::string& GetCurrentSmartToolMacro(void);

	bool ChangeToEditorsWorkingDirectory(void); // asettaa editorin hakemiston ja aseman k�ytt��n
	bool SaveCurrentPath(void); // ottaa nykyisen ty�hakemiston ja aseman talteen
	bool LoadSavedPath(void); // lataa edellisess� talletetut polut ja asemat k�ytt��n
	...
	std::shared_ptr<NFmiDrawParam> GetDrawDifferenceDrawParam(void);
	std::shared_ptr<NFmiDrawParam> GetSelectedGridPointDrawParam(void);
	...
	void StoreSupplementaryData(void); // tallettaa mm. CP pisteet, muutosk�yr�t jne.
	...
	bool DoTimeSeriesValuesModifying(std::shared_ptr<NFmiDrawParam> &theModifiedDrawParam, int theUsedMask, NFmiTimeDescriptor& theTimeDescriptor, std::vector<float> &theModificationFactorCurvePoints, NFmiMetEditorTypes::FmiUsedSmartMetTool theEditorTool, bool fUseSetForDiscreteData, int theUnchangedValue = -1);
	...
	NFmiPoint ActualMapBitmapSizeInPixels(unsigned int theDescTopIndex); // laskee n�ytt�ruudukon yhden ruudun koon pikseleiss�
	std::shared_ptr<NFmiFastQueryInfo> EditedSmartInfo(void);
...
private:
	...
	GeneralDocImpl *pimpl; // t�h�n laitetaan kaikki toiminnot dokumentista!!!
