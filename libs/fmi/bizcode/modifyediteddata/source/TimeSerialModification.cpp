...
{
    // 1. Kaikkien karttan�ytt�jen image cachet tyhjiksi, pakotetaan piirto ja editoituun dataan riippuvaisten cachejen (macroParam) tyhjennys
    theAdapter.MapViewDirty(CtrlViewUtils::kDoAllMapViewDescTopIndex, false, true, true, false, clearEditedDataRelatedCaches, false);
    // 2. Optimoidaan smartmetin n�ytt�jen p�ivityst�, vain ne n�yt�t pit�� p�ivitt��, jotka voivat k�ytt�� editoitua dataa
...
static std::shared_ptr<NFmiAreaMaskList> CreateFilteringMaskList(TimeSerialModificationDataInterface &theAdapter, std::shared_ptr<NFmiFastQueryInfo> &theCopyOfEditedData)
{
	...
	if(maskList == 0)
		maskList = std::shared_ptr<NFmiAreaMaskList>(new NFmiAreaMaskList()); // luodaan aina tyhj� lista, vaikka maskeja ei olisi k�yt�ss�
	if(maskList)
...
{
	std::shared_ptr<NFmiDrawParam> drawParam = theAdapter.ActiveDrawParamFromActiveRow(0); // n�m� moukkausty�kalulla (filtteri ty�kalu) tehdyt muokkaukset tehd��n aina p��karttaikkunan aktiiviselle datalle
			...
			if(::CheckIfSpeedLoadFileHasSimilaGridAsWanted(theAdapter, theLoadedFile))
			{
				...
				if(data)
				{
					if(!theAdapter.UseEditedDataParamDescriptor() || theAdapter.EditedDataParamDescriptor() == data->Info()->ParamDescriptor())
					{
						...
						if(timeDescriptor == data->Info()->TimeDescriptor()) // jos viel� ladattavat ajat ovat samat kuin ladattavassa datassa, ei tarvitse kuin ottaa data k�ytt��n
						{
...
static QueryDataVector LoadAndCreateSelectedQueryData(TimeSerialModificationDataInterface &theAdapter, const std::vector<NFmiString>& theSelectedFileNames)
{
	...
	for(int i=0; i<size; i++)
	{
		...
		catch(std::exception &e)
		{
			...
			// Pakko laittaa tyhj� listaan, jatko olettaa ett� sellainen l�ytyy
