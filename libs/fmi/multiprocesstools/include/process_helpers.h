...
{
    if(!client)
    { // client ei saa yritt�� ensin tuhota ty�jonoja eik� my�sk��n master, koska jos data on jo k�yt�ss� toisaalla, t�m� tuhoaa sen ilman sen kummempia tarkasteluja
...
//            fill_work_queue(workqueue, allocator_holder, running_task_index);
    ...
    // T�m� on client-konstruktori (SmartMetille)
...
const std::size_t killer_work_time_in_ms = 3*5; // t�m�n pituinen ty� tappaa workerprosessin testausmieless� do_some_fake_work-funktiossa
...
bool do_some_fake_work(WorkerProcessStuff &workerProcessStuff)
{
    ...
    if(task)
    {
        ...
        if(work_time_in_ms == killer_work_time_in_ms)
            throw 1; // tietyn pituiset fake teht�v�t kaatavat workerin
