...
public:
    ...
    // Aina kun joku tekee jotain ty�t� tai kun saadaan uusia t�it�, t�t� pit�� kutsua.
...
private:
    ...
    bool transmission_incomplete_; // t�m�n avulla tiedet��n onko saapuva l�hetys luettu loppuun, vai pit��k� fixed_size_read_buffer_ lis�t� v�lill� complete_message_from_read:iin
    std::vector<char> write_buffer_; // t�m�n kokoa muutetaan tarvittaessa, jos l�hetett�v� viesti olisi muuten liian iso
    bool started_;
    std::string username_;
    bool actual_client_connection_; // Jos t�m� on false, ollaan yhteydess� oikeaan workeriin ja jos true, ollaan yhteydess� oikeaan clientiin (esim. SmartMet)
    std::string client_guid_; // eri clientit ja niiden ty�t tunnistetaan t�m�n avulla (saadaan username:sta)
    boost::asio::deadline_timer timer_; // jokaisella yhteydell� on oma timer
    ...
    static boost::asio::deadline_timer static_stopping_service_timer_; // tehd��n luokan staattisille funktioille oma yhteinen timer (stopping_service)
    static boost::asio::deadline_timer static_check_keyboard_timer_; // tehd��n luokan staattisille funktioille omat timerit (check_keyboard)
    static boost::asio::deadline_timer static_check_idle_timer_; // tehd��n luokan staattisille funktioille omat timerit (idle)
    static boost::asio::deadline_timer static_clean_old_results__timer_; // tehd��n luokan staattisille funktioille omat timerit (idle)
    static boost::asio::deadline_timer static_check_workers_running_timer_; // tehd��n luokan staattisille funktioille omat timerit (idle)
    static bool do_time_out_checks_; // Kun debugataan systeemi�, ei haluta tehd� timeout testej�, jolloin t�m� laitetaan false:ksi.
    
    // N�it� staattisia muutujia k�ytet��n k�ynnist�m��n kuollutta workeri� uudestaan eloon.
    // N�m� on tarkoitus asettaa startup_workers -metodilla (joka my�s k�ynnist�� halutun m��r�n workereit� alkuun).
    static int worker_count_; // Kuinka monta workeri� halutaan ty�skentelem��n (koneen s�ikeiden m��r� on hyv� arvaus)
    static std::string worker_base_name_; // T�st� tulee workerin useaname:n pohja esim. worker -> worker#1
    ...
    static bool add_command_line_flags_; // Dos worker ohjelmassa ei ole optioiden purkua, joten sinne annetaan argumentit per�kk�in. MFC-versiolle pit�� argumenteissa olla kirjain optiot, jotka lis�t��n erikseen tarvittaessa.
    ...
    size_t on_check_ping_counter; // lasketaan kuinka monta kertaa ollaan on_check_ping -metodissa ja tehd��n vain joka n. tarkistukselle lokitus (muuten niit� voi tulla kerran sekunnissa n kpl workerilt�)
    const static int no_work_client_ping_time_out_in_ms_ = 5*1000; // kuinka kauan maksimissaan odotetaan clientin pingi� ennen kuin lopetetaan yhteys (vaihtelee no-work 5000 vs. work tiloissa 50 * 1000)
    const static int working_client_ping_time_out_in_ms_ = 50*1000;
    const static int no_work_server_time_out_in_ms_ = 5*60*1000; // kuinka kauan maksimissaan odotetaan uusia t�it� ennen kuin lopetetaan serverin k�ynniss� olo (5 minuuttia)
};
