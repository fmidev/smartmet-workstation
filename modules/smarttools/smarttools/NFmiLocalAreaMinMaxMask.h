...
struct LocalExtreme
{
    ...
    bool fIsMinimum;
    // Indeksi sille kuinka merkittävä lokaali min/max oli (kuinka laaja-alainen ja kuinka syvä/korkea)
    ...
    // Lasketaan 8 ilmansuunnan extreme area piituuksien ja niiden keskiarvon välistä eroa ja lasketaan niistä 
    // kerroin, joka on välillä 0-1. Käytetään laskuissa keskihajonnan kaavaa ja lasketaan siitä 0-1 arvoväli.
    // 1 tarkoittaa täydellistä symmetriaa (alue on ympyrä) ja 0 epäsymmetriaa (jonkinlainen kapea ellipsi ehkä).
    // Tällä kertoimella kerrotaan lopullinen significance.
    float itsSymmetryIndex = 0;
    // Tämä poisto flagi on tarkoitettu helpottamaan koodia, millä poistetaan toisiaan liian lähellä olevia ääripisteitä.
    // Jos tämä on false (normaali tila), tämä ääripiste on tarkoitus ainakin toistaiseksi pitää mukana.
    // Jos true, se on tarkoitus poistaa tulos vektorista ja sitä ei tarvitse ottaa enää huomioon näissä laskuissa.
...
protected:
    ...
    // 1. arvo itsArgumentVector:ista, tällä määrätään kuinka moneen osaan laskut jaetaan ja kuinka suuria äärialueiden pitää minimissään olla
