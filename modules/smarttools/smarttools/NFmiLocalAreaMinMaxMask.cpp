...
double NFmiLocalAreaMinMaxMask::Value(const NFmiCalculationParams &theCalculationParams, bool fUseTimeInterpolationAlways)
{
...
void NFmiLocalAreaMinMaxMask::DoCalculationCheck(const NFmiCalculationParams &theCalculationParams)
{
    ...
    {
        // Tästä saa edetä vain yksi säie kerrallaan, eli joku joka ehtii, laskee cache-matriisin ja asettaa sen
        // itsCurrentGriddedStationData:n arvoksi, muut sitten vain käyttävät sitä
...
public:
    ...
    void checkForExtremes(float value, unsigned long gridPointX, unsigned long gridPointY, const NFmiPoint &latlon)
    {
...
private:
    ...
    static void checkForExtreme(float value, unsigned long gridPointX, unsigned long gridPointY, const NFmiPoint &latlon, LocalExtreme &localExtreme, const CheckingPredicate &predicate)
    {
...
{
    // Laske jokaiselle pää- ja sivuilmansuunnalle että kuinka monta hilapistettä lokaali max/min jatkuu
    std::vector<int> lengthInGridPoints;
    // Hae arvot äärialueen reunoilta, jotta niitä voidaan verrata ääripisteessä olevaan arvoon ja laskea voimakkuus
...
public:
    ...
    int& LengthInGridPoints(SearchDirections searchDirection) { return lengthInGridPoints[searchDirection]; }
    float& ValueOnExtremeAreaEdge(SearchDirections searchDirection) { return  valueOnExtremeAreaEdge[searchDirection]; }
    int& ContinueIterations(SearchDirections searchDirection) { return  continueIterations[searchDirection]; }
    int& SearchReachedToDataEdge(SearchDirections searchDirection) { return  searchReachedToDataEdge[searchDirection]; }
    ...
    static double GetGridSizeInKM(SearchDirections searchDirection, const NFmiPoint &gridPointSizeInKM)
    {
        switch(searchDirection)
        {
        case SearchDirection_N:
    ...
    float CalcAreaSizeAvg(const NFmiPoint &gridPointSizeInKM) const
    {
        ...
        for(int searchDirection = 0; searchDirection <= LastSearchDirection; searchDirection++)
        {
            ...
            auto gridSizeInKM = GetGridSizeInKM(static_cast<SearchDirections>(searchDirection), gridPointSizeInKM);
            avg.Calculate(static_cast<float>(gridSizeFactor * gridSizeInKM * lengthInGridPoints[searchDirection]));
        }
    ...
    float CalcDeepnessAvg(float localExtremeValue) const
    {
        ...
        {
            // On mahdollista että väli-väli-ilmansuunnissa on puuttuvaa (jos ollaan tarpeeksi lähellä datan reunaa) ja silti halutaan pitää ääripiste mukana
    ...
    {
        int hitCounter = 0;
        for(size_t searchDirectionIndex = 0; searchDirectionIndex < lengthInGridPoints.size(); searchDirectionIndex++)
        {
            if(lengthInGridPoints[searchDirectionIndex] <= lengthLimit && !searchReachedToDataEdge[searchDirectionIndex])
                hitCounter++;
    ...
    int FindContinuousUnderLimitCount(int lengthLimit)
    {
        ...
        // käsitellään tilanne jossa mennään rajan alle vektorin lopussa ja siitä tullaan pois vektorin alussa.
        for(size_t index = 0; index < lengthInGridPoints.size() * 2; index++)
        {
            auto actuallyUsedIndex = index % lengthInGridPoints.size();
            if(lengthInGridPoints[actuallyUsedIndex] <= lengthLimit && !searchReachedToDataEdge[actuallyUsedIndex])
            {
                if(!isUnderLimit)
                {
                    isUnderLimit = true;
    ...
    bool DiscardOneSidedLocalExtremes(float localExtremeValue, double gridPointsInSearchRange)
    {
        ...
        // Hylätään extreme alueet, missä alueen reunat tulee liian nopeasti joiltain reunoilta vastaan (datan oikeilla reunoille menemistä ei lasketa mukaan)
        // Tutkitaan tapaus jossa 1 suunnasta tulee reuna liian nopeasti vastaan
        //auto gridPointLengthLimitFor1Direction = std::max(1, boost::math::iround(gridPointsInSearchRange / 30.));
        //if(HasTooOneSidedLengthVector(gridPointLengthLimitFor1Direction, 1))
        //    return true;

        //auto gridPointLengthLimitFor2Direction = std::max(1, boost::math::iround(gridPointsInSearchRange / 20.));
        //if(HasTooOneSidedLengthVector(gridPointLengthLimitFor2Direction, 2))
        //    return true;

        //auto gridPointLengthLimitFor3Direction = std::max(1, boost::math::iround(gridPointsInSearchRange / 15.));
        //if(HasTooOneSidedLengthVector(gridPointLengthLimitFor3Direction, 3))
        //    return true;

        //auto gridPointLengthLimitFor4Direction = std::max(1, boost::math::iround(gridPointsInSearchRange / 10.));
    ...
    // Tutkitaan jatkuuko trendi kun edetään johonkin ilman suuntaan eli arvojen pitää koko ajan laskea (jos kyse oli maksimista)
    // ja nousta (jos kyse oli minimistä). 
    // Jos previousGridPointValue on kFloatMissing, silloin ollaan vielä 1. kierroksella ja pitää käyttää itse localExtreme:sta saatavaa arvoa.
    static bool ExtremeAreaContinues(const LocalExtreme &localExtreme, float value, float previousGridPointValue)
    {
        if(value != kFloatMissing)
        {
            auto usedBaseValue = (previousGridPointValue == kFloatMissing) ? localExtreme.itsValue : previousGridPointValue;
        ...
        if(continueIterations[searchDirection])
        {
            if(NFmiFastInfoUtils::SetInfoToGridPoint(info, checkGridPointX, checkGridPointY))
            {
                ...
                if(ExtremeAreaContinues(localExtreme, value, valueOnExtremeAreaEdge[searchDirection]))
                {
                    ...
                    // Väli-väli ilmansuunnille pitää lisätä tuplana arvoa, koska niitä tarkastellaa vain joka toisella kierroksella
...
// Jos löydetty lokaali piste onkin originaali datan reunalla, sitä ei voi pitää aitona lokaalina min/max pisteenä,
// koska datan jatkuessa (jos data leikattu esim. skandi alueelle) voisi löytyä aito ääripiste kauempaa.
static bool IsExtremePointOnEdge(const LocalExtreme &localExtreme, boost::shared_ptr<NFmiFastQueryInfo> &info)
{
    if(localExtreme.itsOrigDataGridPoint.X() == 0 || localExtreme.itsOrigDataGridPoint.Y() == 0)
        return true;
    if(localExtreme.itsOrigDataGridPoint.X() == info->GridXNumber() - 1 || localExtreme.itsOrigDataGridPoint.Y() == info->GridYNumber() - 1)
        return true;
    return false;
}
...
static bool CalcExtremesAreaSizeIndex(LocalExtreme &localExtreme, boost::shared_ptr<NFmiFastQueryInfo> &info, const NFmiMetTime &interpolationTime, float localAreaSearchRangeInKm)
{
    ...
    unsigned long maxGridPointOffset = boost::math::lround(std::max(4., localAreaSearchRangeInKm / avgGridSizeInKm));
    if(NFmiFastInfoUtils::SetInfoToGridPoint(info, centralGridPointX, centralGridPointY))
    {
        ...
        for(unsigned long gridPointOffset = 1; gridPointOffset < maxGridPointOffset; gridPointOffset++)
        {
            ...
            if(!anyDirectionContinues)
                break;
            // Jos jo 1. kierroksella millä tahansa suunnalla ei enää jatketa, ei kyseessä ollut oikea lokaali ääriarvo
            if(gridPointOffset == 1 && localExtremeSearcherData.CheckIsAnyDirectionStopped())
                return false;
        }
        if(localExtremeSearcherData.DiscardOneSidedLocalExtremes(localExtreme.itsValue, gridPointsInSearchRange))
            return false;
...
public:
    ...
    float OverPercentage() const
    {
        return CalculatePercentage(itsTotalCounter, itsOverValueCounter);
    }
...
static void AddAdditionalLocalExtreme(std::vector<LocalExtreme> &localExtremesInOut, float centerValue, int subGridOrigoX, int subGridOrigoY, int columnIndex, int rowIndex, bool isMinimum)
{
    LocalExtreme localExtreme;
    localExtreme.itsValue = centerValue;
    localExtreme.fIsMinimum = isMinimum;
    localExtreme.itsOrigDataGridPoint = NFmiPoint(static_cast<double>(subGridOrigoX + columnIndex), static_cast<double>(subGridOrigoY + rowIndex));
...
static std::vector<LocalExtreme> LookForAdditionalLocalExtremes(const NFmiDataMatrix<float> &subGridValues, int subGridOrigoX, int subGridOrigoY, int searchCoreRadiusInGridPoints)
{
    std::vector<LocalExtreme> localExtremes;
    ExtremeSearchCoreCounter searchCore;

    // Ei voi etsiä jos matriisi on liian pieni
    if(subGridValues.NX() > 3 * searchCoreRadiusInGridPoints && subGridValues.NY() > 3 * searchCoreRadiusInGridPoints)
    {
        // Käydään läpi kaikki ne pisteet mille voidaan tehdä laskut täydellä searchCore:lla
        for(size_t rowIndex = searchCoreRadiusInGridPoints; rowIndex < subGridValues.NY() - searchCoreRadiusInGridPoints; rowIndex++)
        {
            for(size_t columnIndex = searchCoreRadiusInGridPoints; columnIndex < subGridValues.NX() - searchCoreRadiusInGridPoints; columnIndex++)
            {
                float centerValue = subGridValues[columnIndex][rowIndex];
                searchCore.SetCenterValue(centerValue);
                // Käydään tässä pisteessä läpi searchCore:n arvot eli pistettä ympäröivän searchCore:n kokoisen laatikon arvot
                for(size_t searchCoreRowIndex = rowIndex - searchCoreRadiusInGridPoints; searchCoreRowIndex <= rowIndex + searchCoreRadiusInGridPoints; searchCoreRowIndex++)
                {
                    for(size_t searchCoreColumnIndex = columnIndex - searchCoreRadiusInGridPoints; searchCoreColumnIndex <= columnIndex + searchCoreRadiusInGridPoints; searchCoreColumnIndex++)
                    {
                ...
                if(searchCore.IsMaximum())
                    ::AddAdditionalLocalExtreme(localExtremes, centerValue, subGridOrigoX, subGridOrigoY, static_cast<int>(columnIndex), static_cast<int>(rowIndex), false);
                else if(searchCore.IsMinimum())
                    ::AddAdditionalLocalExtreme(localExtremes, centerValue, subGridOrigoX, subGridOrigoY, static_cast<int>(columnIndex), static_cast<int>(rowIndex), true);
            }
        }
    }
...
static void DoSameTypeLocalExtremeRemovalMarking(LocalExtreme &localExtreme1, LocalExtreme &localExtreme2)
{
...
static void CheckIsAdditionalLocalExtremeRemovedByProximity(LocalExtreme &localExtreme1, LocalExtreme &localExtreme2, double proximityLimitForAdditionalLocalExtremesInGridPoints)
{
...
static void RemoveMarkedLocalExtremesFromVector(std::vector<LocalExtreme> &localExtremesInOut)
{
    ...
    localExtremesInOut.erase(std::remove_if(localExtremesInOut.begin(), localExtremesInOut.end(), [](LocalExtreme &localExtreme) {return localExtreme.fRemoveFromResults; }), localExtremesInOut.end());
}

static void RemoveTooCloseAdditionalLocalExtremes(std::vector<LocalExtreme> &localExtremesInOut, boost::shared_ptr<NFmiFastQueryInfo> &info, float localAreaSearchRangeInKm)
{
    ...
    if(localExtremesInOut.size() > 1)
    {
        ...
        double proximityLimitForAdditionalLocalExtremesInGridPoints = (localAreaSearchRangeInKm / 10.) / avgGridPointSizeInKm;
        for(size_t firstIndex = 0; firstIndex < localExtremesInOut.size() - 1; firstIndex++)
        {
            for(size_t secondIndex = firstIndex + 1; secondIndex < localExtremesInOut.size(); secondIndex++)
            {
                ::CheckIsAdditionalLocalExtremeRemovedByProximity(localExtremesInOut[firstIndex], localExtremesInOut[secondIndex], proximityLimitForAdditionalLocalExtremesInGridPoints);
            }
        }
...
static void FillLocationInfoToAdditionalLocalExtreme(LocalExtreme &localExtreme, boost::shared_ptr<NFmiFastQueryInfo> &info)
{
    if(NFmiFastInfoUtils::SetInfoToGridPoint(info, static_cast<unsigned long>(localExtreme.itsOrigDataGridPoint.X()), static_cast<unsigned long>(localExtreme.itsOrigDataGridPoint.Y())))
...
{
    NFmiDataMatrix<float> matrix(itsCalculationGrid.XNumber(), itsCalculationGrid.YNumber(), kFloatMissing);
    ...
    if(itsInfo->IsInside(itsTime))
    {
        ...
        if(!boundaryVector.empty())
        {
            ...
            FillResultMatrixWithLocalExtremePlaces(localExtremeResults, matrix, itsLocalAreaSearchRangeInKm);
        }
    }
    return matrix;
}

static bool IsMoreSignificantLocalExtreme(const LocalExtreme &extreme1, const LocalExtreme &extreme2)
{
    return extreme1.itsSignificance > extreme2.itsSignificance;
}

static void CheckForExtremeRemovalByProximity(LocalExtreme &importantExtreme, LocalExtreme &lessImportantExtreme, float sameTypeExtremeRangeLimitInKm, float differentTypeExtremeRangeLimitInKm)
{
    ...
    float distanceInKm = static_cast<float>(location.Distance(lessImportantExtreme.itsLatlon) / 1000.);
...
// Eli aliruudukoista voi tulla lähekkäisiä ääripisteitä, jotka joskus ovat jopa samasta ääripisteestä (jossa 
// täsmälleen samoja arvoja monessa vierekkäisessä pisteessä). 
static void RemoveTooCloseExtremes(std::vector<LocalExtreme> &localExtremes, float localAreaSearchRangeInKm)
{
    if(localExtremes.size() > 1)
    {
        // Jos ääripisteet ovat samaa tyyppiä, laitetaan etäisyysraja tiukemmaksi (eli isommaksi eäisyydeksi)
        ...
        // merkittäviin ääripisteisiin ja katsotaan pitääkö jotain niistä eliminoida lyhyiden etäisyyksien perusteella.
        for(size_t importantExtremeIndex = 0; importantExtremeIndex < localExtremes.size() - 1; importantExtremeIndex++)
        {
            for(size_t lessImportantExtremeIndex = importantExtremeIndex + 1; lessImportantExtremeIndex < localExtremes.size(); lessImportantExtremeIndex++)
            {
                ::CheckForExtremeRemovalByProximity(localExtremes[importantExtremeIndex], localExtremes[lessImportantExtremeIndex], sameTypeExtremeRangeLimitInKm, differentTypeExtremeRangeLimitInKm);
            }
        }
...
void NFmiLocalAreaMinMaxMask::FillResultMatrixWithLocalExtremePlaces(std::vector<LocalExtreme> &localExtremeResults, NFmiDataMatrix<float> &resultMatrix, float localAreaSearchRangeInKm)
{
    if(!localExtremeResults.empty())
    {
        // Sortataan tulokset merkittävyys järjestykseen
        std::sort(localExtremeResults.begin(), localExtremeResults.end(), ::IsMoreSignificantLocalExtreme);
        ...
        {
            if(localExtreme.itsSignificance >= significanceLimit && localExtreme.itsAreaSizeAvgInKM >= areaSizeLimit)
            {
                if(itsCalculationGrid.NearestLatLon(localExtreme.itsLatlon.X(), localExtreme.itsLatlon.Y()))
                {
...
void NFmiLocalAreaMinMaxMask::InserDataToCache(const NFmiDataMatrix<float> &theMatrix, const NFmiMetTime &theTime)
{
...
static std::unique_ptr<NFmiArea> CalcBoundaryArea(const NFmiRect &calculationBoundary, boost::shared_ptr<NFmiFastQueryInfo> &info)
{
    auto grid = info->Grid();
...
{
    return area1->IsInside(area2->BottomLeftLatLon()) || 
        area1->IsInside(area2->TopLeftLatLon()) || 
        area1->IsInside(area2->TopRightLatLon()) || 
        area1->IsInside(area2->BottomRightLatLon());
}
...
{
    return ::IsAreaInsideAtAll(boundaryArea, calculationArea) || ::IsAreaInsideAtAll(calculationArea, boundaryArea);
}

static bool IsInsideCalculationArea(const NFmiRect &calculationBoundary, boost::shared_ptr<NFmiFastQueryInfo> &info, const NFmiGrid &calculationGrid)
{
...
// Käy läpi calculationBoundaries rect:it ja katsoo originaali infon kanssa niiden latlon laatikot ja katsoo meneekö
// nuo laatikot calculationGrid:in määrittämän alueen sisälle ollenkaan.
static std::vector<NFmiRect> RemoveOutOfCalculationAreaBoundaries(const std::vector<NFmiRect> &calculationBoundaries, boost::shared_ptr<NFmiFastQueryInfo> &info, const NFmiGrid &calculationGrid)
{
...
// Jakaa itsInfo:n alueen alialueisiin (gridPointIndex rajat NFmiRect:iin) käyttäen itsCalculationGrid:in
// aluetta pohjana (laskettava alue, joka voi olla eri kuin itsInfo:n alue) ja alialueet ovat suhteessa
// itsLocalAreaSearchRangeInKm:in kanssa.
std::vector<NFmiRect> NFmiLocalAreaMinMaxMask::CalculateLocalAreaCalculationBoundaries()
{
    ...
    auto calculationBoundaries = CalculateLocalAreaCalculationBoundaries(subGridCountX, subGridCountY);
...
static int CalcSubGridDecreaseIndex(int subGridCount, int subGridBaseSize, unsigned long actualOriginalGridSize)
{
...
std::vector<NFmiRect> NFmiLocalAreaMinMaxMask::CalculateLocalAreaCalculationBoundaries(int subGridCountX, int subGridCountY)
{
    auto subGridBaseSize = ::CalcSubGridBaseSize(subGridCountX, subGridCountY, *itsInfo->Grid());
    // SubGridBaseSize:ia ei voi käyttää aina jokaiselle ruudukon ruudulle, vaan sitä pitää pienentää x- ja y-suunnissa 
    // yhdellä tiettyjen pisteiden jälkeen (eri pisteet x- ja y-suunnissa).
    auto subGridDecreaseIndexX = ::CalcSubGridDecreaseIndex(subGridCountX, static_cast<int>(subGridBaseSize.X()), itsInfo->GridXNumber());
    auto subGridDecreaseIndexY = ::CalcSubGridDecreaseIndex(subGridCountY, static_cast<int>(subGridBaseSize.Y()), itsInfo->GridYNumber());
    return CalculateLocalAreaCalculationBoundaries(subGridCountX, subGridCountY, subGridBaseSize, subGridDecreaseIndexX, subGridDecreaseIndexY);
}
...
// Laskettavista hilalaatikoista halutaan ei-päällekkäisiä. Eli esim. jos hilan x koko on 12 ja halutaan 3 alihilaa, siitä seuraa x-suunnassa seuraava:
// 1. väli = 0 - 3, 2. väli 4 - 7, 3. väli 8 - 11
// Tämä tarkoittaa että gridPointIncrementX/Y:ein kanssa saa tehdä hieman kikkailua eri kohdissa loppeja...
std::vector<NFmiRect> NFmiLocalAreaMinMaxMask::CalculateLocalAreaCalculationBoundaries(int subGridCountX, int subGridCountY, const NFmiPoint &subGridBaseSize, int subGridDecreaseIndexX, int subGridDecreaseIndexY)
{
    ...
    {
        int gridPointIncrementY = ::CalcGridPointIncrement(static_cast<int>(subGridBaseSize.Y()), subGridDecreaseIndexY, subGridYIndex);
        int gridPointIndexX = 0;
        for(int subGridXIndex = 0; subGridXIndex < subGridCountX; subGridXIndex++)
        {
            int gridPointIncrementX = ::CalcGridPointIncrement(static_cast<int>(subGridBaseSize.X()), subGridDecreaseIndexX, subGridXIndex);
            // boundingRect:in kokoa pitää vähentää 1:llä, jotta aliruudut eivät menisi päällekkäin.
            NFmiRect boundingRect(gridPointIndexX, gridPointIndexY, gridPointIndexX + gridPointIncrementX - 1, gridPointIndexY + gridPointIncrementY - 1);
            // TODO: Tee tarkastelu, että onko kyseinen grid-point-laatikko laskenta-alueen sisällä ollenkaan
...
void NFmiLocalAreaMinMaxMask::SetArguments(std::vector<float> &theArgumentVector)
{
    ...
    // -1 tarkoittaa että funktion 1. argumentti tulee suoraan itsIfo:sta, eli sitä ei anneta argumentti-listassa
    if(static_cast<int>(itsArgumentVector.size()) != itsFunctionArgumentCount - 1)  
        throw std::runtime_error(
            "Internal SmartMet error: Vertical function was given invalid number of arguments, cannot calculate the macro.");
}
