...
// Muodostetaan yhtenäinen string esitys kaikista asetuksista (paitsi toolMasterAvailable_ asetuksesta), 
// jossa arvot on eroteltu pilkuilla. Arvot ovat luokan esittelyjärjestyksessä. Esimerkiksi oletus arvoilla saatu tulos:
// 1,0,1,0.5,0,1.25,0.15
std::string NFmiGriddingProperties::toString() const
{
    std::ostringstream out;
    out << function_ << "," << rangeLimitInKm_ << "," << localFitMethod_ << "," << localFitDelta_ << "," << smoothLevel_ << "," << localFitFilterRadius_ << "," << localFitFilterFactor_;
    return out.str();
}
