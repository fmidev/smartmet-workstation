...
private:
    static filename_t calc_filename(const filename_t& filename, std::size_t index)
    {
        ...
        if(index)
        {
            auto str = fmt::format(SPDLOG_FILENAME_T("{}.{}"), filename, index);
            ::append_to_buffer(w, str);
        }
        else
        {
            auto str = fmt::format(SPDLOG_FILENAME_T("{}"), filename);
            ::append_to_buffer(w, str);
        }
        return w.data();
    }
...
struct default_daily_file_name_calculator
{
    ...
    static filename_t calc_filename(const filename_t& basename)
    {
        ...
        std::conditional<std::is_same<filename_t::value_type, char>::value, fmt::memory_buffer, fmt::wmemory_buffer>::type w;
        auto str = fmt::format(SPDLOG_FILENAME_T("{}_{:04d}-{:02d}-{:02d}_{:02d}-{:02d}-{:02d}"), basename, tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
        ::append_to_buffer(w, str);
        return w.data();
    }
};
...
struct dateonly_daily_file_name_calculator
{
    ...
    static filename_t calc_filename(const filename_t& basename)
    {
        std::tm tm = spdlog::details::os::localtime();
        std::conditional<std::is_same<filename_t::value_type, char>::value, fmt::memory_buffer, fmt::wmemory_buffer>::type w;
        auto str = fmt::format(SPDLOG_FILENAME_T("{}_{:04d}-{:02d}-{:02d}"), basename, tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday);
        ::append_to_buffer(w, str);
        return w.data();
    }
};
