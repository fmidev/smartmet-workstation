...
//		sc.nWidth = 0;
	...
	szTable.cx = static_cast<LONG>(cur_table.width.size());
    szTable.cy = static_cast<LONG>(cur_table.height.size());
	...
	if (MODE_FIRSTPASS != m_nNumPass)
	{
        ...
        int nWidthTable = static_cast<int>(m_defStyle.nPadding + cur_table.width.size() - 1);
		for (pos = 0; pos < cur_table.width.size(); ++pos)
			nWidthTable += cur_table.width [pos] + m_defStyle.nPadding;
        rcTable.bottom += static_cast<LONG>(m_defStyle.nPadding + cur_table.height.size() - 1);
...
SIZE CPPHtmlDrawer::DrawHtmlString (CPPString & sHtml, LPCRECT lpRect)
{
	...
	COLORREF clrShadow = m_bIsEnable ? m_crShadow : GetColorByName(_TEXT(""));
									...
									if (!sParameter.IsEmpty())
									{
										...
										else if (sParameter == _T("color"))
										{
											if (m_bIsEnable)
                                                ...
                                                m_defStyle.crText = GetColorByName(_TEXT(""));
										}
								...
								if (!sParameter.IsEmpty())
								{
									...
									else if (sParameter == _T("color"))
									{
										if (m_bIsEnable)
                                            ...
                                            csTemp.crText = GetColorByName(_TEXT(""));
									}
								} //if
							} //while
							...
							else if (NULL != m_hImageList)
							{
								...
								if ((int)si.nIndexImageList < ImageList_GetImageCount(m_hImageList))
								{
									...
									if (NULL != hIcon)
									{
										...
										::DestroyIcon(hIcon); // siirto pois if-lauseesta Marko, poisti loputkin resurssi vuodoista
										ptOutput.x += szReal.cx;
									} //if
								} //if
							} //if
...
int CPPHtmlDrawer::InitNewLine(int x)
{
	...
	else if (MODE_DRAW == m_nNumPass)
	{
        ...
        if(m_nCurLine < static_cast<int>(m_arrHtmlLine.size())) // MARKO: lisдsin indeksin tarkastelun, koska SmartMet on crashrpt-raportin mukaan kaatunut seuraavalle riville
        {
			...
			if (0 != chSymbol)
			{
				...
				else if (strName == _T("color"))
				{
					if (m_bIsEnable)
                        ...
                        cs.crText = GetColorByName(_TEXT(""));
				}
				...
				else if (strName == _T("border-color"))
				{
					if (m_bIsEnable)
                        ...
                        cs.crBorderLight = GetColorByName(_TEXT(""));
	...
	while (i < sProperties.GetLength())
	{
		...
		else if (sParameter == _T("bordercolordark"))
		{
			if (m_bIsEnable)
                ...
                cs.crBorderDark = GetColorByName(_TEXT(""));
		}
