...
const int MARKO_DEBUG_MODE_NO_TRACE = 1; // kaikkia trace juttuja ei jaksa antaa mennд developer studion output-ikkunaan (aseta 0:ksi jos haluat taas menevдn)
/*
...
CPPToolTip::CPPToolTip()
{
	...
	SetNotify(FALSE);
	SetDefaultSizes();
	SetDirection(PPTOOLTIP_LEFTEDGE_VCENTER);
	SetBehaviour();
	SetDebugMode(FALSE);
	SetMaxTipWidth(0);
...
void CPPToolTip::PrepareDisplayTooltip(LPPOINT lpPoint)
{
	...
	else
	{
		SetWindowPos(NULL, 
			...
			m_rcBoundsTooltip.Width(), 
			m_rcBoundsTooltip.Height(),
			SWP_SHOWWINDOW|SWP_NOACTIVATE|SWP_NOZORDER/*|SWP_NOCOPYBITS*/); // Marko: lisдsin SWP_NOZORDER -attribuutin myцs tooltip piirtoon, koska halusin estдд ettд SmartMetissa ikkunoiden piirtojдrjestys muuttuu kun tooltip tulee nдkyviin.
	}
...
// Crude fix does following: If Taskbar is visible and has size, we narrow 
// screen size given to tooltip positioning and direction calculations by that taskbar area.
static void DoTooltipBehindTaskBarFixToScreenRect(CRect& screenRectInOut)
{
    ...
    if(appbarData.rc.bottom != appbarData.rc.top && appbarData.rc.left != appbarData.rc.right)
    {
        ...
        {
            screenRectInOut.top = appbarData.rc.bottom;
        }
        else if(appbarData.uEdge == ABE_LEFT)
        {
            screenRectInOut.left = appbarData.rc.right;
        }
        else if(appbarData.uEdge == ABE_RIGHT)
        {
            screenRectInOut.right = appbarData.rc.left;
        }
    }
}
...
DWORD CPPToolTip::GetTooltipDirection(DWORD dwDirection, const CPoint & ptPoint, CPoint & ptAnchor, CRect & rcBody, CRect & rcFull, CRect & rcTipArea)
{
	...
	CString str;
	str.Format(_TEXT("0x%08X"), dwDirection);
...
// Tдmд korjaus on otettu http://www.codeproject.com/KB/miscctrl/pptooltip.aspx?msg=1141859#xx1141859xx
...
/* // originaali metodi ei toiminut, jos halusi tehdд tooltippeja dialogissa kohtaan missд ei ollut mitддn ali-ikkunaa tai kontrolleja
HWND CPPToolTip::GetWndFromPoint(const LPPOINT lpPoint, PPTOOLTIP_INFO & ti, BOOL bCheckTool)
{
	// the default implementation of tooltips just calls WindowFromPoint
	// which does not work for certain kinds of combo boxes
	CPoint pt = *lpPoint;
...
BOOL CPPToolTip::IsComboBoxControl(HWND hWnd, UINT nStyle)
{
    ...
    if((UINT)(::GetWindowLongPtr(hWnd, GWL_STYLE) & 0x0F) != nStyle)
		return FALSE;
...
CString CPPToolTip::GetDebugInfoTool(LPPOINT lpPoint)
{
	...
	//4. Window's ID
    str.Format(_T("%d"), GetWindowLongPtr(hWnd, GWLP_ID));
	strOutput += CreateDebugCell(_T("Control ID"), str);

	//5. Window's styles
    str.Format(_T("0x%08X"), (DWORD)::GetWindowLongPtr(hWnd, GWL_STYLE));
