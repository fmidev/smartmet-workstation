...
//
LONG_PTR CALLBACK ComboBoxListBoxProc(HWND hWnd, UINT nMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (nMsg) {
		...
		case WM_RBUTTONDOWN: {
			...
			// right button, remove this ifdef. Personally, I don't really like it
				...
				if (m_pComboBox != 0) {
                    ...
                    m_pComboBox->GetParent()->SendMessage(WM_COMMAND, MAKELONG(GetWindowLongPtr(m_pComboBox->m_hWnd, GWLP_ID), CBN_SELCHANGE), (LPARAM)m_pComboBox->m_hWnd);
		...
		case WM_CHAR: {
			if (wParam == VK_SPACE) {
				...
				INT nIndex = static_cast<int>(CallWindowProc(m_pWndProc, hWnd, LB_GETCURSEL, wParam, lParam));
...
void CCheckComboBox::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
    ...
    // Marko: HUOM! Tätä LONG castia ei saa muuttaa x64 yhteensopivuuden nimissä LONG_PTR:ksi!
...
std::vector<bool> CCheckComboBox::GetSelectionVector(void)
{
	std::vector<bool> selectionVector;

	INT nCount = GetCount();
	for (INT i = 0; i < nCount; i++)
		selectionVector.push_back(GetCheck(i) == TRUE);

	return selectionVector;
}

std::vector<std::pair<int, CString> > CCheckComboBox::GetSelectedWithStr(void)
{
	std::vector<std::pair<int, CString> > selectionVector;
	...
	for (int i = 0; i < nCount; i++)
	{
		if(GetCheck(i))
		{
			CString lbStr;
			GetLBText(i, lbStr);
			selectionVector.push_back(std::make_pair(i, lbStr));
		}
	}
	return selectionVector;
}
...
void CCheckComboBox::InitToolTip()
{
	if (m_ToolTip.m_hWnd == NULL)
	{
		try
		{
			...
			m_ToolTip.SetDelayTime(TTDT_INITIAL, 150); // lyhyt alku aika (150 ms) ennen kuin tootip tulee näkyviin
			m_ToolTip.SetDelayTime(TTDT_AUTOPOP, 30 * 1000); // 30 sekuntia on pisin saika minkä sain tooltipin pysymään näkyvissä (jos suurempi, menee defaultti arvoon 5 sekuntia)
