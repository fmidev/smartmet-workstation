...
DrawStringData::DrawStringData(const CString &text, const CString &fontName, int fontSize, COLORREF fontColor, const CPoint &absoluteTextPosition, bool bold)
    :mText(text)
    , mFontName(fontName)
    , mFontSize(fontSize)
    , mFontColor(fontColor)
    , mAbsoluteTextPosition(absoluteTextPosition)
    , mBold(bold)
{
}
...
{
    pDC->SetTextColor(theStringData.mFontColor);

    CFont font;
    LOGFONT lf;
    memset(&lf, 0, sizeof(LOGFONT));       // zero out structure
    lf.lfHeight = theStringData.mFontSize;
    if(theStringData.mBold)
        lf.lfWeight = FW_BOLD;
    _tcsncpy_s(lf.lfFaceName, LF_FACESIZE, theStringData.mFontName, theStringData.mFontName.GetLength());
    VERIFY(font.CreateFontIndirect(&lf));  // create the font

    CFont* oldFont = pDC->SelectObject(&font);

    pDC->TextOut(theStringData.mAbsoluteTextPosition.x, theStringData.mAbsoluteTextPosition.y, theStringData.mText);
...
{
    for(const auto &stringData : theStringDataVector)
        DrawDynamicText(pDC, stringData);
}
...
void CSplashThread::SetSplashScreenTextDataVector(const std::vector<DrawStringData> &theDynamicTextsDataVector)
{
    m_SplashScreen.SetSplashScreenTextDataVector(theDynamicTextsDataVector);
}
...
void CSplashWnd::SetSplashScreenTextDataVector(const std::vector<DrawStringData> &theDynamicTextsDataVector)
{
