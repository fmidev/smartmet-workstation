...
BOOL CControlSplitter::OnSetCursor(CWnd*, UINT, UINT)
{
    if(m_hCursor != NULL)
    {
        DestroyCursor(m_hCursor);
    ...
    m_hCursor = ::LoadCursor(NULL, (CS_HORZ == m_nType) ? IDC_SIZENS : IDC_SIZEWE);
    ::SetCursor(m_hCursor); //Affect only current window
...
void CControlSplitter::SetMaxRect(const CRect &newMaxRect)
{
    m_rectMax = newMaxRect;
}
