...
void CTabCtrlSSL::ResizeActiveSSLPage()
{
    if(GetSafeHwnd() != NULL)
    {
        CRect rect;
        GetClientRect(&rect);
        ResizeDialog(GetSSLActivePage(), rect.Width(), rect.Height());
    }
}
...
void CTabCtrlSSL::OnDestroy (void) {
	int nCount = static_cast<int>(m_tabs.GetSize ());
...
int CTabCtrlSSL::AddPage (LPCTSTR pszTitle, int nPageID, TabDelete tabDelete) {
    ...
    if (NULL == tabDelete.pTabPage) {
		...
		int nArrayIndex = static_cast<int>(m_tabs.Add (tabDelete));
		ASSERT (nIndex == nArrayIndex);

        nArrayIndex = static_cast<int>(m_nPageIDs.Add(nPageID));
		ASSERT (nIndex == nArrayIndex);
		...
		if (OnInitPage (nIndex, nPageID)) {
            ...
            nArrayIndex = static_cast<int>(m_hFocusWnd.Add(hwndFocus));
			ASSERT (nIndex == nArrayIndex);
		}
		else {
