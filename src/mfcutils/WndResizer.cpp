...
void CWndResizer::OnSizing(UINT fwSide, LPRECT pRect)
{
  ...
  ::AdjustWindowRectEx(&rcMin, static_cast<DWORD>(style), (m_pHookedWnd->GetMenu() != NULL), static_cast<DWORD>(styleEx));
  ::AdjustWindowRectEx(&rcMax, static_cast<DWORD>(style), (m_pHookedWnd->GetMenu() != NULL), static_cast<DWORD>(styleEx));
