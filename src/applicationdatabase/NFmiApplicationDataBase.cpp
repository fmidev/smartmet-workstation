...
{
	std::string str = fromLocaleStringToUtf8(theString); // ensin muutetaan mahdolliset ��kk�set ja muut utf8:ksi
...
// Nyt tuli ongelmia Turun siitep�ly editointi konffien kanssa, koska siin� oli control-polku seuraava:
	...
	if(action.empty() == false)
{
	...
	{
		resultStr += "&pcname=";
		pcname = ::FixStringWithUtf8AndUrlEncode(pcname); // t�ss� voisi olla spaceja ja ��kk�si�, siksi pit�� varmuuden vuoksi encodata
		resultStr += pcname;
	}
	...
	{
		resultStr += "&username=";
		username = ::FixStringWithUtf8AndUrlEncode(username); // t�ss� voisi olla spaceja ja ��kk�si�, siksi pit�� varmuuden vuoksi encodata
		resultStr += username;
	}
...
std::string NFmiApplicationDataBase::GetProcessPathAndName(DWORD processID)
{
	...
	CloseHandle(hProcess); // vapauta handle k�yt�n j�lkeen
...
//   - Siin� otetaan joku randomin n�k�inen muistiosoite talteen ja siit� nysv�t��n sopivilla siirtymill� major/minor/build numerot
//   - Ainoa lohtu on ett� se toimii ainakin Windows 10:iss�, muita koneita ei ole minulla k�yt�ss�
#ifndef UNIX
...
void NFmiApplicationDataBase::CollectSmartMetData(NFmiApplicationDataBase::Action theAction, FmiLanguage applicationLanguage, int applicationRunningTimeInSeconds, bool toolMasterAvailable, NFmiInfoOrganizer *infoOrganizer)
{
	...
	// avsexpstatus  // AVS Express ei viel� k�yt�ss�
	// avsexpversion // AVS Express ei viel� k�yt�ss�
	...
	if(osvi.dwMajorVersion >= 6)
	{  // k�ytet��n t�t� jos voidaan, t�m� ei mene heti ymp�ri, kuten GetTickCount() menee 49.7 vrk:ssa, kun on 64-bitti� k�yt�ss� (liikkumavara on n. 0.5 mrd vuotta).
		// HUOM! t�t� voi kutsua Vistasta (6.0) ja WinServer2008:sta (6.0) alkaen
		// HUOM! T�t� koko GetTickCount64 -funktiota ei voi k�ytt�� ollenkaan, koska sit� ei l�ydy Kernel32.dll:st� esim. XP:ss� ja ohjelman suoritys tyss�� heti k�ynnnistyess�, vaikka koko funktiota ei oikeasti k�ytett�isi
...
//		pcuptime = upTimeInMilliSeconds/1000;
	...
	// gcdriverversion
	// T�H�N kohtaan en l�yt�nyt nyt keinoa saada yksiselitteisesti arvoa, en rekisterist�, enk� win32 API:sta
