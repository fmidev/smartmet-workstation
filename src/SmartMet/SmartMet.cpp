...
namespace
{
	// T‰m‰ osio SmartMetin asetuksia initialisoidaan ennen j‰ttim‰isen NFmiEditMapGeneralDataDoc -olion
	// alustusta. T‰t‰ tarvitaan kun CrashRpt-systeemi alustetaan. Sitten kun lopulta alustetaan itse 
	// NFmiEditMapGeneralDataDoc -olio, annetaan t‰m‰ otus parametrina, jolloin t‰st‰ tehd‰‰n kopio 
	// NFmiEditMapGeneralDataDoc -olion sis‰iseen vastaavaan dataosaan.
	NFmiBasicSmartMetConfigurations gBasicSmartMetConfigurations; 
		...
		#define DO_CRASH_REPORTING 1 // Laita t‰m‰ kommenttiin, jos et halua laittaa kaatumis raportointia
        #include "CrashRpt.h"
	#endif
#endif
...
BOOL CSmartMetApp::InitInstance()
{
	...
	AfxInitRichEdit(); // T‰m‰ pit‰‰ kutsua ett‰ ne dialogit, joissa on k‰ytetty Rich-edit controllia toimisivat (mm. SmartToolDlg ja luotausn‰ytˆn txt-dialogi).
						// En tied‰ kumpi muutos aiheutti ongelman: siirtyminen VC++ 2010:aan vai siirtyminen k‰ytt‰m‰‰n MFC feature packia.

	CFmiUsedAppParent::InitInstance();

    // T‰m‰ on kutsuttava ennen gBasicSmartMetConfigurations.Init -kutsua
	...
	CrashRptInstall(); // t‰m‰ on tarkoitus kutsua ennen InitGeneralDataDoc-kutsua, mutta t‰t‰ ei voinut laittaa suoraan applikaation konstruktoriin (kaatuu).
    ...
    DoToolMasterInitialization(); // m_pMainWnd luodaan vasta ProcessShellCommand -metodissa, joten sen j‰lkeen voidaan ToolMaster initialisoida
...
//	CreateMenuDynamically();
    ...
    itsGeneralData->FillStartupViewMacro(); // Otetaan SmartMetista 'alkukuva' n‰yttˆmakroon kaikkien alustuksien j‰lkeen
...
void CAboutDlg::OnPaint()
{
    ...
    // Piirret‰‰n dynaamiset tekstit bitmapin p‰‰lle
...
void CSmartMetApp::DoFinalInitializations(void)
{
	...
	// tehd‰‰n automaattinen datan lataus, jos niin on haluttu
	...
	((CMainFrame*)m_pMainWnd)->StartAdditionalWorkerThreads(); // k‰ynnistet‰‰n muut working-theadit, vasta kun MainFrame on luotu.
}
...
void CSmartMetApp::DoFirstTimeOnIdleInitializations()
{
    ...
    if(firstTime)
    { // en saanut muuten auto zoomia toimimaan kun editori k‰ynnistet‰‰n kuin laittamalla tanne refresh komento ruudulle kun 1. ollaan onidlessa.
        firstTime = false;

        // T‰m‰ pit‰‰ tehd‰ 1. kerran vasta kun kaikki FrameWorkin initialisoinnit on tehty
...
BOOL CSmartMetApp::OnIdle(LONG lCount)
{
	...
	if(itsGeneralData)
	{
        ...
        if(itsGeneralData->DoImageCacheUpdates() == 1)
            return 1; // jos olis imageCache ruudunp‰ivityksi‰, lopetetaan onIdle siihen ja palautetaan 1

		// palauttaa 0 jos ei animaatiota ja 1 jo tehd‰‰n animaatiota
		return itsGeneralData->DoAllAnimations();
	}
	return 0; // onidle k‰sittely loppuu 0:lla
}
...
void CSmartMetApp::InitNonDictionaryToolbarItems()
{
    try
    {
        // Toolbarissa olevat kartta nappien tooltip tekstit pit‰‰ ottaa konfiguraatioista.
        // T‰‰ll‰ t‰ytet‰‰n id-text map, josta kyseiset tekstit saa vastaavaa button id:t‰ vastaan.
        // P‰‰karttan‰ytˆn toolbar button id:t
        ...
        // Apukarttan‰yttˆjen toolbar button id:t laitetaan lukuina, koska en halua includata SmartMetToolboxDep\SmartMetToolboxDep_resource.h -tiedostoa
...
// T‰m‰n funktion avulla saadaan status ja tooltip stringeist‰
...
{
	if(theCmdInfo && theCmdInfo->m_nShellCommand == CCommandLineInfo::FileOpen) // jos on tiputettu editor-ikonin p‰‰lle tai annettu komento rivilla tiedoston nimi, mik‰ pit‰‰ avata
	{
		...
		{ // t‰h‰n voisi tehd‰ varoituksia ja muita virityksi‰
		}
	}
}
...
// Unicode moodissa k‰ytet‰‰n t‰t‰ muuttamaan 
// wchar_t ** __wargv -> char** argv
// Huolehtii myˆs luodun C-string taulukon tuhoamisesta.
// Jos ei Unicode-moodissa, ei luoda taulukkoa, vaan k‰ytet‰‰n originaalia.
...
// suoraan tiedosto, jota k‰ytet‰‰n 'smartmet.conf':ina.
// Ohjelman otsikko teksti -t argumenttina.
// d optiolla s‰‰det‰‰n k‰ynnistyst‰ Visual C++:sta (erilainen working path)
// n optiolla voidaan sanoa ett‰ ei k‰ytet‰ ToolMasteria ollenkaan.
// s optiolla voidaan antaa extra asetuksia listassa (NFmiSettings asetuksia)
// v optiolla voidaan laittaa enemm‰n lokitusta p‰‰lle (ainakin initialisoinnissa)
// c optiolla disabloidaan CrashReporter systeemin k‰yttˆ kokonaan.
//
...
bool CSmartMetApp::ParseCommandLineArguments()
{
	...
	if(cmd.isOption('s'))
	{
		...
		for(int i = 0; i < settingsVector.size(); i++)
		{
			...
			{
				NFmiSettings::Set(keyValuePair[0], keyValuePair[1], false); // false tarkoittaa ett‰ lis‰t‰‰n uusia asetuksia
			}
		}
	}
...
// N‰m‰ est‰v‰t Windowsin WER (Windows Error Reporting) dialogien k‰ynnistymisen kaatumisen yhteydess‰.
// N‰it‰ ei saa pois p‰‰lt‰ mill‰‰n win-registry asetuksilla, varsinkaan jos koneeseen on asennettu
// Visual C++ k‰‰nt‰j‰, jolloin systeemi haluaa tarkistaa viel‰ k‰ytt‰j‰lt‰ ett‰ haluaako h‰n debugata ohjelman kaatoa.
...
bool CSmartMetApp::CrashRptInstall(void)
{
    ...
    if(gBasicSmartMetConfigurations.EnableCrashReporter())
    {
        // http l‰hetys on otettu kayttoon kaikille, koska email attachmenttien koko on rajoitettu ja 
        ...
        // Nyt k‰ytˆss‰ vain s‰hkˆpostil‰hetys
