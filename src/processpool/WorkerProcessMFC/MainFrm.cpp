...
namespace
{
	CSemaphore gWorkerLoopThreadRunning; // t�m�n avulla yritet��n lopettaan jatkuvasti py�riv� worker thread 'siististi'
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	itsCheckClosingTimer = static_cast<UINT>(SetTimer(kFmiCheckClosingTimer, 500, NULL)); // tarkistetaan pari kertaa sekunnissa ett� pit��k� ohjelma sulkea, k�sky tulee threadista, joten se pit�� tehd� n�in timerin kautta
...
void log_point_values(const task_holder_t &task, const std::vector<float> &x_values, const std::vector<float> &y_values, const std::vector<float> &z_values)
{
    ...
    { // turha rakentaa t�t� ty�l�st� stringi�, jos sit� ei kuitenkaan logata
...
void CMainFrame::StartWorkerLoopThread(void)
{
    ...
    work_queue_verbose_logging(mpp_options.verbose_logging); // laitetaan my�s work_queue:n verbose-log tila p��lle
