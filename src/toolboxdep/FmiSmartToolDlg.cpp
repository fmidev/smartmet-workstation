...
#endif // DISABLE_EXTREME_TOOLKITPRO
{
	assert(itsSmartToolInfo); // t�ss� pit�� olla jotain, koska my�hemmin ei tarkistuksia!
...
// niit� ei voisi luka vanhemmilla SmartMet versioilla (teksteihin tulisi bin��ri m�ss�� sekaan).
std::string CFmiSmartToolDlg::GetMacroTextFromSyntaxEditor()
{
    CXTPSyntaxEditBufferManager* pDataMan = itsSyntaxEditControl->GetEditBuffer();
    if(pDataMan)
    {
        // 1. Luetaan koko kontrollin tekstisis�lt� CMemFile olioon.
        CMemFile memFile;
...
void CFmiSmartToolDlg::OnButtonAction()
{
	if(itsSmartMetDocumentInterface)
	{
		...
		UpdateWindow();
        // koska smarttool muokkaus menee nyky��n omaan threadiin ja p��karttan�ytt� lukitaan Progress-cancelointi -dialogilla, 
...
{ // ladataan virallinen DBCheck-macro ruudulle esim. testi k�ytt��n
    LoadSmarttoolFormula(itsSmartToolInfo->DBCheckerFileName(), true);
	itsSmartToolInfo->CurrentScript(itsSmartToolInfo->DBCheckerText()); // pit�� p�ivitt�� my�s currenttia skripti�
        ...
        if(!itsLoadedMacroParamPathTextU_.IsEmpty())
        {
            ...
            if(currentMacroParam && currentMacroParam->IsMacroParamDirectory() == false)
            {
                ...
                if(currentMacroParam->Store(absoluteMacroParamDirectory, currentMacroParam->Name()))
                {
                    ...
                    RefreshApplicationViewsAndDialogs("SmartToolDlg: macro-param save", false, finalMacroParamPath); // p�ivitet��n varmuuden vuoksi ruutuja, jos karttan�yt�ll� olleen macroparametrin macroa on muutettu
                    return;
                }
                else
    ...
    if(BetaProduct::GetFilePathFromUserTotal(g_MacroParamFileFilter, initialPath, filePath, false, initialFilename, g_MacroParamFileExtension, macroParamSystemPtr->RootPath(), this))
    {
        ...
        if(macroParamSystemPtr->SetCurrentToWantedMacroPath(filePath))
        {
            ...
            if(currentMacroParam)
            {
                ...
                updateViews = true; // nyt tiedet��n, ett� pit�� p�ivitt�� n�ytt�j�
            }
        }
        else if(!::IsInCurrentMacroParamDirectory(*macroParamSystemPtr, filePath))
        {
            // Jos macroa ei l�ydy nyky hakemistosta, pit�� hakemisto hakea globaalisti root-hakemiston alta
        ...
        if(updateViews)
        {
            std::vector<std::string> modifiedMacroParamPaths{ realMacroParamDrawParamFileName };
            itsSmartMetDocumentInterface->MacroParamDataCache().clearMacroParamCache(modifiedMacroParamPaths);
            RefreshApplicationViewsAndDialogs("SmartToolDlg: macro-param save", false, filePath); // p�ivitet��n varmuuden vuoksi ruutuja, jos karttan�yt�ll� olleen macroparametrin macroa on muutettu
        }
    }
}
...
void CFmiSmartToolDlg::UpdateMacroParamDisplayList(bool fForceThreadUpdate)
{
    ...
    {
        // Ei p�ivitet� valitun macroParamin sis�lt��, koska se 'resetoi' mahdolliset k�ytt�j�n tekem�t editoinnit
////        LoadFormulaFromMacroParam();
    }

    if(fForceThreadUpdate)
    {
		CFmiMacroParamUpdateThread::ForceUpdate(); // t�m� pakottaa ett� my�s pop-up valikoiden k�ytt�m�� puu rakennetta p�ivitet��n
...
void CFmiSmartToolDlg::OnBnClickedButtonMacroParamRemove()
{
    ...
    if(!macroParamName.empty())
	{
        ...
        if (::MessageBox(this->GetSafeHwnd(), CA2T(str.c_str()), CA2T(titleStr.c_str()), MB_ICONINFORMATION | MB_OKCANCEL) == IDOK)
		{
			...
			RefreshApplicationViewsAndDialogs("SmartToolDlg: macro-param remove", false, "", false); // p�ivitet��n varmuuden vuoksi ruutuja, jos poitettiin karttan�yt�lt� macroparametri
		}
	}
}
...
void CFmiSmartToolDlg::DoMacroParamLoad(const std::string& theMacroParamName, bool fDoSpeedLoad)
{
    if(!theMacroParamName.empty())
    {
        ...
        {
            LoadFormulaFromMacroParam();
            itsSmartToolInfo->CurrentScript(GetSmarttoolFormulaText()); // p�ivitet��n my�s currentiksi macro-tekstiksi
            itsMacroErrorTextU_ = "";
        }
    }
