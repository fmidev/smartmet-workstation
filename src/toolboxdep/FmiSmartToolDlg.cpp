...
,itsMacroParamDrawParam(new NFmiDrawParam() )
...
{
	UpdateData(TRUE);
	if(itsMacroParamNameU_ == CString(_TEXT("macroParam")))
	{ // ei voida sallia t‰m‰n nimist‰ macroParam:ia, koska se on varattu ns. default macroParamille
		string str(::GetDictionaryString("SmartToolDlgMacroParamToken1"));
		str += "\n";
		str += ::GetDictionaryString("SmartToolDlgMacroParamToken2");
		str += "\n";
		str += ::GetDictionaryString("SmartToolDlgMacroParamToken3");
		str += "\n\n";
		str += ::GetDictionaryString("SmartToolDlgMacroParamToken4");
        ::MessageBox(this->GetSafeHwnd(), CA2T(str.c_str()), CA2T(::GetDictionaryString("SmartToolDlgMacroParamToken5").c_str()), MB_ICONINFORMATION | MB_OK);
		return ;
	}
	std::string tmpName = CT2A(itsMacroParamNameU_);
	NFmiStringTools::Trim(tmpName, ' '); // pakko trimmata pois mahdolliset vasemman/oikeanpuoleiset v‰lilyˆnnit, muuten makroa ei voi valita en‰‰ myˆhemmin
	if(tmpName.empty())
		return ;
	if(tmpName[0] == '<')
	{
		CreateNewMacroParamDirectory(tmpName);
		return ;
	}
	boost::shared_ptr<NFmiMacroParam> tmp(new NFmiMacroParam);
	tmp->Name(tmpName);
	tmp->MacroText(GetMacroTextFromSyntaxEditor());
	tmp->DrawParam(itsMacroParamDrawParam);
	tmp->DrawParam()->ParameterAbbreviation(tmp->Name());
    tmp->DrawParam()->DataType(GetUsedMacroParamType());
	NFmiMacroParamSystem& mpSystem = itsSmartMetDocumentInterface->MacroParamSystem();
	std::string initFileName(mpSystem.CurrentPath());
	initFileName += tmp->Name();
	initFileName += ".dpa";
	...
	if(mpSystem.FindTotal(initFileName))
	{
        ...
        string str(::GetDictionaryString("SmartToolDlgMacroParamOverWrite1"));
		str += "\n";
        str += currentMacroParam->Name();
		str += " ";
		str += ::GetDictionaryString("SmartToolDlgMacroParamOverWrite2");
        if (::MessageBox(this->GetSafeHwnd(), CA2T(str.c_str()), CA2T(::GetDictionaryString("SmartToolDlgMacroParamOverWrite3").c_str()), MB_ICONINFORMATION | MB_OKCANCEL) == IDCANCEL)
			return ;
        tmp->DrawParam(currentMacroParam->DrawParam()); // pit‰‰ ottaa talteen vanhat piirto-ominaisuudet!
		updateViews = true; // nyt tiedet‰‰n, ett‰ pit‰‰ p‰ivitt‰‰ n‰yttˆj‰
	}
	else
		index = LB_ERR; // laitetaan t‰h‰n ei validi indeksi kun oli uusi macroParam, eik‰ vanhan p‰‰lle talletus, ett‰ tiedet‰‰n indeksi myˆhemmin laittaa osoittamaan viimeiseen

	tmp->DrawParam()->InitFileName(initFileName);
	tmp->DrawParam()->MacroParamRelativePath(mpSystem.RelativePath());
	...
	try
	{
		...
		if(currentFolder)
		{
			currentFolder->Save(tmp);
            CatLog::logMessage(string("Saved macroParam: ") + tmp->Name(), CatLog::Severity::Info, CatLog::Category::Macro);
		}
	}
	...
	if(updateViews)
	{
		...
		RefreshApplicationViewsAndDialogs("SmartToolDlg: macro-param save"); // p‰ivitet‰‰n varmuuden vuoksi ruutuja, jos karttan‰ytˆll‰ olleen macroparametrin macroa on muutettu
	}
}
...
void CFmiSmartToolDlg::UpdateMacroParamDisplayList(bool fForceThreadUpdate)
{
    ...
    if(fForceThreadUpdate)
    {
        ...
        itsSmartMetDocumentInterface->UpdateParamAddingSystem(); // Joonas: ennen t‰t‰ pit‰isi macroparamsystemin puurakenne saada p‰ivitetty‰!
    }
}
...
void CFmiSmartToolDlg::OnBnClickedButtonMacroParamRemove()
{
	...
	if(index != LB_ERR)
	{
        ...
        if (::MessageBox(this->GetSafeHwnd(), CA2T(str.c_str()), CA2T(titleStr.c_str()), MB_ICONINFORMATION | MB_OKCANCEL) == IDOK)
		{
			...
			RefreshApplicationViewsAndDialogs("SmartToolDlg: macro-param remove"); // p‰ivitet‰‰n varmuuden vuoksi ruutuja, jos poitettiin karttan‰ytˆlt‰ macroparametri
		}
	}
}
...
{
	// katsotaan editoidaanko jotain valittua macroParamia vai defaulttia (=itsMacroParamDrawParam)
	bool useSelected = false;
	NFmiMacroParam *selected = FindSelectedMacroParam();
	if(selected)
		useSelected = true;

	CFmiModifyDrawParamDlg dlg(itsSmartMetDocumentInterface, useSelected ? selected->DrawParam() : itsMacroParamDrawParam, itsSmartMetDocumentInterface->InfoOrganizer()->GetDrawParamPath(), true, false, itsSelectedMapViewDescTopIndex, this); // smarttool-dialogista ei voi toistaiseksi lis‰t‰ kuin p‰‰karttan‰ytˆlle macroParameja
	if(dlg.DoModal() == IDOK)
	{
