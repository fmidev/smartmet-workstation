...
enum class CheckStatus
{
	...
	Warning,
	Error,
	NoCheck, // Tarkistusta ei ole tehty, milloin voi olla t‰m‰ tilanne?
	NoData, // Kun datasta ei lˆydy testattua parametria ollenkaan
	NoBaseValue // Kun tehd‰‰n perus status testi‰, t‰m‰ on vaihtoehto, jos kyse ei ole NoCheck eik‰ NoData tapauksesta
};
...
static std::string MakeMinMaxTimeLocationString(bool shortVersion, bool mainStatus, std::string nameStr, const NFmiMetTime &atime, const NFmiPoint &locationPoint)
{
	...
	{
		str += "\n" + nameStr + " time: ";
		str += atime.ToStr("ww HH:mm DD.MM.YYYY"); // on sama mik‰ aika (min, max) t‰h‰n‰ otetaan koska kyse on yhden aika-askeleen arvoista
	}
	if(!shortVersion)
	{
		str += "\n" + nameStr + " location : ";
