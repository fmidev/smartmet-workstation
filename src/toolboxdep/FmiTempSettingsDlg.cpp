...
BOOL CFmiTempSettingsDlg::OnInitDialog()
{
	...
	itsTempSystem->SkewTDegree(-1); // kikka, pit�� asettaa negatiivinen luku, jotta saadaan sit� vastaavat alku ja loppu arvot ulos
	itsTStart1 = itsTempSystem->TAxisStart();
	itsTEnd1 = itsTempSystem->TAxisEnd();
	itsTempSystem->SkewTDegree(0); // kikka, pit�� asettaa negatiivinen luku, jotta saadaan sit� vastaavat alku ja loppu arvot ulos
	...
	// huom indeksi SoundingColor-metodiin yhden pienempi kuin muuttuja nimiss�!!
...
{
	UpdateData(TRUE);
	// Kopioidaan mahd. muutokset takaisin alkuper�isiin
...
{
	// label info on feikki, koska sit� ei voi kuitenkaan muokata
...
{
	// labelInfo on feikki, koska sit� ei voi kuitenkaan muokata
...
{
	// lineInfo on feikki, koska sit� ei voi kuitenkaan muokata
    ...
    NFmiTempLabelInfo *theLabelInfo)
{
	...
	if(theLineInfo)
	{
		...
		// GDI ei osaa piirt�� yht� pikseli� paksumpia erikois viivoja (eli type muu kuin yhten�inen
		// joten jouduin viritt�m��n v�h�n piirtoa ja n�iss� demo kuvissa pit�� kanssa piirt�� feikaten millaiset
		// viivat oikeasti piirtyv�t luotaus kuviin)
		...
		dc->SelectObject(oldPen); // laitetaan vanha kyn� takaisin
		myPen.DeleteObject(); // resurssit pit�� vapauttaa viel�
		...
		{ // vedet��n ruksi koko jutun p��lle, jos apu viivaa ei piirret�
		...
		{ // t�ll�in piirret��n ruksi vain demo labelin p��lle, ett� sit� ei piirret�
...
// HUOM! En saanut luotausn�yt�n ruudunp�ivityst� toimimaan, joten disabloin Refresh -nappulan ja laitoin sen n�kym�tt�m�ksi.
