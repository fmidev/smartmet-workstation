...
BOOL CTimeEditValuesDlg::OnInitDialog() 
{
    ...
    itsTimeEditValuesView->OnInitialUpdate(); // pit‰‰ kutsua erikseen, koska formvieta ei ole sidottu dokumenttiin
	...
	EnableButtons();
	SeViewModeButtonsSetup(); // t‰t‰ kutsutaan vain kerran ja laittaa monet kontrollit pois p‰‰lt‰, jos ollaan ns. view-moodissa
	...
	fUseAnalyzeTool = FALSE; // alussa analyysi tyˆkalu ei ole k‰ytˆss‰
...
// T‰m‰ Aikasarjamuokkaus toiminnon progress/peruutus ei toimi, niin ett‰ laitetaan
// Muokkaus koodi threadiin ja Progress-Cancel-dialogi p‰‰threadiin osottamaan.
// T‰st‰ seurasi ongelmia, kun SmartMet lukee dataa taustalla ja p‰ivittelee ruutuja.
// T‰llˆin tulee v‰ist‰m‰tt‰ ongelmia, koska SmartMetin datanmuokkaus koodeja ei ole
// tehty thread-turvallisesti.
// Kokeile niin ett‰ datan muokkaus tapahtuu p‰‰s‰ikeess‰ ja progress-cancel-dialogi on omassa threadissa.
// Kokeilin ja tuomio on se ett‰ mit‰‰n GUI-juttuja ei voi ajaa main-threadin ulkopuolella, joten ainoa
// oikea keino on tehd‰ SmartMetin datan muokkauksesta threadi turvallinen.
/*
...
void CTimeEditValuesDlg::OnButtonToiminto() 
{
	...
	// Tehd‰‰n aikamuokkaukselle progress ja peruutus dialogi ja toiminnot.
	...
	if(status == IDCANCEL) // Jos k‰ytt‰j‰ keskeytti tyˆn, pit‰‰ tehd‰ undo.
		OnButtonUndo();
...
void CTimeEditValuesDlg::OnButtonToiminto() 
{
	...
	if(itsSmartMetDocumentInterface->MetEditorOptionsData().ControlPointMode())
		EnableDlgItem(IDC_BUTTON_TOIMINTO, false); // koska mm. kontrollipiste muokkaus menee nyky‰‰n omaan threadiin ja p‰‰karttan‰yttˆ lukitaan Proegress-cancelointi -dialogilla, 
													// pit‰‰ t‰ss‰ est‰‰ ett‰ ei voi painaa uudestaan Muokkaa-nappulaa. Napin normaali tila palautetaan kun l‰hetet‰‰n viesti ID_MESSAGE_TIME_SERIAL_MODIFICATION_ENDED
	bool anyDataModified = itsTimeEditValuesView->ChangeTimeSeriesValues();

	// HUOM! jos mit‰‰n dataa ei muokattu kontrollipite moodissa, pit‰‰ nappuloiden tila palauttaa taas ennalleen ja ruutuja p‰ivitt‰‰!
...
{
	DoWhenClosing();
	ShowWindow(SW_HIDE); // suljettaessa vain piilotetaan, tuhoaminen tehd‰‰n ulkoak‰sin
}
...
void CTimeEditValuesDlg::SeViewModeButtonsSetup(void)
{
    ...
    // Riippuen ollaanko ns. view-moodissa vai ei, disabloidaan/enabloidaan editoinnissa k‰ytettyj‰ nappuloita
...
// asettaa sen 'valittu muokkaus vaikutusalue'-s‰‰timen koon (askelten m‰‰r‰n) dynaamisesti 
// riippuen datassa olevista aikojen lukum‰‰r‰st‰
...
{
	if(fUseAnalyzeTool) // jos analyysi-moodi on p‰‰ll‰, pit‰‰ moni kontrolli est‰‰ ja muutama mahdollistaa ja tuoda n‰kyviin
	{
		UpdateProducerLists();
...
{
	if(itsSmartMetDocumentInterface && itsSmartMetDocumentInterface->SmartMetEditingMode() == CtrlViewUtils::kFmiEditingModeNormal) // tehd‰‰n n‰it‰ asetuksia vain ns. edit-moodissa
	{
		// analyysi tyˆkalu on enabloitava aina, koska tuottajaa voidaan vaihtaa...
		bool status = EnableDlgItem(IDC_CHECK_USE_ANALYZE_TOOL, true);
		UpdateControlsAfterAnalyzeMode();
/* 
		if(itsSmartMetDocumentInterface->EnableAnalyzeTool()) // jos shortrange ja analyysi dataa lˆytyy
		{
			bool status = EnableDlgItem(IDC_CHECK_USE_ANALYZE_TOOL, true);
			UpdateControlsAfterAnalyzeMode();
		}
		...
		{
			fUseAnalyzeTool = FALSE; // jos ei voi olla k‰ytˆss‰, otetaan se pois p‰‰lt‰!
			itsSmartMetDocumentInterface->AnalyzeToolData().AnalyzeToolMode(fUseAnalyzeTool == TRUE); // asetetaan viel‰ moodi varmuuden vuoksi dociin!
...
{
	// jos ollaan CPManager-tekstin p‰‰ll‰, rullalla voi vaihtaa edelliseen/seuraavaan listalla olevaan CPManageriin
...
// T‰m‰ PreTranslateMessage-k‰sittely mahdollistaa pikan‰pp‰inten k‰sittelyn (kelluvassa) dialogissa.
...
void CTimeEditValuesDlg::OnAcceleratorToggleTooltip()
{
	if(itsSmartMetDocumentInterface)
	{
		if(itsSmartMetDocumentInterface->ShowToolTipTimeView())
            itsTimeEditValuesView->HideToolTip(); // jostain syyst‰ aikasarjaikkunassa tooltip ei itsest‰‰n h‰vi‰ (niin kuin muissa ikkunoissa k‰y)
...
{ // t‰m‰ on siis SPACE-n‰pp‰imen handleri, jolla tehd‰‰ aikasarjojen auto s‰‰tˆ
...
// T‰ss‰ tutkit‰‰n analyysi tyˆkalun valintoja, ja tutkitaan
// onko analyysityˆkalun toiminta mahdollista. Lˆytyykˆ dataa ja
// onko datojen ajoissa sellaisia aikoja ett‰ modifikaatio on mahdollista.
...
void CTimeEditValuesDlg::UpdateProducerList(CComboBox &theProducerSelector, const NFmiProducer &theLastSessionProducer)
{
	...
	{ // yritet‰‰n lˆyt‰‰ edellisen smartmet session viimeksi valittua
	...
	{ // toiseksi yritet‰‰n viimeksi auto valittua
	...
	{ // kolmanneksi valitaan vain listalta ensimm‰inen mik‰ lˆytyy
...
BOOL CTimeEditValuesDlg::OnWndMsg(UINT message, WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
	...
	{
		EnableDlgItem(IDC_BUTTON_TOIMINTO, true); // enabloidaan aikasarja muokkausten j‰lkeen taas muokkaa-nappi
		EnableButtons();
...
HBRUSH CTimeEditValuesDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
	...
	if(pWnd->GetDlgCtrlID() == IDC_STATIC_GRIDDING_FACTOR_STR)
	{
		if(itsSmartMetDocumentInterface->CPGriddingFactor() > 0 && itsSmartMetDocumentInterface->CPGriddingFactor() < 1)
			pDC->SetTextColor(RGB(255, 0, 0)); // varoitetaan punaisella v‰rill‰, jos k‰ytˆss‰ on hilauksen tarkkuutta harventava kerroin
		else
