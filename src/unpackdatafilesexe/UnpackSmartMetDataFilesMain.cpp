...
static void DoFilesOnDirectory(const std::string& currentDirectory, const std::string& parallerDirectory, int changeTimesByHour)
{
    ...
    for(const auto& queryDataFileName : files)
    {
        ...
        {
            // Odotetaan jokaisen tiedoston v‰lill‰ niin ett‰ v‰li‰ on v‰hint‰in 1.5 sekuntia, t‰llˆin jokainen
            // luotu tiedosto saa oman luomishetki sekunnin, ja t‰llˆin smartmet saa luettavakseen eri malliajo tiedostot
            // oikeassa aikaj‰rjestyksess‰.
