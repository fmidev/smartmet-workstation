...
namespace
{
	// T�m�n avulla yritet��n lopettaan jatkuvasti py�riv� working thread 'siististi'
	...
	// t�t� k�ytet��n mm. CaseStudy-datan yhteydess�, t�ll� varmistetaan ett� uudet data polut tulevat k�ytt��n
	...
	// Tietyt data poistuvat k�yt�st�, mutta niiden k�ytt�� pit�� tukea, kun ollaan CaseStudy moodissa,
...
// palauttaa true, jos uutta dataa tuli k�ytt��n, muuten jos lukitus failaa, tai ei ollut dataa, palautetaan false.
	...
	if(theDataInfo.DataType() != NFmiInfoData::kSatelData) // satelliitti data on kuva, eik� sit� ladata t��ll�
	{
		...
		try
		{
            ...
            if(data)
            {
                ...
                data->LatLonCache(); // T�m� alustaa latlon cachen worker threadissa, jotta se olisi sitten k�yt�ss� SmartMetissa multi-thread koodeissa
		...
		for( ; ; counter++)
		{
			...
			if(gFirstTimeGoingThrough || gDoDataCheckNow || loadingStatus || timer.CurrentTimeDiffInMSeconds() > (30 * 1000)) // tarkistetaan v�hint��n puolen minuutin v�lein onko tullut uusia datoja
			{
				...
				if(gFirstTimeGoingThrough)
				{
					...
					// pois p��lt�, jotta systeemi ehtii ottaa kierroksella tulleet datat k�ytt��n ennen moodin vaihtoa,
					Sleep(1 * 1000);
				}
