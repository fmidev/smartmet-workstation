...
namespace
{
	CSemaphore gThreadRunning; // t�m�n avulla yritet��n lopettaan jatkuvasti py�riv� working thread 'siististi'
	NFmiStopFunctor gStopFunctor;
	bool fDoUpdateNow; // t�ll� voidaan pakottaa macroParam update v�litt�m�sti
	int gStartUpDelayInMS = 0; // t�m�n avulla voidaan s��t�� kuinka kauan alussa odotellaan, ennen kuin tehd��n ty�t ensimm�isen kerran
...
bool CFmiMacroParamUpdateThread::MakePossibleMacroParamSystemUpdateInDoc(std::shared_ptr<NFmiMacroParamSystem> theMacroParamSystemPtr)
{
	...
	if(theMacroParamSystemPtr->IsUpdateNeeded(*gMediatorMacroParamSystemPtr))
	{
		...
		gMediatorMacroParamSystemPtr = std::make_unique<NFmiMacroParamSystem>();
		gMediatorMacroParamSystemPtr->Init(*theMacroParamSystemPtr);
		// Asetetaan p�ivitetty macroParamSystem otus dokumentille k�ytt��n
