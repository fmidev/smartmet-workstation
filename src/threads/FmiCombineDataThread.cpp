...
namespace
{
	CSemaphore gCombineDataThreadRunning; // t�m�n avulla yritet��n lopettaan jatkuvasti py�riv� working thread 'siististi'
	CSemaphore gSoundingIndexDataThreadRunning; // t�m�n avulla yritet��n lopettaan jatkuvasti py�riv� working thread 'siististi'

	NFmiStopFunctor gCombineDataStopFunctor, gSoundingIndexDataStopFunctor;
	int gCombineDataStartUpDelayInMS = 0; // t�m�n avulla voidaan s��t�� kuinka kauan alussa odotellaan, ennen kuin tehd��n ty�t ensimm�isen kerran
	int gSoundingIndexStartUpDelayInMS = 0; // t�m�n avulla voidaan s��t�� kuinka kauan alussa odotellaan, ennen kuin tehd��n ty�t ensimm�isen kerran
	...
	public:
		...
		std::string itsTargetFileFilter; // yhdistelm� tiedosto polkuineen ja filefilttereineen. nimess� pit�� olla timestamppi minuutteja my�ten, aikaleima laitetaan *-merkin tilalle
...
#undef CreateDirectory // win32 systeemit m��rittelev�t CreateDirectory-makron, joka otetaan t�ss� pois k�yt�st� varmuuden vuoksi
#endif
...
void CFmiSoundingIndexDataThread::InitSoundingIndexDataInfos(NFmiHelpDataInfoSystem &theHelpDataInfoSystem, const std::string &autoGeneratedSoundingIndexBasePath)
{
	...
	const auto &helpDataInfoVector = theHelpDataInfoSystem.DynamicHelpDataInfos();
	std::vector<NFmiHelpDataInfo> newHelpDataInfos; // t�h�n lis�t��n uudet datat, jotka pit�� rekister�id�
	for(size_t i = 0; i < helpDataInfoVector.size(); i++)
	{
		...
		{ 
			...
			// tee target hakemistot ja filefilterit automaattisesti (miss� rekister�id��n ne luettaviksi infoOrganizeriin???, pit�isik� t�m� luoda GenDocin init:iss�?)
			...
			// Lis�ksi luodut data pit�� rekister�id�, ett� SmartMet lukee ne normaali k�yt�nn�n mukaisesti.
...
//																					// kuin ennen, eiv�t jo luodut n�ytt�makrot toimi n�iden kanssa en��. Tekisi mieli tehd� t�m� toisin...
			newHelpDataInfos.push_back(hInfo);
		}
	}
	// rekiter�id��n uudet datat
...
{
	// T�h�n tiedostoon data kirjoitetaan v�liaikaisesti aluksi, ja sellaisella nimell� ett� SmartMet ei tarraa siihen kiinni kesken kaiken
	std::string itsOutputTmpFilePath;
	// T�h�n talletetaan data lopullisesti rename:n avulla edell� olevasta tiedostosta
	...
	SoundingIndexOutpuFileNameMaker(SoundingIndexDataInfo& dataInfo)
	{
		...
		std::string timeStampStr = static_cast<char*>(currentTime.ToStr(kYYYYMMDDHHMM)); // tehd��n minuutin tarkkuudella aikaleima, t�ll�in jos toinen k�ynniss� oleva SmartMet tekee jo ennenmin tiedoston, t�t� ei tarvitse tallentaa
...
static void	DoSoundingIndexDataWork()
{
	...
	// TODO soundingIndex datojea voisi my�s p�ivitt��, kun tulee uutta mesa analyysej� ja havaintoja, jolloin niille ajoille voisi laskea havaintojen avulla tietyt parametrit uudestaan
	...
	// 2. tee sounding index data
			...
			try
			{
				...
				std::string producerNameStr; // TODO: laita t�h�n kuvaava nimi
				 // otetaan k�ytt��n n. 40 % koneen s�ikeist�
			...
			// 3. tarkista ensin ett� ja talleta queryData tiedostoon oikeaan cache-hakemistoon, mutta v��r�ll� nimell� ja lopuksi tee rename jolloin tiedosto on halutun niminen
				if(NFmiFileSystem::FileExists(fileNameMaker.itsOutputFilePath) == false && NFmiFileSystem::FileExists(fileNameMaker.itsOutputTmpFilePath) == false)
				{
					...
					NFmiFileSystem::CleanDirectory(dataInfo.itsTargetDirectory, maxFileAgeInHours); // tuhotaan tuntia vanhemmat tiedostot, ei voida k�ytt�� fileFilter siivousta, t�ll�in hakemistoon j�isi mahdolliset tmp-tiedostot
				}
				else
				{
